...
require_once('ExcelImportClass.php');
require_once('ExcelImportCheck.php');
                            ...
                            $userID,$kwMap,$duplicatedLogic = array('hitCriteria' => 'name', 'actionOnHit' => null))
{
  ...
  for($idx = 0; $idx <$tc_qty ; $idx++)
  {
    ...
    $tc_id=isset($tc['tc_id']) ? $tc['tc_id'] : '';
    $bpm_id=isset($tc['bpm_id']) ? $tc['bpm_id'] : '';
    $complexity=isset($tc['complexity']) ? $tc['complexity'] : 1;
    $reviewed_status=isset($tc['reviewed_status']) ? $tc['reviewed_status'] : 1;
    $reviewer_id=isset($tc['reviewer_id']) ? $tc['reviewer_id'] : 0;
        ...
        switch($tcase_qty)
        {
           case 1:
             ...
             $ret = $tcase_mgr->update($tcase_id,$tcversion_id,$name,$summary,
                                       ...
                                       $node_order,$exec_type,$importance,$tc_id,$bpm_id,
                                       $complexity,$reviewed_status,$reviewer_id,$attr,$updOpt);
    ...
    {     
        ...
        if ($ret = $tcase_mgr->create($container_id,$name,$summary,$preconditions,$steps,
                                      ...
                                      $exec_type,$importance,$tc_id,$bpm_id,$complexity,$reviewed_status,$reviewer_id,$createOptions))
        {
          $resultMap[] = array($name,$ret['msg']);
        }  
    }
...
function processRequirements(&$dbHandler,&$reqMgr,$tcaseName,$tcaseId,$tcReq,$reqSpecSet,$messages,$userID)
{
  ...
  foreach($tcReq as $ydx => $value)
  {
    ...
    if($doit)
    {
      ...
      {
        $sql = " SELECT REQ.id from " . $dbHandler->get_table('requirements')." REQ " .
...
/* function:importTestCaseDataFromSpreadsheet
                                          从excel中导入测试用�?			args:db[reference]:db object
			fileName:XLS file name
			parentID:testcase parent node(container)
			tproject_id:testProject where to import testcase
			...
			return:map
*/
function importTestCaseDataFromSpreadsheet(&$db,$fileName,$parentID,$tproject_id,$userID,$options=null)
{
    $excel_import = new ExcelImportClass($db);
    $php_excel = $excel_import->create_php_excel($fileName);
    
    $excel_check = new ExcelImportCheck($db);
    $duplicate_logic = $options['duplicateLogic'];
    $action_on_duplicated_name = $duplicate_logic['actionOnHit'];
    
    $excel_check->import_check($php_excel);
    $result_msg = $excel_import->import_testcase_from_excel($php_excel, $action_on_duplicated_name,
        $excel_check->get_new_keyword_list());
    return $result_msg;
}
