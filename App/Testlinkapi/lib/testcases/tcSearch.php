...
$ga = initializeGui($args,$tproject_mgr,$db);
...
if ($args->tprojectID && $args->doAction == 'doSearch')
{
  ...
  if($args->keyword_id)       
  {
  	 $from['by_keyword_id'] = " JOIN " . $db->get_table('testcase_keywords')." KW ON KW.testcase_id = NH_TC.id ";
  ...
  if($args->custom_field_id > 0)
  {
    ...
    $from['by_custom_field']= " JOIN " . $db->get_table('cfield_design_values')." CFD " .
  ...
  if($args->requirement_doc_id != "")
  {
    ...
    $from['by_requirement_doc_id'] = " JOIN " . $db->get_table('req_coverage')." RC" .  
                                     " ON RC.testcase_id = NH_TC.id " .
                                     " JOIN " . $db->get_table('requirements')." REQ " .
                                     " ON REQ.id=RC.req_id " ;
  ...
  if( $args->tc_id !="")
  {
      $filter['tc_id'] = " AND TCV.tc_id LIKE '%{$args->tc_id}%' ";
  }
  
  if( $args->complexity > 0)
  {
      $filter['complexity'] = " AND TCV.complexity = {$args->complexity} ";
  }
  
  if( $args->bpm_id !="")
  {
      $filter['bpm_id'] = " AND TCV.bpm_id LIKE '%{$args->bpm_id}%' ";
  }
  
  if( $args->reviewer_id > 0)
  {
      $filter['reviewer_id'] = " AND TCV.reviewer_id = {$args->reviewer_id} ";
  }
  
  if( $args->reviewed_status > 0)
  {
      $filter['reviewed_status'] = " AND TCV.reviewed_status = {$args->reviewed_status} ";
  }
  ...
  if( $args->created_by != '' )
  {
    $from['users'] .= " JOIN " . $db->get_table('users')." AUTHOR ON AUTHOR.id = TCV.author_id ";
  ...
  if( $args->edited_by != '' )
  {
    $from['users'] .= " JOIN ".$db->get_table('users')." UPDATER ON UPDATER.id = TCV.updater_id ";
  ...
  $sqlPart2 = " FROM " . $db->get_table('nodes_hierarchy')." NH_TC " .
              " JOIN " . $db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.parent_id = NH_TC.id  " .
              " JOIN " . $db->get_table('tcversions')." TCV ON NH_TCV.id = TCV.id " .
              " LEFT OUTER JOIN ".$db->get_table('nodes_hierarchy')." NH_TCSTEPS ON NH_TCSTEPS.parent_id = NH_TCV.id " .
              " LEFT OUTER JOIN ".$db->get_table('tcsteps')." TCSTEPS ON NH_TCSTEPS.id = TCSTEPS.id  " .
...
function init_args(&$tprojectMgr)
{
  ...
  $iParams = array("doAction" => array(tlInputParameter::STRING_N,0,10),
                   ...
                   "tc_id" => array(tlInputParameter::STRING_N,0,50),
                   "complexity" => array(tlInputParameter::INT_N),
                   "bpm_id" => array(tlInputParameter::STRING_N,0,50),
                   "reviewer_id" => array(tlInputParameter::INT_N),
                   "reviewed_status" => array(tlInputParameter::INT_N),
 ...
 *
 */
function initializeGui(&$argsObj,&$tprojectMgr, $db)
{
  $gui = new stdClass();
  ...
  $gui->option_complexity = array('0'=>'','1'=>lang_get('very_simple'),'2'=>lang_get('little_simple'),
      '3'=>lang_get('general'),'4'=>lang_get('complex'),'5'=>lang_get('very_complex'));
  $gui->option_reviewed_status = array('0'=>'','1'=>lang_get('wait_review'),'2'=>lang_get('reviewed_passed'),
      '3'=>lang_get('reviewed_not_pass'));
  $gui->option_reviewer_id=array();
  $gui->option_reviewer_id = get_select_tcproject_user_id(intval($argsObj->tprojectID), $db);
...
function initSearch(&$gui,&$argsObj,&$tprojectMgr)
{
  ...
  $gui->tc_id = (is_null($argsObj->tc_id) || $argsObj->tc_id == '') ? '' : $argsObj->tc_id;
  $gui->complexity = intval($argsObj->complexity);
  $gui->bpm_id = (is_null($argsObj->bpm_id) || $argsObj->bpm_id == '') ? '' : $argsObj->bpm_id;
  $gui->reviewer_id = intval($argsObj->reviewer_id);
  $gui->reviewed_status = intval($argsObj->reviewed_status);
