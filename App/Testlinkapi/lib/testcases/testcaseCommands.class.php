...
class testcaseCommands
{
  ...
  function __construct(&$db,&$userObj,$tproject_id)
  {
    ...
    $this->complexity = $this->tcaseMgr->getComplexity();
    $this->reviewed_status = $this->tcaseMgr->getReviewed_status();
  ...
  function initGuiBean(&$argsObj)
  {
    ...
    $obj->complexity = $this->complexity;
    $obj->reviewed_status = $this->reviewed_status;
    
    $parent_id=$argsObj->container_id;
    if($parent_id==0){
        $parent_id=$argsObj->tcase_id;
    }
    $obj->reviewer_id = get_select_tcproject_user_id($argsObj->tproject_id, $this->db);
  ...
  function doCreate(&$argsObj,&$otCfg,$oWebEditorKeys,$request)
  {
    ...
    $tcase = $this->tcaseMgr->create($argsObj->container_id,$argsObj->name,$argsObj->summary,$argsObj->preconditions,
                                     ...
                                     $argsObj->exec_type,$argsObj->importance,$argsObj->tc_id,$argsObj->bpm_id,
                                     $argsObj->complexity,$argsObj->reviewed_status,
                                     $argsObj->reviewer_id,$options);
  ...
  */
  function doUpdate(&$argsObj,$request)
  {
    ...
    /**
       ...
       */
    if(trim($argsObj->name) != trim($_REQUEST['tmp_testcase_name'])){  
    //if(!isset($_SESSION['jim_case_title_step_change'])){
        $this->tcaseMgr->testCaseStepAndTitleChangeToLog($argsObj->tcversion_id);  
    ...
    $ret = $this->tcaseMgr->update($argsObj->tcase_id, $argsObj->tcversion_id, $argsObj->name, 
                                   ...
                                   $argsObj->importance,$argsObj->tc_id,$argsObj->bpm_id,
                                   $argsObj->complexity,$argsObj->reviewed_status,
                                   $argsObj->reviewer_id,$options);
    ...
    if( $op['status_ok'] )
    {
      ...
      $this->initTestCaseBasicInfo($argsObj,$guiObj);
       /**
         ...
         */
        //if(!isset($_SESSION['jim_case_title_step_change'])){
        if($argsObj->steps != '' && $argsObj->expected_results != ''){
          $this->tcaseMgr->testCaseStepAndTitleChangeToLog($argsObj->tcversion_id);  
        }
        //  $_SESSION['jim_case_title_step_change'] = 1;
        //}
   ...
   * doUpdateStep
   *
   *modify:have not a little update not save
   *
   */
  function doUpdateStep(&$argsObj,$request)
  {
    ...
    /*
    $t1 = $this->html_chars_change(trim(strip_tags($old_step_info['expected_results'])));
    $t1 = htmlspecialchars($t1);
    var_dump($t1);
    echo "<br/>";    
    $t2 = $this->html_chars_change(trim(strip_tags($argsObj->expected_results)));
    $t2 = htmlspecialchars($t2);
    var_dump($t2);
    if($t1 == $t2)
    {
      echo 'same expected_results';echo "<br/>";
    }else{
      echo 'diff expected_results';echo "<br/>";
    }
    
   
    var_dump($this->html_chars_change(trim(strip_tags($old_step_info['actions']))));
    $t1 = $this->html_chars_change(trim(strip_tags($old_step_info['actions'])));
    echo "<br/>";
    var_dump($this->html_chars_change(trim(strip_tags($argsObj->steps))));
    $t2 = $this->html_chars_change(trim(strip_tags($argsObj->steps)));
    if($t1 == $t2)
    {
      echo 'same actions';echo "<br/>";
    }else{
      echo 'diff actions';echo "<br/>";
    }
    */

    if($old_step_info['step_number'] == intval($argsObj->step_number)    
    && $this->html_chars_change(trim(strip_tags($old_step_info['actions']))) == $this->html_chars_change(trim(strip_tags($argsObj->steps)))
    && $this->html_chars_change(trim(strip_tags($old_step_info['expected_results']))) == $this->html_chars_change(trim(strip_tags($argsObj->expected_results)))
    && intval($old_step_info['execution_type']) == $this->db->prepare_int($argsObj->exec_type)
    ){
      $title = '什么都没修改';
   ...
   * history:2016.6.7
   * **/
  function settc_id(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->tc_id = $argsObj->tc_id;
      $guiObj->tcversion_id = $argsObj->tcversion_id;
  
      $this->initTestCaseBasicInfo($argsObj,$guiObj);
  
      $this->tcaseMgr->settc_id($argsObj->tcversion_id,$argsObj->tc_id);
      $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,$argsObj->user_id);
   ...
   * history:2016.6.8
   * **/
  function setAllFields(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->tc_id = $argsObj->tc_id;
      $guiObj->complexity = $argsObj->complexity;
      $guiObj->bpm_id = $argsObj->bpm_id;
      $guiObj->reviewer_id = $argsObj->reviewer_id;
      $guiObj->reviewed_status = $argsObj->reviewed_status;
      $guiObj->step_exec_type = $argsObj->exec_type;
      ...
      $this->tcaseMgr->settc_id($argsObj->tcversion_id,$argsObj->tc_id);
      $this->tcaseMgr->setComplexity($argsObj->tcversion_id,$argsObj->complexity);
      $this->tcaseMgr->setBpm_id($argsObj->tcversion_id,$argsObj->bpm_id);
      $this->tcaseMgr->setReviewer_id($argsObj->tcversion_id,$argsObj->reviewer_id);
      $this->tcaseMgr->setReviewed_status($argsObj->tcversion_id,$argsObj->reviewed_status);
      $this->tcaseMgr->setStatus($argsObj->tcversion_id,$argsObj->status);
      $this->tcaseMgr->setImportance($argsObj->tcversion_id,$argsObj->importance);
      $this->tcaseMgr->setExecutionType($argsObj->tcversion_id,$argsObj->exec_type);
      $this->tcaseMgr->setEstimatedExecDuration($argsObj->tcversion_id,$argsObj->estimatedExecDuration);
   ...
   * history:2016.6.8
   * **/
  function setComplexity(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->complexity = $argsObj->complexity;
   ...
   * history:2016.6.8
   * **/
  function setReviewed_status(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->reviewed_status = $argsObj->reviewed_status;
   ...
   * history:2016.6.8
   * **/
  function setReviewer_id(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->reviewer_id = $argsObj->reviewer_id;
   ...
   * history:2016.6.8
   * **/
 /*  function setAuthor_id(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->author_id = $argsObj->author_id;
      $guiObj->tcversion_id = $argsObj->tcversion_id;
  
      $this->initTestCaseBasicInfo($argsObj,$guiObj);
  
      $this->tcaseMgr->setAuthor_id($argsObj->tcversion_id,$argsObj->author_id);
      $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,$argsObj->user_id);
   ...
   * history:2016.6.8
   * **/
  function setBpm_id(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->bpm_id = $argsObj->bpm_id;
   ...
   * history:2016.6.8
   * **/
  /* function setCreation_ts(&$argsObj,$request)
  {
      $guiObj = $this->initGuiBean($argsObj);
      $guiObj->user_feedback = '';
      $guiObj->creation_ts = $argsObj->creation_ts;
      $guiObj->tcversion_id = $argsObj->tcversion_id;
  
      $this->initTestCaseBasicInfo($argsObj,$guiObj);
  
      $this->tcaseMgr->setCreation_ts($argsObj->tcversion_id,$argsObj->creation_ts);
      $this->tcaseMgr->update_last_modified($argsObj->tcversion_id,$argsObj->user_id);
