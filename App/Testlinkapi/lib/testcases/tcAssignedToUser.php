...
if($args->result != '' && $args->tcvx > 0)
{
  ...
  // get version number
  $sql =  " SELECT TCV.version FROM  ".$db->get_table('tcversions')." TCV WHERE TCV.id = " . $args->tcvx;
  $xx = $db->get_recordset($sql);
  $version_number = $xx[0]['version'];

  $sql = " INSERT INTO ".$db->get_table('executions')." ".
...
{   
  ...
  $sql="SELECT name,id FROM ".$db->get_table('nodes_hierarchy')." " .
