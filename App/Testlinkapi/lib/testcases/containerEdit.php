...
switch($action)
{
  ...
  case 'reorder_testcases':
    reorderTestCasesByCriteria($args,$tsuite_mgr,$tree_mgr,$sortCriteria, $db);
    $gui = initializeGui($tsuite_mgr,$args->testsuiteID,$args,$l18n);
                             ...
                             $argsObj,$feedback='',$cf=null)
{
  ...
  $sqlA = " SELECT MAX(TCV.version) AS lvnum, NHTC.node_order, NHTC.name, NHTC.id, TCV.tc_external_id AS tcexternalid" .
          " FROM " . $dbHandler->get_table('nodes_hierarchy')." NHTC " .
          " JOIN " . $dbHandler->get_table('nodes_hierarchy')." NHTCV ON NHTCV.parent_id = NHTC.id " .
          " JOIN " . $dbHandler->get_table('tcversions')." TCV ON TCV.id = NHTCV.id " .
          " JOIN " . $dbHandler->get_table('node_types')." NT ON NT.id = NHTC.node_type_id AND NT.description='testcase'" .
  ...
  $sqlB = " SELECT SQLA.id AS tcid, SQLA.name AS tcname,SQLA.node_order AS tcorder, SQLA.tcexternalid," . 
          ...
          " JOIN " . $dbHandler->get_table('nodes_hierarchy')." MNHTCV ON MNHTCV.parent_id = SQLA.id " .
          " JOIN " . $dbHandler->get_table('tcversions')." MTCV ON MTCV.id = MNHTCV.id AND MTCV.version = SQLA.lvnum";
  $orderClause = " ORDER BY TCORDER,TCNAME";        
 ...
 *
 */
function reorderTestCasesByCriteria($argsObj,&$tsuiteMgr,&$treeMgr,$sortCriteria, $dbHandler)
{
    $pfn = ($sortCriteria == 'NAME') ? 'reorderTestCasesDictionary' : 'reorderTestCasesByExtID';
    $pfn($argsObj,$tsuiteMgr,$treeMgr, $dbHandler);
}
 ...
 *
 */
function reorderTestCasesDictionary($argsObj,&$tsuiteMgr,&$treeMgr, $dbHandler)
{
 ...
 *
 */
function reorderTestCasesByExtID($argsObj,&$tsuiteMgr,&$treeMgr, $dbHandler)
{
    ...
    $sql = " SELECT DISTINCT NHTC.id,TCV.tc_external_id " .
                    ...
                    " JOIN " . $dbHandler->get_table('nodes_hierarchy')." NHTCV ON NHTCV.parent_id = NHTC.id " .
                    " JOIN " . $dbHandler->get_table('tcversions')." TCV ON TCV.id = NHTCV.id " .
                    " JOIN " . $dbHandler->get_table('testsuites')." TS ON NHTC.parent_id = TS.id " .
