...
class jiradbInterface extends issueTrackerInterface
{
  ...
  function getIssue($issueID)
  {
    ...
    {  
      ...
      $addJoin = " JOIN " . $this->dbConnection->get_table($this->dbConnection->project) . " PROJECT ON ISSUES.project = PROJECT.id ";
    ...
    $sql = "/* $debugMsg */ " .
           ...
           " FROM " . $this->dbConnection->get_table($this->dbSchema->issues) . " ISSUES " .
           " JOIN " . $this->dbConnection->get_table($this->dbSchema->status) . " ST ON ST.ID = ISSUES.issuestatus " .
           $addJoin . $where;
  ...
  function getStatuses()
  {
    ...
    // Field names on Jira tables seems to be sometimes on CAPITALS
    $sql = "/* $debugMsg */ " .
           " SELECT ST.ID AS id,ST.pname AS name FROM " . $this->dbConnection->get_table($this->dbSchema->status). " ST";
    try
    {
      $rs = $this->dbConnection->fetchRowsIntoMap($sql,'id');
