...
function initPrintOpt(&$UIhash,&$docInfo)
{
  ...
  $pOpt = array( 'toc' => 0,'body' => 0,'summary' => 0, 'header' => 0,'headerNumbering' => 1,
                 ...
                 'req_spec_scope' => 0,'req_business_rules'=> 0,'req_add_remaks'=> 0,
                 'req_add_definitions'=> 0,'req_testplans_id'=> 0,'req_tpname'=> 0,
                 'req_spec_author' => 0,'build_cfields' => 0,
                                 ...
                                 $pnFilters=null,$opt=null)
{
  ...
  $filters = array();
  if (array_key_exists('build_id', $ctx))
  {
  ...
  foreach($platformIDSet as $platform_id)  
  {
    ...
    $filters['setAssignedTo'] = $pnOptions['setAssignedTo'];
                                       ...
                                       &$tplanMgr,$options=null)
{
  ...
  if($docInfo->type == DOC_TEST_PLAN_EXECUTION_ON_BUILD)
  {
    ...
    // 20160113 changed by zhouzhaoxin for to get really executed testcases
    if (isset($options['setAssignedTo']) && $options['setAssignedTo'])
    {
