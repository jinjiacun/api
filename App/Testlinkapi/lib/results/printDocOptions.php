...
switch($args->doc_type) 
{
  ...
  case DOC_TEST_PLAN_EXECUTION_ON_BUILD:
    ...
    {
      case 'addTC':
        $gui->onlyShowPlan = true;
        $gui->testplanName = $testplan_name;
      ...
      default:
            ...
            && $gui->buildInfoSet != null)
        {
            ...
            $url_array = parse_url($req_url);
            $array_query = convert_url_params($url_array['query']);
            if (array_key_exists("build_id", $array_query))
            {
                $args->build_id = $array_query["build_id"];
                unset($array_query["build_id"]);
                
                $gui->docSourceUrl = "http://" . $url_array['host'] . $url_array[path] . "?" .
                    build_url_without_bd($array_query);
            }
            else 
            {
                $args->build_id = $tplan_mgr->get_max_build_id($args->tplan_id);
                $gui->docSourceUrl = $req_url;
            }
            $gui->selectde_build = $args->build_id;
        }
        ...
        list($treeContents, $testcases_to_show) = testPlanTree($db,$rightPaneAction,$args->tproject_id,
            ...
            $testplan_name, $args->build_id, $filters,$opt_etree);
...
function init_checkboxes(&$args) 
{
  ...
  {
    case 'reqspec':
      $key2init = array('req_spec_scope','req_business_rules','req_add_remaks',
                        'req_add_definitions','req_tpname',
                        'req_spec_author','req_spec_overwritten_count_reqs',
      ...
      $key2init2yes = array('req_spec_scope' => 'y','req_scope' => 'y',
          'req_add_definitions' => 'y', 'req_business_rules' => 'y', 
          'req_add_remaks' => 'y', 'req_tpname' => 'y');
...
//20170112 add by zhouzhaoxin for convert url params
function convert_url_params($url_query)
{
    $query_part = explode('&', $url_query);
    $params = array();
    foreach ($query_part as $param)
    {
        $item = explode('=', $param);
...
//20170112 add by zhouzhaoxin for build url without build_id
function build_url_without_bd($query_array)
{
    $tmp = array();
    foreach ($query_array as $key => $param) 
    {
        $tmp[] = $key . "=" . $param;
    }
