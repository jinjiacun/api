...
function initializeGui(&$dbHandler,&$args,$images)
{
  ...
  $gui->l18n = init_labels(array('tcversion_indicator' => null,
                                 ...
                                 'testcases_count_by_case_exec_per_day_user' => null,
                                 'design' => null, 
                                 'execution' => null, 
  ...
  $gui->pageTitle = sprintf($gui->l18n['testcases_count_by_case_exec_per_day_user'],
                            $gui->tproject_name);
...
function initializeGuiForResult(&$dbHandler,$argsObj,&$guiObj)
{
  ...
  $options['setting_testplan']  = " ".$argsObj->setting_testplan;
  $options['setting_build']     = " ".$argsObj->setting_build;

  $mgr = new testproject($dbHandler);
  $guiObj->searchDone = 1;
  $guiObj->resultSet = $mgr->getCountPerUserExectionByPlanBuild($argsObj->tproject_id,$argsObj->user_id,
                                                    $options);
  var_dump($$guiObj->resultSet);
  //var_dump($guiObj->resultSet);die;
...
function initializeGuiForInput(&$dbHandler,$argsObj,&$guiObj)
{
  ...
  $guiObj->test_plan = new stdClass();
  //$guiObj->test_plan->items = get_select_tcproject_plan_id($argsObj->tproject_id, $dbHandler);
...
function init_args(&$dbHandler)
{
  ...
  $iParams = array("apikey"               => array(tlInputParameter::STRING_N,32,32),
                   ...
                   "setting_testplan"     => array(tlInputParameter::INT_N),
                   "setting_build"        => array(tlInputParameter::INT_N));
