  ...
  for($idx=0; $doIt && $idx < $tc_qty;$idx++)
  {
    ...
    if( $status_ok ) 
    {
      ...
      else 
      {
        ...
        {
          $sql = " /* $debugMsg */ " .
                 " INSERT INTO ".$db->get_table('executions')." (build_id,tester_id,status,testplan_id," .
          ...
          */
          ...
          { 
            foreach($tcase_exec['bug_id'] as $bug_id)
            {
              $bug_id = trim($bug_id);
              $sql = " /* $debugMsg */ " .            
                     " SELECT execution_id AS check_qty FROM  ".$db->get_table('execution_bugs')." " .
                     " WHERE bug_id = '{$bug_id}' AND execution_id={$execution_id} ";
              $rs = $db->get_recordset($sql); 
              if( is_null($rs) )
              {
                $sql = " /* $debugMsg */ " .
                       " INSERT INTO ".$db->get_table('execution_bugs')." (bug_id,execution_id)" .
...
function check_exec_values(&$db,&$tcase_mgr,&$user_mgr,$tcaseCfg,&$execValues,&$columnDef)
{
  ...
  {
    $sql = "SELECT id,login FROM ".$db->get_table('users')." WHERE login ='" . 
