...
{    
    ...
    if(!is_null($tcasesID) && count($tcasesID) > 0)
    {
		...
		$sql = " /* $debugMsg */ " .
	           ...
	           " FROM ".$db->get_table('nodes_hierarchy')." NHA " .
	           " JOIN ".$db->get_table('node_types')." NT ON NHA.node_type_id=NT.id " .
	           " LEFT OUTER JOIN ".$db->get_table('req_coverage')." REQC on REQC.testcase_id=NHA.id " .
...
if($gui->has_tc = (!is_null($uncovered) && count($uncovered) > 0) )
{
    ...
    $sql = "/* $debugMsg */ " .
         ...
         " ".$db->get_table('tcversions')." TCV, ".$db->get_table('node_types')." NT " .
