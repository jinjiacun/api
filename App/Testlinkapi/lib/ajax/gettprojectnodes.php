...
//          " FROM {$tables['nodes_hierarchy']} NHA, {$tables['node_types']} NT " .
         " FROM ".$dbHandler->get_table('nodes_hierarchy')." NHA, ".$dbHandler->get_table('node_types')." NT " .
...
//             " JOIN {$tables['nodes_hierarchy']} NHB ON NHA.parent_id = NHB.id " . 
            " FROM " . $dbHandler->get_table('tcversions')." TCV " .
            " JOIN " . $dbHandler->get_table('nodes_hierarchy')." NHA  ON NHA.id = TCV.id  " .
