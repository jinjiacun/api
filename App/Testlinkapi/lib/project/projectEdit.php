...
function create(&$argsObj,&$tprojectMgr)
{
  ...
  // add by zhouzhaoxin 20160706 for set requirement default open when create
  $argsObj->optReq = 1;
...
function initializeGui(&$dbHandler,$argsObj)
{
  ...
  {
      $sql = "select prj.project_name as name from " . $dbHandler->get_table('inf_project_info')." prj, " .
             $dbHandler->get_table('system_mapping') . " map, " . $dbHandler->get_table('testprojects')." pj where prj.project_no = map.emp_system_id and " .
             "pj.id = map.testproject_id and pj.id = " . $argsObj->tprojectID . ";";
    
      $rs = $dbHandler->get_recordset($sql);
      $argsObj->link = $rs[0]['name'];
  }
  else 
  {
      $argsObj->link = "";
