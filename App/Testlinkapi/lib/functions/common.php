...
function tlAutoload($class_name) 
{
  ...
  if( strstr($class_name,'Zend_') !== FALSE || strstr($class_name, 'PHPEx') !== FALSE)
  {
    return false;
  }
...
function getEntityByAPIKey(&$dbHandler,$apiKey,$type)
{
  ...
  $sql = "/* $debugMsg */ " .
         " SELECT id FROM " . $dbHandler->get_table($target) . " WHERE api_key = '{$apiKey}'";
 ...
 * author:liuchunping
 * history:2016.6.12
 * modify by zhouzhaoxin 20161109 to set it as a global function and change to use db class not mysql
 * **/
function get_select_tcproject_user_id($testproject_id, $dbHandler)
{
    $stdSet = array();
    $stdSet['0'] = "";
    
    $sql = "SELECT utr.user_id, usr.first, usr.last FROM " . $dbHandler->get_table('user_testproject_roles') . " utr " .
        "join "  . $dbHandler->get_table('users') . " usr on utr.user_id = usr.id " .
        "WHERE testproject_id = " . $testproject_id;
    $result = $dbHandler->fetchRowsIntoMap($sql, 'user_id');
    if (count($result, COUNT_NORMAL) > 0)
    {
        foreach ($result as $user_id => $user_info)
        {
            $stdSet[$user_id] = $user_info['first'] . $user_info['last'];
        }
    }
 ...
 *  get user name by id
 * @param: $user_id =>user
 * @param: index$dbHandler
 * @return:user first + last name
 * @author:  zhouzhaoxin
 * @version: 20161115 add by zhouzhaoxin
 * **/
function get_user_name_by_id($dbHandler, $user_id)
{
    $name = "";
    if ($user_id <= 0)
    {
    ...
    $sql = "SELECT usr.id, usr.first, usr.last FROM " . $dbHandler->get_table('users') . " usr " .
 ...
 * author:  zhouzhaoxin
 * history: 20161109
 * add by zhouzhaoxin to get node abs by node id
 * **/
function GetAbsByNodeId($node_id, $db)
{
    $sql = "SELECT nh.node_depth_abs as abs from ".
        $db->get_table('nodes_hierarchy') . " nh WHERE " .
        "nh.id = ". $node_id;
    $result = $db->fetchFirstRow($sql);
    
    if ($result && array_key_exists('abs', $result))
    {
        return $result['abs'];
    }
}
 ...
 * 
 * author:jinjiacun
 * time:2017-12-29 14:56
 *
 * get modules by project_id
 */
function GetModulesByProjectId($project_id, $db){

  if($project_id <= 0)
    return null;

  $sql_template = "select id,name "
                 ." from ".$db->get_table('nodes_hierarchy')
                 ." where parent_id=%d and node_type_id in(2,3)";
  $sql = sprintf($sql_template, $project_id);
  $rs = $db->fetchColumnsIntoMap($sql, "id","name");
  if($rs)
    return $rs;
 ...
 * author:  zhouzhaoxin
 * history: 20161116
 * add by zhouzhaoxin to log array info
 * **/
/*
function logger_array($array, $depth)
{
    $depth++;
    ...
    if (is_array($array))
    {
        ...
        {
            error_log("key " . $depth . " is :" . $key);
            logger_array($value, $depth);
        }
    }
    else 
    {
        $value_depth = $depth - 1;
        error_log("value " . $value_depth . " is :" . $array);
    }
}*/
...
function get_company_by_logins(&$db, $logins){
    $user_company_map = array();
    if($logins == ''){
      return $user_company_map;
    }

    $template_sql = "select idl.name, iem.ldap_emp_id "
                    ." from ".$db->get_table('inf_dynamic_list')." as idl   "
                    ." inner join ".$db->get_table('inf_emp_info')." as iei "
                    ." on idl.id = iei.belong_company "
                    ." inner join ".$db->get_table('inf_emp_mapping')." as iem "
                    ." on iei.emp_id = iem.emp_id "
                    ." where iem.ldap_emp_id in (%s)"
                    ." and idl.type = 1;";
     $sql = sprintf($template_sql, $logins);
     unset($template_sql);
     $user_company_map = $db->fetchColumnsIntoMap($sql, "ldap_emp_id", "name");
     unset($sql);
