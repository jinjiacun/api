...
// 		$sql = "/* $debugMsg */ SELECT {$fields2get} FROM {$this->tables['inventory']} " .
		$sql = "/* $debugMsg */ SELECT {$fields2get} FROM ".$this->db->get_table('inventory')." " .
	...
	protected function writeToDB(&$db)
	{
		...
		{
			$sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('inventory')." (name," .
		...
		{
			$sql = "/* $debugMsg */UPDATE ".$this->db->get_table('inventory')." " .
	...
	{
		$debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
		$sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('inventory')." WHERE id = " . $this->inventoryId;
		$result = $this->db->exec_query($sql);
...
// 			$sql = "/* $debugMsg */ SELECT id FROM {$this->tables['inventory']} " .
			$sql = "/* $debugMsg */ SELECT id FROM ".$this->db->get_table('inventory')." " .
					 " WHERE name='" . $name.
...
// 		       " SELECT id, name AS tproject_name FROM {$this->tables['nodes_hierarchy']} " .
		       " SELECT id, name AS tproject_name FROM ".$this->db->get_table('nodes_hierarchy')." " .
