...
//              " FROM {$this->tables['transactions']} ";
             " FROM ".$db->get_table('transactions')." ";
    $clauses = null;
  ...
  public function writeToDB(&$db)
  {
    ...
    if (!$this->dbID)
    {
      ...
      $query = "/* $debugMsg */ INSERT INTO ".$db->get_table('transactions')." " .
    ...
    else
    {
      ...
      $query = " /* $debugMsg */ " .
               " UPDATE ".$db->get_table('transactions')." SET end_time = {$endTime} " . 
...
//       $usersFilter = " JOIN {$this->tables['transactions']}  T " .
      $usersFilter = " JOIN ".$this->db->get_table('transactions')."  T " .
                     " ON T.id = E.transaction_id AND T.user_id IN ({$users}) ";
    }
//     $query = "SELECT E.id FROM {$this->tables['events']} E {$usersFilter}";
    $query = "SELECT E.id FROM ".$this->db->get_table('events')." E {$usersFilter}";
    if ($clauses)
    {
      $query .= " WHERE " . implode(" AND ",$clauses);
    }
  ...
  function deleteEventsFor($logLevels = null,$startTime = null)
  {
    ...
    $query = "DELETE FROM ".$this->db->get_table('events')." ";
    if ($clauses)
    {
      $query .= " WHERE " . implode(" AND ",$clauses);
    }
...
//     $query = " DELETE FROM {$this->tables['transactions']} WHERE id IN ( {$subsql} )";
    $subsql = " SELECT id FROM ( SELECT id FROM ".$this->db->get_table('transactions')." t " .
    " WHERE (SELECT COUNT(0) FROM ".$this->db->get_table('events')." e WHERE e.transaction_id = t.id) = 0) XX";
    $query = " DELETE FROM ".$this->db->get_table('transactions')." WHERE id IN ( {$subsql} )";
    $this->db->exec_query($query);  
  }
}
...
//              " FROM {$this->tables['events']} ";
  ...
  public function writeToDB(&$db)
  {
    ...
    if (!$this->dbID)
    {
      ...
      $query = "/* $debugMsg */ " .
               "INSERT INTO ".$db->get_table('events')." (transaction_id,log_level,description,source," .
