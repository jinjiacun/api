...
class requirement_mgr extends tlObjectWithAttachments
{
  ...
  // 20160713 zhouzhaoxin remove csv type and DocBook type for no use, add DOC Type
...
function get_by_id($id,$version_id=self::ALL_VERSIONS,$version_number=1,$options=null,$filters=null)
{
  ...
  $sql = " /* $debugMsg */ SELECT REQ.id,REQ.srs_id,REQ.req_doc_id," . 
         ...
         " REQV.modification_ts,REQV.testplans_id,REQV.business_rules,REQV.add_remaks,REQV.add_definitions,REQV.revision, -1 AS revision_id, " .
         " NH_REQ.name AS title, REQ_SPEC.testproject_id, " .
         " NH_RSPEC.name AS req_spec_title, REQ_SPEC.doc_id AS req_spec_doc_id, NH_REQ.node_order,tpname_REQV.name AS tpname " .
...
//          " JOIN {$this->tables['nodes_hierarchy']} tpname_REQV ON tpname_REQV.id = REQV.testplans_id ".
         " FROM ".$this->db->get_table($this->object_table)." REQ " .
         " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQ ON NH_REQ.id = REQ.id " .
         " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQV ON NH_REQV.parent_id = NH_REQ.id ".
         " JOIN ".$this->db->get_table('req_versions')." REQV ON REQV.id = NH_REQV.id " .
         " JOIN ".$this->db->get_table('req_specs')." REQ_SPEC ON REQ_SPEC.id = REQ.srs_id " .
         " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_RSPEC ON NH_RSPEC.id = REQ_SPEC.id " .
         " JOIN ".$this->db->get_table('nodes_hierarchy')." tpname_REQV ON tpname_REQV.id = REQV.testplans_id ".
  ...
  /*
    function: create
     update by chy 160706 add elements
...
function create($srs_id,$reqdoc_id,$title, $scope, $user_id,
                ...
                $expected_coverage=1,$node_order=0,$business_rules,$add_remaks,
                $add_definitions,$sel_plan,$tproject_id=null, $options=null)
{
  ...
  if( $op['status_ok'] )
  {
    ...
    if($result["status_ok"])
    {
      ...
      $op = $this->create_version($result['id'],1,$scope,$user_id,
                                  $status,$type,intval($expected_coverage),$business_rules,
                                  $add_remaks,$add_definitions,$sel_plan);
      $result['msg'] = $op['status_ok'] ? $result['msg'] : $op['msg'];
      ...
      {
        $sql =   "/* $debugMsg */ " .
            "UPDATE ".$this->db->get_table('req_versions')." " .
...
function update($id,$version_id,$reqdoc_id,$title, $scope, $user_id, $status, $type,
                ...
                $business_rules,$add_remaks,$add_definitions,$sel_plan)
{
  $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
  ...
  if($chk['status_ok'] || $skip_controls)
  {
    ...
    {  
     ...
     $sql = array();
      ...
      $q = "/* $debugMsg */ UPDATE ".$this->db->get_table('nodes_hierarchy')." " .
      ...
      $sql[] = "/* $debugMsg */ UPDATE ".$this->db->get_table('requirements')." " .
               " SET req_doc_id='" . $this->db->prepare_string($reqdoc_id) . "'" .
               " WHERE id={$id}";
      
      $sql_temp = "/* $debugMsg */ UPDATE ".$this->db->get_table('req_versions')." " .
                  " SET scope='" . $this->db->prepare_string($scope) . "', " .
                  " status='" . $this->db->prepare_string($status) . "', " .
                  " expected_coverage={$expected_coverage}, " . 
                  " type='" . $this->db->prepare_string($type) . "' ,".
                  "testplans_id={$sel_plan},".
                  "business_rules='" . $this->db->prepare_string($business_rules) . "', " .
                  "add_remaks='" . $this->db->prepare_string($add_remaks) . "', " .
                  "add_definitions='" . $this->db->prepare_string($add_definitions) . "' " ;
...
//              "SELECT NH.id FROM {$this->tables['nodes_hierarchy']} NH " .
             "SELECT NH.id FROM ".$this->db->get_table('nodes_hierarchy')." NH " .
             "WHERE NH.parent_id ";
      if( is_array($id) )
      {
        $sql .=  " IN (" .implode(',',$id) . ") ";
      ...
      $sql = "DELETE FROM ".$this->db->get_table('req_coverage')." " . $where['coverage'];
...
//       $sql = "SELECT COUNT(0) AS VQTY, parent_id FROM {$this->tables['nodes_hierarchy']} " . 
      $sql = "SELECT COUNT(0) AS VQTY, parent_id FROM ".$this->db->get_table('nodes_hierarchy')." " .
...
//       $sql = "/* $debugMsg */ SELECT id from {$this->tables['nodes_hierarchy']} " .
      $sql = "/* $debugMsg */ SELECT id from ".$this->db->get_table('nodes_hierarchy')." " .
      ...
      if( !is_null($revisionSet) )
      {
        ...
        $sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('req_revisions')." " . 
               "WHERE parent_id IN ( {$implosion} ) ";
        $result = $this->db->exec_query($sql);
              
        $sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " .
               "WHERE parent_id IN ( {$implosion} ) " .
               "AND node_type_id=" . 
              $this->node_types_descr_id['requirement_revision'];
        $result = $this->db->exec_query($sql);
      ...
      $sql = "DELETE FROM ".$this->db->get_table('req_versions')." " . $where['children'];
      $result = $this->db->exec_query($sql);
          
      $sql = "DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " . 
             $where['children'] .
    ...
    if( $kaboom )
    {
      $sql = "DELETE FROM " . $this->db->get_table($this->object_table) . $where['this'];
      $result = $this->db->exec_query($sql);

      $sql = "DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " . $where['this'] .
...
//                     " JOIN {$this->tables['req_coverage']} RC ON RC.testcase_id = NH_TC.id ";
     $common['join'] = " FROM ".$this->db->get_table('nodes_hierarchy')." NH_TC " .
                      " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.parent_id=NH_TC.id " .
                      " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.id=NH_TCV.id " .
                      " JOIN ".$this->db->get_table('req_coverage')." RC ON RC.testcase_id = NH_TC.id ";
...
//            " LEFT OUTER JOIN {$this->tables['users']} U ON U.id = RC.author_id " .
           " LEFT OUTER JOIN ".$this->db->get_table('users')." U ON U.id = RC.author_id " .
           $common['where'];
  }
  else
  {
...
//           $sql = "/* $debugMsg */ SELECT id,name FROM {$this->tables['nodes_hierarchy']} NH " .
          $sql = "/* $debugMsg */ SELECT id,name FROM ".$this->db->get_table('nodes_hierarchy')." NH " .
...
//     $sql=" SELECT id FROM {$this->tables['nodes_hierarchy']} NH " .
    $sql=" SELECT id FROM ".$this->db->get_table('nodes_hierarchy')." NH " .
  ...
  foreach ($reqSet as $reqID)
  {
    ...
    for ($idx = 0; $idx < $count; $idx++) 
    {
      ...
      $tcase = $tcase_mgr->create($tsuite_id,$tcase_name,$content,
                            ...
                            $testcase_importance_default,$tc_id,$bpm_id,$complexity,$reviewed_status,$reviewer_id,$createOptions);
...
//       $sql = " /* $debugMsg */ SELECT req_id,testcase_id FROM {$this->tables['req_coverage']} " .
      $sql = " /* $debugMsg */ SELECT req_id,testcase_id FROM ".$this->db->get_table('req_coverage')." " .
      ...
      for($idx=0; $idx < $loop2do; $idx++)
      {
        ...
        {
          $sql = "INSERT INTO ".$this->db->get_table('req_coverage')." (req_id,testcase_id,author_id,creation_ts) " .
  ...
  */
  ...
  {
    $output = 0;
    $sql = " DELETE FROM ".$this->db->get_table('req_coverage')." " .
           " WHERE req_id={$req_id} " .
...
//     $sql = " /* $debugMsg */ SELECT req_id,testcase_id FROM {$this->tables['req_coverage']} " .
    ...
    $now = $this->db->db_now();
    $insert_sql = "INSERT INTO ".$this->db->get_table('req_coverage')." (req_id,testcase_id,author_id,creation_ts) ";
...
//            "      {$this->tables['req_coverage']} req_coverage " .
           ...
           "      ".$this->db->get_table('req_coverage')." req_coverage " .
...
//            "      {$this->tables['req_specs']} RSPEC " ;
            " FROM ".$this->db->get_table($this->object_table)." REQ, " .
            "      ".$this->db->get_table('req_coverage')." REQ_COVERAGE," .
            "      ".$this->db->get_table('nodes_hierarchy')." NHA," .
            "      ".$this->db->get_table('nodes_hierarchy')." NHB," .
            "      ".$this->db->get_table('req_specs')." RSPEC " ;
...
{            
  ...
  $elemTpl = "\t" .   "<requirement>" .
             ...
             "\n\t\t" . "<description><![CDATA[\n||DESCRIPTION||\n]]></description>".
             "\n\t\t" . "<business_rules><![CDATA[\n||BUSINESSRULES||\n]]></business_rules>".
             "\n\t\t" . "<add_remaks><![CDATA[\n||ADDREMARKS||\n]]></add_remaks>".
             "\n\t\t" . "<add_definitions><![CDATA[\n||ADDDEFINITIONS||\n]]></add_definitions>".
             "\n\t\t" . "<testplans_id><![CDATA[\n||TESTPLANSID||\n]]></testplans_id>".
  ...
  $info = array("||DOCID||" => "req_doc_id","||TITLE||" => "title",
                ...
                "||BUSINESSRULES||"=>"business_rules","||ADDREMARKS||"=>"add_remaks",
                "||ADDDEFINITIONS||"=>"add_definitions","||TESTPLANSID||"=>"testplans_id",
...
function xmlToMapRequirement($xml_item)
{
  ...
  //modify by liuchunping
  $dummy['testplans_id'] = trim((string)$xml_item->testplans_id);
  $dummy['business_rules'] = trim((string)$xml_item->business_rules);
  $dummy['add_remaks'] = trim((string)$xml_item->add_remaks);
  $dummy['add_definitions'] =trim((string)$xml_item->add_definitions);
  ...
  if(is_null($check_in_reqspec))
  {
    ...
    if(is_null($check_in_tproject))
    {
      $newReq = $this->create($parent_id,$req['docid'],$req['title'],$req['description'],
                         ...
                         $req['node_order'],$req['business_rules'],$req['add_remaks'],
                         $req['add_definitions'],$req['testplans_id'],$tproject_id,array('quickAndDirty' => true));
      ...
      switch($my['options']['actionOnHit'])
      {
        case 'update_last_version':
          $result = $this->update($reqID,$last_version['id'],$req['docid'],$req['title'],$req['description'],
                                  $author_id,$req['status'],$req['type'],$req['expected_coverage'],
                                  $req['node_order'],$tproject_id,$skip_controls=0,$create_revision=false,
                                  $log_msg=null,$req['business_rules'],$req['add_remaks'],$req['add_definitions'],$req['testplans_id']);
...
//             " WHERE REQ.req_doc_id {$check_criteria} ";
       $sql .= " FROM ".$this->db->get_table($this->object_table)." REQ " .
                " /* Get Req info from NH */ " .
                " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQ ON NH_REQ.id = REQ.id " .
                " JOIN ".$this->db->get_table('req_specs')." REQ_SPEC ON REQ_SPEC.id = REQ.srs_id " .
                " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_RSPEC ON NH_RSPEC.id = REQ_SPEC.id " .
      ...
      if ($new_item['status_ok'])
      {
        ...
        foreach($item_versions as &$req_version)
        {
          $op = $this->create_version($new_item['id'],$req_version['version'],
                                      ...
                                      $req_version['expected_coverage'],$req_version['business_rules'],$req_version['add_remaks'],$req_version['add_definitions'],$req_version['testplans_id']);
  ...
  */
  ...
  function create_req_only($srs_id,$reqdoc_id,$title,$user_id,$node_order=0)
  {
    ...
    $req_id = $this->tree_mgr->new_node($srs_id,$this->node_types_descr_id['requirement'],
                                        $title,$node_order);
    $sql = "/* $debugMsg */ INSERT INTO " . $this->db->get_table($this->object_table) .
  ...
  */
  function create_version($id,$version,$scope, $user_id, $status = TL_REQ_STATUS_VALID, 
                          $type = TL_REQ_TYPE_INFO, $expected_coverage=1,$business_rules,
                                  $add_remaks,$add_definitions,$sel_plan)
  {
    static $debugMsg;
    ...
    $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('req_versions')." " .
           " (id,version,scope,status,type,expected_coverage,author_id,creation_ts,testplans_id,business_rules,add_remaks,add_definitions) " . 
           " VALUES({$item_id},{$version},'" . trim($this->db->prepare_string($scope)) . "','" . 
           $this->db->prepare_string($status) . "','" . $this->db->prepare_string($type) . "'," .
           "{$expected_coverage}," . intval($user_id) . "," . $this->db->db_now() .","."{$sel_plan}".",'". 
           trim($this->db->prepare_string($business_rules))."','".trim($this->db->prepare_string($add_remaks)). "','" . 
           trim($this->db->prepare_string($add_definitions)). "')";
...
//            " {$this->tables['nodes_hierarchy']} NH WHERE ".
           " FROM ".$this->db->get_table('req_versions')." REQV," .
           " ".$this->db->get_table('nodes_hierarchy')." NH WHERE ".
           " NH.id = REQV.id ".
           " AND NH.parent_id = {$id} ";
...
//              " {$this->tables['nodes_hierarchy']} NH ".
         $sql = "/* $debugMsg */ SELECT REQV.* FROM ".$this->db->get_table('req_versions')." REQV," .
  ...
  function copy_version($id,$from_version_id,$to_version_id,$as_version_number,$user_id)
  {
    ...
    $now = $this->db->db_now();
    $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('req_versions')." " .
           " (id,version,author_id,creation_ts,scope,status,type,expected_coverage,testplans_id,business_rules,add_remaks,add_definitions) " .
           " SELECT {$to_version_id} AS id, {$as_version_number} AS version, " .
           "        {$user_id} AS author_id, {$now} AS creation_ts," .
           "        scope,status,type,expected_coverage,testplans_id,business_rules,add_remaks,add_definitions " .
//            " FROM {$this->tables['req_versions']} " .
           " FROM ".$this->db->get_table('req_versions')." " .
  ...
  private function updateBoolean($reqVersionID,$field,$value)
  {
    ...
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* $debugMsg */ UPDATE ".$this->db->get_table('req_versions')." " .
...
//            " FROM {$this->tables['req_relations']} " .
  ...
  public function delete_relation($id) {
    ...
    $debugMsg = '/* Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__ . ' */';
    $sql = " $debugMsg DELETE FROM ".$this->db->get_table('req_relations')." WHERE id=$id ";
    $this->db->exec_query($sql);
  }
  ...
  public function delete_all_relations($id) {
    ...
    $sql = " $debugMsg DELETE FROM ".$this->db->get_table('req_relations')." " . 
...
//            " {$this->tables['nodes_hierarchy']} NH WHERE ".
           " FROM ".$this->db->get_table($table)." CHILD," .
           " ".$this->db->get_table('nodes_hierarchy')." NH WHERE ".
           " NH.id = CHILD.id ".
           " AND NH.parent_id = {$id} ";
...
//               " {$this->tables['nodes_hierarchy']} NH ".
         $sql .= " FROM ".$this->db->get_table($table)." CHILD," .
              " ".$this->db->get_table('nodes_hierarchy')." NH ".
  ...
  function create_new_revision($parent_id,$user_id,$tproject_id,$req = null,$log_msg = null)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      ...
      $sql =   "/* $debugMsg */ " .
          " UPDATE ".$this->db->get_table('req_revisions')." " .
...
//         " INSERT INTO {$this->tables['req_revisions']} " .
        " INSERT INTO ".$this->db->get_table('req_revisions')." " .
        " (parent_id,id,revision,scope,status,type,active,is_open, " .
          "  expected_coverage,author_id,creation_ts,modifier_id,modification_ts,log_message,testplans_id,business_rules,add_remaks,add_definitions) " .
        ...
        " REQV.scope,REQV.status,REQV.type,REQV.active,REQV.is_open, " .
          ...
          " REQV.modification_ts,REQV.log_message,REQV.testplans_id,REQV.business_rules,REQV.add_remaks,REQV.add_definitions" .
//           " FROM {$this->tables['req_versions']} REQV " .
          " FROM ".$this->db->get_table('req_versions')." REQV " .
...
//         " JOIN {$this->tables['nodes_hierarchy']} NH_REQV ON NH_REQV.id = REQRV.parent_id " .
        " FROM ".$this->db->get_table('req_revisions')." REQRV " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQV ON NH_REQV.id = REQRV.parent_id " .
...
//           " JOIN {$this->tables['requirements']} REQ ON REQ.id = NH_REQ.id " .
          " FROM ".$this->db->get_table('req_versions')."  REQV " .
          " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQV ON NH_REQV.id = REQV.id " .
          " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQ ON NH_REQ.id = NH_REQV.parent_id " .
          " JOIN ".$this->db->get_table('requirements')." REQ ON REQ.id = NH_REQ.id " .
          " WHERE NH_REQV.parent_id = {$id} ";
...
//               " JOIN {$this->tables['req_revisions']} REQRV " .
              ...
              " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQ ON NH_REQ.id = NH_REQV.parent_id " .
              " JOIN ".$this->db->get_table('requirements')." REQ ON REQ.id = NH_REQ.id " .
              " JOIN ".$this->db->get_table('req_revisions')." REQRV " .
  ...
  function get_version($version_id,$opt=null)
  {
    ...
    $sql = " /* $debugMsg */ SELECT REQ.id,REQ.srs_id,REQ.req_doc_id," . 
           ...
           " REQV.expected_coverage,REQV.creation_ts,REQV.testplans_id,REQV.business_rules,REQV.add_remaks,REQV.add_definitions,REQV.modifier_id," .
           " REQV.modification_ts,REQV.revision,NH_REQ.name AS title, REQ_SPEC.testproject_id, " .
           " NH_RSPEC.name AS req_spec_title, REQ_SPEC.doc_id AS req_spec_doc_id,tpname_REQV.name AS tpname, NH_REQ.node_order " .
//            " FROM {$this->object_table} REQ " .
//            " JOIN {$this->tables['nodes_hierarchy']} NH_REQ ON NH_REQ.id = REQ.id " .
//            " JOIN {$this->tables['nodes_hierarchy']} NH_REQV ON NH_REQV.parent_id = NH_REQ.id ".
//            " JOIN  {$this->tables['req_versions']} REQV ON REQV.id = NH_REQV.id " .  
...
//            " JOIN {$this->tables['nodes_hierarchy']} tpname_REQV ON tpname_REQV.id = REQV.testplans_id ".
  ...
  function get_revision($revision_id,$opt=null)
  {
    ...
    $sql = " /* $debugMsg */ SELECT REQ.id,REQ.srs_id,REQ.req_doc_id," . 
           ...
           " REQRV.expected_coverage,REQRV.creation_ts,REQRV.testplans_id,REQRV.business_rules,REQRV.add_remaks,REQRV.add_definitions,REQRV.modifier_id," .
           " REQRV.modification_ts,REQRV.revision, REQRV.id AS revision_id," .
           " NH_REQ.name AS title, REQ_SPEC.testproject_id, " .
           " NH_RSPEC.name AS req_spec_title, REQ_SPEC.doc_id AS req_spec_doc_id,tpname_REQRV.name AS rtpname, NH_REQ.node_order " .
//            " FROM {$this->tables['req_revisions']} REQRV " .
...
//            " JOIN {$this->tables['req_specs']} REQ_SPEC ON REQ_SPEC.id = REQ.srs_id " .
//            " JOIN {$this->tables['nodes_hierarchy']} NH_RSPEC ON NH_RSPEC.id = REQ_SPEC.id " .
//            " JOIN {$this->tables['nodes_hierarchy']} tpname_REQRV ON tpname_REQRV.id = REQRV.testplans_id ".
            " FROM ".$this->db->get_table('req_revisions')." REQRV " .
            " JOIN ".$this->db->get_table('req_versions')." REQV ON REQV.id = REQRV.parent_id ".
            " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQV ON NH_REQV.id = REQRV.parent_id ".
            " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_REQ ON NH_REQ.id = NH_REQV.parent_id " .
            " JOIN ".$this->db->get_table('requirements')." REQ ON REQ.id = NH_REQ.id " .
            " JOIN ".$this->db->get_table('req_specs')." REQ_SPEC ON REQ_SPEC.id = REQ.srs_id " .
            " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_RSPEC ON NH_RSPEC.id = REQ_SPEC.id " .
            " JOIN ".$this->db->get_table('nodes_hierarchy')." tpname_REQRV ON tpname_REQRV.id = REQRV.testplans_id ".
  ...
  function get_version_revision($version_id,$revision_access,$opt=null)
  {
    ...
    {  
      ...
      $sql .= " SELECT NH_REQV.parent_id AS req_id, REQV.id AS version_id, REQV.version," .
              ...
              "     REQV.modification_ts, REQV.modifier_id, " . 
              self::NO_REVISION . " AS revision_id, " .
              "      REQV.revision, REQV.scope,REQV.testplans_id,REQV.business_rules,REQV.add_remaks,REQV.add_definitions, " .
              "      REQV.status,REQV.type,REQV.expected_coverage,NH_REQ.name, REQ.req_doc_id,tpname_REQV.name AS tpname, " .
...
//               " JOIN {$this->tables['requirements']} REQ ON REQ.id = NH_REQ.id " .
      ...
      $sql .=  " UNION ALL ( " .
            ...
            "     REQRV.modification_ts, REQRV.modifier_id, " . 
            "     REQRV.id AS revision_id, " .
            "     REQRV.revision,REQRV.scope,REQRV.testplans_id,REQRV.business_rules,REQRV.add_remaks,REQRV.add_definitions,REQRV.status,REQRV.type, " .
            "     REQRV.expected_coverage,REQRV.name,REQRV.req_doc_id,tpname_REQRV.name AS rtpname, " .
...
//             " JOIN {$this->tables['nodes_hierarchy']} tpname_REQRV ON tpname_REQRV.id = REQRV.testplans_id ".
    ...
    {  
      // revision_id is present ONLY on req revisions table, then we do not need UNION
       $sql .=  " SELECT NH_REQV.parent_id AS req_id, REQV.id AS version_id, REQV.version, " .
                "     REQRV.creation_ts, REQRV.author_id, " .
                "     REQRV.modification_ts, REQRV.modifier_id, tpname_REQRV.name AS rtpname," . 
                "     REQRV.id AS revision_id, " .
                "     REQRV.revision,REQRV.scope,REQRV.testplans_id,REQRV.business_rules,REQRV.add_remaks,REQRV.add_definitions,REQRV.status,REQRV.type, " .
                "     REQRV.expected_coverage,REQRV.name,REQRV.req_doc_id, " .
                "     COALESCE(REQRV.log_message,'') as log_message, NH_REQ.name AS title " .
...
//          "JOIN {$this->tables['req_specs']} RSPEC ON RSPEC.id = REQ.srs_id ";
        "SELECT DISTINCT REQ.id,REQ.req_doc_id FROM ".$this->db->get_table('requirements')." REQ " .
        "JOIN ".$this->db->get_table('req_specs')." RSPEC ON RSPEC.id = REQ.srs_id ";
    

    if( isset($context['tplan_id']) )
    {
      
      $sql .= "JOIN ".$this->db->get_table('req_coverage')." REQCOV ON REQCOV.req_id = REQ.id " .
          "JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.parent_id = REQCOV.testcase_id " .
          "JOIN ".$this->db->get_table('testplan_tcversions')." TPTCV ON TPTCV.tcversion_id = NH_TCV.id ";
...
//          " JOIN {$this->tables['nodes_hierarchy']} NH_RSPEC ON NH_RSPEC.id = REQ_SPEC.id " .
//          " JOIN {$this->tables['nodes_hierarchy']} NH_REQ ON NH_REQ.id = REQ.id " .
          " FROM ".$this->db->get_table('nodes_hierarchy')." NH_REQV JOIN " .
          "( SELECT XNH_REQV.parent_id,MAX(XNH_REQV.id) AS LATEST_VERSION_ID " .
          "  FROM  ".$this->db->get_table('nodes_hierarchy')." XNH_REQV " .
          "  WHERE XNH_REQV.parent_id {$in_clause} " .
          "  GROUP BY XNH_REQV.parent_id ) ZAZA ON NH_REQV.id = ZAZA.LATEST_VERSION_ID " .
          
          " JOIN ".$this->db->get_table('req_versions')." REQV ON REQV.id = NH_REQV.id " .
          " JOIN ".$this->db->get_table($this->object_table)." REQ ON REQ.id = NH_REQV.parent_id " .
...
//            " FROM {$this->tables['req_coverage']} " .
           " FROM ".$this->db->get_table('req_coverage')." " .
