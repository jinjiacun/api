...
//     $sql .= " FROM {$this->object_table} a ";
    $sql .= " FROM ".$db->get_table($this->object_table)." a ";
    
    if ($getFullDetails)
    {
      $sql .= " LEFT OUTER JOIN ".$db->get_table('role_rights')." b ON a.id = b.role_id " . 
              " LEFT OUTER JOIN ".$db->get_table('rights')."  c ON b.right_id = c.id ";
    }
  ...
  public function writeToDB(&$db)
  {
    ...
    {   
      if ($this->dbID)
      {
        ...
        if ($result >= tl::OK)
        {
          $sql = "UPDATE " . $db->get_table($this->object_table) .
      ...
      {
        $sql = "INSERT INTO " . $db->get_table($this->object_table) . " (description,notes) " .
  ...
  public function deleteFromDB(&$db)
  {
    ...
    if ($result >= tl::OK)
    {
      ...
      $sql = "DELETE FROM " . $db->get_table($this->object_table) . " WHERE id = {$this->dbID}";
  ...
  protected function replaceUserRolesWith(&$db,$newRole)
  {
    ...
    {
      $sql = "UPDATE ".$db->get_table($table)." SET role_id = {$newRole} WHERE role_id = {$this->dbID}";
...
//            " WHERE users.id = user_testproject_roles.user_id";
    $sql = "SELECT DISTINCT id FROM ".$db->get_table('users')." users," .
        " ".$db->get_table('user_testproject_roles')." user_testproject_roles " .
        " WHERE users.id = user_testproject_roles.user_id";
...
//            " {$this->tables['user_testplan_roles']} user_testplan_roles " .
  ...
  */
  ...
  protected function deleteRightsFromDB(&$db)
  {
    ...
    $sql = "DELETE FROM " . $db->get_table($tablename) . " WHERE role_id = {$this->dbID}";
    $result = $db->exec_query($sql);
  ...
  protected function addRightsToDB(&$db)
  {
    ...
    if ($this->rights)
    {
      ...
      {
        $rightID = $right->dbID;
        $sql = "INSERT INTO ".$db->get_table('role_rights')." (role_id,right_id) " .
...
//              "WHERE role_id = {$this->dbID}";
    $sql = "SELECT right_id,description FROM ".$db->get_table('role_rights')." a " .
...
//     $sql = "SELECT id FROM {$tables['roles']} ";
    $sql = "SELECT id FROM ".$db->get_table('roles')." ";
...
//     $sql = "SELECT description FROM {$tables['roles']} ";
    $sql = "SELECT description FROM ".$db->get_table('roles')." ";
