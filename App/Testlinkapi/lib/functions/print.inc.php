...
function renderReqForPrinting(&$db,$node, &$options, $tocPrefix, $reqLevel, $tprojectID) 
{
  ...
  if (!$req_mgr) 
  {
    ...
    $labels = array('requirement' => 'requirement', 'status' => 'status', 
                    'scope' => 'scope','business_rules'=>'requirment_business_rules',
                    'add_remaks'=>'requirment_add_remaks','add_definitions'=>'requirment_add_definitions',
                    'testplans_id'=>'xls_jmexport_testplan','tpname'=>'xls_jmexport_testplan',
  ...
  //adde reqsource by chy0708
  if ($options['req_tpname']){
      $output .= '<tr><td width="' . $firstColWidth . '"><span class="label">' .
          $labels['tpname'] . "</span></td>" .
          "<td>" . $req['tpname'] . "</td></tr>";
  }
  ...
  //update by chy for add new elements
 
  if ($options['req_business_rules'])
  {
      $output .= "<tr><td colspan=\"$tableColspan\"> <br/>" . $req['business_rules'] . "</td></tr>";
  }
  if ($options['req_add_remaks'])
  {
      $output .= "<tr><td colspan=\"$tableColspan\"> <br/>" . $req['add_remaks'] . "</td></tr>";
  }

  if ($options['req_add_definitions'])
  {
      $output .= "<tr><td colspan=\"$tableColspan\"> <br/>" . $req['add_definitions'] . "</td></tr>";
  }
...
//            " JOIN {$tables['builds']} B ON B.id = E.build_id " .
           " FROM ".$db->get_table('executions')." E " .
 ...
 $tcInfo = $tc_mgr->get_by_id($id,$getByID['tcversion_id'],$getByID['filters'],
  ...
  //liu chunping history:2016.6.13  function:to show the new field("tc_id,complexity, reviewer_id,reviewed_status,bpm_id") in the print view
  //tc_id
  $code .= '<tr><td width="' . $cfg['firstColWidth'] . '" valign="top">' .
      '<span class="label">'. $labels['tc_id'].':</span></td>' .
      '<td colspan="' .  ($cfg['tableColspan']-1) . '">' .  $tcInfo['tc_id'];
  $code .= "</td></tr>\n";
  
  //complexity
  $code .= '<tr><td width="' . $cfg['firstColWidth'] . '" valign="top">' .
      '<span class="label">'.$labels['complexity'].':</span></td>' .
      '<td colspan="' .  ($cfg['tableColspan']-1) . '">';
  switch ($tcInfo['complexity'])
  {
      case 1:
          $code .= $labels['very_simple'];
          break;
  
      case 2:
          $code .= $labels['little_simple'];
          break;
      case 3:
          $code .= $labels['general'];
          break;
  ...
  //bpm_id
  $code .= '<tr><td width="' . $cfg['firstColWidth'] . '" valign="top">' .
      '<span class="label">'. $labels['bpm_id'].':</span></td>' .
      '<td colspan="' .  ($cfg['tableColspan']-1) . '">' .  $tcInfo['bpm_id'];
  $code .= "</td></tr>\n";
  
  //reviewer_id
  $code .= '<tr><td width="' . $cfg['firstColWidth'] . '" valign="top">' .
      '<span class="label">'. $labels['reviewer_id'].':</span></td>' .
      '<td colspan="' .  ($cfg['tableColspan']-1) . '">' .  gendocGetUserName($db, $tcInfo['reviewer_id']);
  $code .= "</td></tr>\n";
  
  //reviewed_status
  $code .= '<tr><td width="' . $cfg['firstColWidth'] . '" valign="top">' .
      '<span class="label">'.$labels['reviewed_status'].':</span></td>' .
      '<td colspan="' .  ($cfg['tableColspan']-1) . '">';
  switch ($tcInfo['reviewed_status'])
  {
      case 1:
          $code .= $labels['wait_review'];
          break;
  
      case 2:
          $code .= $labels['reviewed_passed'];
          break;
      case 3:
      default:
          $code .= $labels['reviewed_not_pass'];
          break;
  }
  $code .= "</td></tr>\n";
  ...
  switch ($tcInfo['execution_type'])
  {
    ...
    case TESTCASE_EXECUTION_TYPE_MANUAL:
      ...
      break;
    default:
      $code .= $labels['execution_type_manual_automated'];          
    break;
  }
  $code .= "</td></tr>\n";
...
function initRenderTestCaseCfg(&$tcaseMgr,$options)
{
  ...
  if( (isset($options['step_exec_status']) &&  $options['step_exec_status']) )
  {
    ...
    $labelsKeys=array('last_exec_result', 'report_exec_result','execution_details','execution_mode',
                      ...
                      'execution_type_manual','execution_type_auto','execution_type_manual_automated',
                      'importance','relations','tc_id','complexity','very_simple','little_simple',
                      'general','complex','very_complex','bpm_id','reviewer_id','reviewed_status',
                      'wait_review','reviewed_passed','reviewed_not_pass',
...
//          " JOIN {$st->tables['builds']} B  ON B.id = E.build_id " .
         " FROM ".$dbHandler->get_table($st->tables['executions'])." E " .
         " JOIN ".$dbHandler->get_table($st->tables['builds'])." B  ON B.id = E.build_id " .
