...
class tlReqMgrSystem extends tlObject
{
  ...
  function create($system)
  {
    ...
    {
      $sql = "/* debugMsg */ INSERT  INTO ".$this->db->get_table('reqmgrsystems')." " .
             " (name,cfg,type) " .
  ...
  function update($system)
  {
    ...
    if( $ret['status_ok'] )    
    {
      $sql =  "UPDATE ".$this->db->get_table('reqmgrsystems')."  " .
  ...
  function delete($id)
  {
    ...
    if( is_null($links) )
    {
      $sql =  " /* $debugMsg */ DELETE FROM ".$this->db->get_table('reqmgrsystems')."  " .
  ...
  function getByAttr($attr, $options=null)
  {
    ...
    $sql .= " FROM ".$this->db->get_table('reqmgrsystems')." " . $where;
  ...
  function link($id,$tprojectID)
  {
    ...
    if(is_null($id))
    {
      return;
        ...
        {
      $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('testproject_reqmgrsystem')." " .
           " (testproject_id,reqmgrsystem_id) " .
           " VALUES(" . intval($tprojectID) . "," . intval($id) . ")";
    }
    else
    {
      $sql = "/* $debugMsg */ UPDATE ".$this->db->get_table('testproject_reqmgrsystem')." " .
  ...
  function unlink($id,$tprojectID)
  {
    ...
    $sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('testproject_reqmgrsystem')." " .
              " WHERE testproject_id = " . intval($tprojectID) . 
  ...
  function getLinks($id, $opt=null)
  {
    ...
    $sql = "/* $debugMsg */ " .
         ...
         " FROM ".$this->db->get_table('testproject_reqmgrsystem')." TPMGR" .
         " LEFT OUTER JOIN ".$this->db->get_table('nodes_hierarchy')." NHTPR " .
  ...
  function getLinkSet()
  {
    ...
    $sql = "/* $debugMsg */ " .
         ...
         " FROM ".$this->db->get_table('testproject_reqmgrsystem')." TPIT" .
         " LEFT OUTER JOIN ".$this->db->get_table('nodes_hierarchy')." NHTPR " .
  ...
  function getAll($options=null)
  {
    ...
    $sql .= " FROM ".$this->db->get_table('reqmgrsystems')." {$orderByClause} ";
    ...
    if( !is_null($rs) )
    {
      ...
      if( $my['options']['output'] == 'add_link_count' )
      {
        ...
        $sql .= " FROM ".$this->db->get_table('reqmgrsystems')." ITD " .
                " JOIN ".$this->db->get_table('testproject_reqmgrsystem')." " .
  ...
  function getLinkedTo($tprojectID)
  {
    ...
    $sql = "/* $debugMsg */ " .
         ...
         " FROM ".$this->db->get_table('testproject_reqmgrsystem')." TPIT" .
         " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTPR " .
         " ON NHTPR.id = TPIT.testproject_id " . 
         " JOIN ".$this->db->get_table('reqmgrsystems')." ITRK " .
