...
class tlKeyword extends tlDBObject implements iSerialization,iSerializationToXML,iSerializationToCSV,iDBBulkReadSerialization
{
  ...
  public function readFromDB(&$db,$options = self::TLOBJ_O_SEARCH_BY_ID)
  {
    ...
    $query = $this->getReadFromDBQuery($this->dbID, $db, $options);
   ...
   */
  public function getReadFromDBQuery($ids, $db, $options = self::TLOBJ_O_SEARCH_BY_ID)
  {
//     $query = " SELECT id,keyword,notes,testproject_id FROM {$this->tables['keywords']} ";
    $query = " SELECT id,keyword,notes,testproject_id FROM ".$db->get_table('keywords')." ";
  ...
  public function writeToDB(&$db)
  {
    ...
    if ($result >= tl::OK)
    {
      ...
      if ($this->dbID)
      {
        $query = "UPDATE ".$db->get_table('keywords')." " .
      ...
      {
        $query = " INSERT INTO ".$db->get_table('keywords')." (keyword,testproject_id,notes) " .
  ...
  {
    $sql = "DELETE FROM ".$db->get_table('testcase_keywords')." WHERE keyword_id = " . $this->dbID;
    $result = $db->exec_query($sql);
    if ($result)
    {
      $sql = "DELETE FROM ".$db->get_table('object_keywords')."  WHERE keyword_id = " . $this->dbID;
      $result = $db->exec_query($sql);
    }
    if ($result)
    {
      $sql = "DELETE FROM ".$db->get_table('keywords')." WHERE id = " . $this->dbID;
      $result = $db->exec_query($sql);
    }
...
//     $query = " SELECT id FROM {$tables['keywords']} " .
    $query = " SELECT id FROM ".$db->get_table('keywords')." " .
