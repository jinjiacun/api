...
// 			" AND RSPEC.id = {$id} ";
    $sql .= " FROM ".$this->db->get_table($this->object_table)." RSPEC " .
            " JOIN ".$this->db->get_table('req_specs_revisions')." RSPEC_REV " .
            " ON RSPEC_REV.parent_id = RSPEC.id " .
            " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_RSPEC " .
            " ON RSPEC.id = NH_RSPEC.id " .
            " WHERE RSPEC.id = NH_RSPEC.id " .
            " AND RSPEC_REV.id = {$childID} " .
            " AND RSPEC.id = {$id} ";
...
//   $sql = "/* $debugMsg */ SELECT count(0) AS cnt FROM {$this->tables['requirements']} WHERE srs_id={$id}";
  $sql = "/* $debugMsg */ SELECT count(0) AS cnt FROM ".$this->db->get_table('requirements')." WHERE srs_id={$id}";
  $output['total'] = $this->db->fetchFirstRowSingleColumn($sql,'cnt');

//   $sql = "/* $debugMsg */ SELECT total_req FROM {$this->object_table} WHERE id={$id}";
  $sql = "/* $debugMsg */ SELECT total_req FROM ".$this->db->get_table($this->object_table)." WHERE id={$id}";
...
//          " WHERE REQ.srs_id={$id} " ;
  $sql = "/* $debugMsg */ SELECT DISTINCT REQ.id " .
        " FROM ".$this->db->get_table('requirements')." REQ " .
        " JOIN ".$this->db->get_table('req_coverage')." REQ_COV ON REQ.id=REQ_COV.req_id" .
        " WHERE REQ.srs_id={$id} " ;
...
// 	       " FROM {$this->object_table} RSPEC, {$this->tables['nodes_hierarchy']} NH " .
	       " FROM ".$this->db->get_table($this->object_table)." RSPEC, ".$this->db->get_table('nodes_hierarchy')." NH " .
  ...
  function update($item,$options=null)
  {
   	...
   	if ($chk['status_ok'] || $my['options']['skip_controls'])
   	{
	    ...
	    $db_now = $this->db->db_now();
 	    $sql = " UPDATE " . $this->db->get_table($this->object_table) .
...
{   
  ...
  // delete revisions
  $sqlx = array();
  $sqlx[] = "DELETE FROM ".$this->db->get_table('req_specs_revisions')." " .
            "WHERE parent_id = {$id}";

  $sqlx[] = "DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " . 
            "WHERE parent_id = {$id} " .
            "AND node_type_id=" . 
  ...
  $sqlx = array();
  $sqlx[] = "DELETE FROM " . $this->db->get_table($this->object_table) . " WHERE id = {$id}";
  $sqlx[] = "DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " .
...
// 	$sql = "/* $debugMsg */ SELECT NH_REQ.id FROM {$this->tables['nodes_hierarchy']} NH_REQ ";
	$sql = "/* $debugMsg */ SELECT NH_REQ.id FROM ".$this->db->get_table('nodes_hierarchy')." NH_REQ ";
  $addFields = '';
	switch($range)
	{
		case 'all';
...
// 			$sql .= " JOIN {$this->tables['req_coverage']} REQ_COV ON REQ_COV.req_id=NH_REQ.id ";
			$sql .= " JOIN ".$this->db->get_table('req_coverage')." REQ_COV ON REQ_COV.req_id=NH_REQ.id ";
...
// 		       " FROM {$this->tables['nodes_hierarchy']} NH_REQV " .
		       " FROM ".$this->db->get_table('nodes_hierarchy')." NH_REQV " .
...
// 				" JOIN {$this->tables['req_specs_revisions']} RSPEC_REV " .
				" FROM ".$this->db->get_table('req_specs')." RSPEC " .
				" JOIN ".$this->db->get_table('req_specs_revisions')." RSPEC_REV " .
				" ON RSPEC_REV.parent_id = RSPEC.id " .
// 				" JOIN {$this->tables['nodes_hierarchy']} NH_RSPEC " .
				" JOIN ".$this->db->get_table('nodes_hierarchy')." NH_RSPEC " .
...
// 				" JOIN {$this->tables['nodes_hierarchy']} NH_RSPEC " .
// 				" ON NH_RSPEC.id = RSPEC.id ";
		$sql .=	" FROM ".$this->db->get_table('req_specs')." RSPEC " .
		      " JOIN ".$this->db->get_table('req_specs_revisions')." RSPEC_REV " .
		      " ON RSPEC_REV.parent_id = RSPEC.id " .
		      " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_RSPEC " .
		      " ON NH_RSPEC.id = RSPEC.id ";
...
// 	  	$sql = "/* $debugMsg */ SELECT * from {$this->tables['nodes_hierarchy']} " .
	  	$sql = "/* $debugMsg */ SELECT * from ".$this->db->get_table('nodes_hierarchy')." " .
	...
	function create_revision($rspecID,$item)
	{
		...
		$sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('req_specs_revisions')." " .
		       " ($fields2insert) " . 
...
// 				" FROM {$this->tables['req_specs_revisions']} RSPEC_REV" .
				" FROM ".$this->db->get_table('req_specs_revisions')." RSPEC_REV" .
...
// 				" WHERE RSPEC_REV.id={$id} ";
		$sql = 	'/* $debugMsg */' .
		    " SELECT RSPEC_REV.*, RSPEC.testproject_id " .
		    " FROM ".$this->db->get_table('req_specs_revisions')." RSPEC_REV " .
		    " JOIN ".$this->db->get_table('req_specs')." RSPEC " .
		    " ON RSPEC.id = RSPEC_REV.parent_id " .
		    " WHERE RSPEC_REV.id={$id} ";
	...
	function update_revision($item)
	{
		...
		$sql = '/* $debugMsg */' .
				   " UPDATE ".$this->db->get_table('req_specs_revisions')." " .
...
// 		       " SELECT RSPEC.id FROM {$this->object_table} RSPEC WHERE testproject_id={$tproject_id}";
		       " SELECT RSPEC.id FROM ".$this->db->get_table($this->object_table)." RSPEC WHERE testproject_id={$tproject_id}";
		return $this->db->get_recordset($sql);
	}
...
//            " LEFT OUTER JOIN {$this->tables['users']} U ON U.id = REQ_COV.author_id ";
    $sql = "/* $debugMsg */ SELECT NH_REQ.id,U.login, REQ_COV.creation_ts " .
        " FROM ".$this->db->get_table('nodes_hierarchy')." NH_REQ " .
        " JOIN ".$this->db->get_table('req_coverage')." REQ_COV ON REQ_COV.req_id=NH_REQ.id " .
        " LEFT OUTER JOIN ".$this->db->get_table('users')." U ON U.id = REQ_COV.author_id ";
    $sql .= " WHERE NH_REQ.parent_id={$id} " .
