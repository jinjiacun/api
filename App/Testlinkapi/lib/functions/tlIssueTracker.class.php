...
class tlIssueTracker extends tlObject
{
  ...
  function create($it)
  {
    ...
    {
      $sql =  "/* debugMsg */ INSERT  INTO ".$this->db->get_table('issuetrackers')." " .
              " (name,cfg,type) " .
  ...
  function update($it)
  {
    ...
    if( $ret['status_ok'] )   
    {
      $sql =  "UPDATE ".$this->db->get_table('issuetrackers')."  " .
  ...
  function delete($id)
  {
    ...
    if( is_null($links) )
    {
      $sql =  " /* $debugMsg */ DELETE FROM ".$this->db->get_table('issuetrackers')."  " .
...
//     $sql .= " FROM {$this->tables['issuetrackers']} " . $where;
    $sql .= " FROM ".$this->db->get_table('issuetrackers')." " . $where;
  ...
  function link($id,$tprojectID)
  {
    ...
    if(is_null($id))
    {
      return;
        ...
        {
      $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('testproject_issuetracker')." " .
           " (testproject_id,issuetracker_id) " .
           " VALUES(" . intval($tprojectID) . "," . intval($id) . ")";
    }
    else
    {
      $sql = "/* $debugMsg */ UPDATE ".$this->db->get_table('testproject_issuetracker')." " .
  ...
  function unlink($id,$tprojectID)
  {
    ...
    $sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('testproject_issuetracker')." " .
             " WHERE testproject_id = " . intval($tprojectID) . 
...
//          " LEFT OUTER JOIN {$this->tables['nodes_hierarchy']} NHTPR " .
         " FROM ".$this->db->get_table('testproject_issuetracker')." TPIT" .
...
//     $sql .= " FROM {$this->tables['issuetrackers']} {$orderByClause} ";
    $sql .= " FROM ".$this->db->get_table('issuetrackers')." {$orderByClause} ";
...
//                 " JOIN {$this->tables['testproject_issuetracker']} " .
        $sql .= " FROM ".$this->db->get_table('issuetrackers')." ITD " .
                " JOIN ".$this->db->get_table('testproject_issuetracker')." " .
...
//          " JOIN {$this->tables['issuetrackers']} ITRK " .
         " JOIN ".$this->db->get_table('issuetrackers')." ITRK " .
