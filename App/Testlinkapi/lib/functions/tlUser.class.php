...
//            " FROM {$this->object_table}";
            " FROM ".$db->get_table($this->object_table);
    $clauses = null;
...
//            " FROM {$this->tables['user_testproject_roles']} user_testproject_roles " .
           " FROM ".$db->get_table('user_testproject_roles')." user_testproject_roles " .
  ...
  {
    if (!isset($_SESSION['testprojectID']) || $_SESSION['testprojectID'] <= 0 || $testPlanID == null)
    {
        return tl::OK;
    }
...
//            " FROM {$this->tables['user_testplan_roles']} user_testplan_roles " .
           " FROM ".$db->get_table('user_testplan_roles')." user_testplan_roles " .
           " WHERE user_id = " . intval($this->dbID);
    if ($testPlanID)
    {
...
//         $gsql = " /* debugMsg */ SELECT password FROM {$this->object_table} WHERE id = " . $this->dbID;
        $gsql = " /* debugMsg */ SELECT password FROM ".$db->get_table($this->object_table)." WHERE id = " . $this->dbID;
        ...
        $sql = "/* debugMsg */ UPDATE ".$db->get_table('users')." " .
      ...
      {
        $sql = "/* debugMsg */ INSERT INTO ".$db->get_table('users')." " .
  ...
  public function deleteFromDB(&$db)
  {
    ...
    $sqlSet = array();
    $sqlSet[] = "DELETE FROM " . $db->get_table('user_assignments')." WHERE user_id = {$safeUserID}";
    $sqlSet[] = "DELETE FROM " . $db->get_table('users')."  WHERE id = {$safeUserID}";
  ...
  {
    $sql = "DELETE FROM ".$db->get_table('user_testproject_roles')." WHERE user_id = " . intval($this->dbID);
...
//            " {$this->tables['user_testplan_roles']} user_testplan_roles " .
       $sql = "SELECT DISTINCT id FROM ".$db->get_table('users')." users," .
           " ".$db->get_table('user_testplan_roles')." user_testplan_roles " .
...
//          " WHERE b.description='" . $db->prepare_string($rightNick) . "'";
    $sql = "/* $debugMsg */ SELECT DISTINCT u.id,u.login,u.first,u.last FROM ".$db->get_table('users')." u" .
    " JOIN ".$db->get_table('role_rights')." a ON a.role_id=u.role_id" .
    " JOIN ".$db->get_table('rights')." b ON a.right_id = b.id " .
    " WHERE b.description='" . $db->prepare_string($rightNick) . "'";
...
//          " WHERE b.description='" . $db->prepare_string($rightNick) . "'";
    $sql = "/* $debugMsg */ SELECT DISTINCT u.id,u.login,u.first,u.last FROM ".$db->get_table('users')." u" .
    " JOIN ".$db->get_table('user_testproject_roles')." p ON p.user_id=u.id" .
    " AND p.testproject_id=" . intval($testprojectID) .
    " JOIN ".$db->get_table('role_rights')." a ON a.role_id=p.role_id" .
    " JOIN ".$db->get_table('rights')." b ON a.right_id = b.id " .
    " WHERE b.description='" . $db->prepare_string($rightNick) . "'";
...
//     $sql = " SELECT id,login,first,last FROM {$this->tables['users']}";
    $sql = " SELECT id,login,first,last FROM ".$db->get_table('users');
    $inClause = '';
    if( !is_null($idSet) )
    {
  ...
  {
    if (!isset($testprojectID) || $testprojectID <= 0) {
        return array(); 
    }
...
//            " LEFT OUTER JOIN {$this->tables['user_testplan_roles']} USER_TPLAN_ROLES" .
           " FROM ".$db->get_table('nodes_hierarchy')." NH" .
           " JOIN ".$db->get_table('testplans')." TPLAN ON NH.id=TPLAN.id  " .
           " LEFT OUTER JOIN ".$db->get_table('user_testplan_roles')." USER_TPLAN_ROLES" .
  ...
  /**
   * get user accessible build info 
   *
   * @param $options filter active build or all
   * @return buildSet include accessible builds info
   * @author zhouzhaoxin
   * @date   20160708
       */
  function getAccessibleBuilds(&$db, $testprojectID, $options=null)
  {
      $my['options'] = array( 'output' => null, 'active' => ACTIVE);
      $my['options'] = array_merge($my['options'], (array)$options);
      
      $sql = "SELECT bd.id as build_id, bd.name as build_name, bd.testplan_id as testplan_id, " .
          " bd.active as active, tl.is_public as is_public, nh.name as testplan_name, coalesce(ur.testplan_id,0) AS has_role, " .
//           "ur.role_id as role,0 AS selected FROM builds bd join nodes_hierarchy nh on bd.testplan_id = nh.id " .
//           "join testplans tl on tl.id = nh.id left outer join user_testplan_roles ur on bd.testplan_id = ur.testplan_id" .
          "ur.role_id as role,0 AS selected FROM ".$db->get_table('builds')." bd join ".$db->get_table('nodes_hierarchy')." nh on bd.testplan_id = nh.id " .
          "join ".$db->get_table('testplans')." tl on tl.id = nh.id left outer join ".$db->get_table('user_testplan_roles')." ur on bd.testplan_id = ur.testplan_id" .
          " and ur.user_id = " . intval($this->dbID) .
          " where nh.parent_id = " . intval($testprojectID) . " and ";
      ...
      if( $projectNoRights || ($analyseGlobalRole && $globalNoRights))
      {
          $sql .= "(ur.role_id is not null AND ur.role_id != " . TL_ROLES_NO_RIGHTS . ")";
      }
      else
      {
          $sql .= "(ur.role_id is null OR ur.role_id != " . TL_ROLES_NO_RIGHTS . ")";
      }
      
      if (!is_null($my['options']['active']))
      {
          $sql .= " and bd.active = {$my['options']['active']}  and bd.testplan_id in (select id from ".$db->get_table('testplans')." where active = ".ACTIVE.")";
      }
      
      $sql .= " order by nh.name asc, build_id asc";
      
      //get build set 
      $buildSet = $db->fetchRowsIntoMap($sql,'build_id');
       
      // filter the  exception
      $doReindex = false;
      if( $this->globalRoleID != TL_ROLES_ADMIN && count($buildSet) > 0 )
      {
          foreach($buildSet as $idx => $item)
          {
              if( $item['is_public'] == 0 && $item['has_role'] == 0 )
              {
                  unset($buildSet[$idx]);
      ...
      if($doReindex)
      {
          $buildSet = array_values($buildSet);
      }
...
//     $sql = " SELECT id FROM {$tables['users']} ";
    $sql = " SELECT id FROM ".$db->get_table('users')." ";
  ...
  public function setActive(&$db,$value)
  {
    ...
    $sql = " UPDATE ".$db->get_table('users')." SET active = {$booleanVal} " .
...
//       $gsql = " SELECT password FROM {$this->object_table} WHERE id = " . intval($this->dbID);
      $gsql = " SELECT password FROM ".$db->get_table($this->object_table)." WHERE id = " . intval($this->dbID);
      $rs = $db->get_recordset($gsql);
      if(strcmp($rs[0]['password'],$this->password) != 0) 
      {
...
//     $sql = "SELECT COUNT(0) AS hits FROM $this->object_table " .
    $sql = "SELECT COUNT(0) AS hits FROM ".$db->get_table($this->object_table)." " .
...
//     $sql = " SELECT user_id FROM {$tables['user_testproject_roles']} " .
    $sql = " SELECT user_id FROM ".$dbHandler->get_table('user_testproject_roles')." " .
...
//     $sql = " SELECT user_id FROM {$tables['user_testplan_roles']} " .
    $sql = " SELECT user_id FROM ".$dbHandler->get_table('user_testplan_roles')." " .
...
//     $sql = "SELECT * FROM {$tables['users']} WHERE script_key='" . $dbHandler->prepare_string($target) . "'";
    $sql = "SELECT * FROM ".$dbHandler->get_table('users')." WHERE script_key='" . $dbHandler->prepare_string($target) . "'";
