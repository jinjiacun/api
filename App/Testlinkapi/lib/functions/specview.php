                       ...
                       $map_node_tccount, $filters=null, $options = null, $tproject_id = null)
{
  ...
  $my['filters'] = array('keywords' => 0, 'testcases' => null ,'exec_type' => null, 
                         'importance' => null, 'cfields' => null, 'assign_status' => null);
  ...
  $key2map = array('keyword_id' => 'keywords', 'tcase_id' => 'testcases', 
                   ...
                   'status' => 'workflow_status', 'assign_status' => 'assign_status');
                                   ...
                                   $options = null)
{
  ...
  if( property_exists($argsObj,'build_id') && intval($argsObj->build_id) > 0 )
  {
      $filters['build_id'] = intval($argsObj->build_id);
  }
 ...
 *
 */
function getFilteredLinkedVersionsNew(&$dbHandler,&$argsObj, &$tplanMgr, &$tcaseMgr, $filters,
    $options = null)
{
    static $tsuite_mgr;
    ...
    $tplan_tcases = $tplanMgr->newgetLinkedTCVXmen($argsObj->id, $filters, $opx);
...
function getTestSpecFromNode(&$dbHandler,&$tcaseMgr,&$linkedItems,$masterContainerId,$nodeId,$specViewType,$filters)
{
  ...
  $nullCheckFilter = array('tcase_id' => false, 'importance' => false,'tcase_name' => false, 
                           'cfields' => false, 'status' => false, 'assign_status' => false);
  ...
  if( $useFilter['assign_status'] && $filters['assign_status'])
  {
      $applyFilters = true;
      ...
      $assign_sql = "select tt.tcversion_id from " . 
          $dbHandler->get_table('testplan_tcversions') . " tt " .
          " where tt.build_id = '" . $build_id ."'";

      $assign_list = $dbHandler->fetchColumnsIntoArray($assign_sql, 'tcversion_id');
  }
  ...
  if( $applyFilters )
  {
    ...
    if( count($itemSet) > 0 && 
        ($useFilter['execution_type'] || $useFilter['importance'] || $useFilter['cfields'] || 
         $useFilter['status'] || ( $useFilter['assign_status'] && $filters['assign_status'])) 
      )
    {
        ...
        case 'testPlanLinking':
          ...
          foreach($targetSet as $idx => $key)
          {
            ...
            if( !is_null($item) )
            {
              ...
              if ($useFilter['assign_status'] && $filters['assign_status'] )
              {
                  if ( $filters['assign_status'] == 1 )
                  {
                      if (!in_array($item['tcversion_id'], $assign_list))
                      {
                          $tspecKey = $itemSet[$targetTestCase];  
                          $test_spec[$tspecKey]=null;
                      }
...
function addLinkedVersionsInfo($testCaseVersionSet,$a_tsuite_idx,&$out,&$linked_items,$opt=null)
{
  ...
  $optionalArrayFields = array('user_id','keyword','keyword_id');
 ...
 *
 */
function getFilteredSpecViewFlatNew(&$dbHandler, &$argsObj, &$tplanMgr, &$tcaseMgr, $filters=null, $options=null)
{
    $tprojectMgr = new testproject($dbHandler);
    $tsuite_data = $tcaseMgr->tree_manager->get_node_hierarchy_info($argsObj->id);

    $my = array();  // some sort of local scope
    $my['filters'] = array('testcaseFilter' => null,'executionTypeFilter' => null);
    $my['filters'] = array_merge($my['filters'], (array)$filters);
    ...
    // This does filter on keywords ALWAYS in OR mode.
    $tplan_linked_tcversions = getFilteredLinkedVersionsNew($dbHandler,$argsObj, $tplanMgr, $tcaseMgr, $filters, $options);
   /* 
    // With these pieces we implement the AND type of keyword filter.
    $testCaseSet = null;
    $tryNextFilter = true;
    ...
    */
    $result = array('spec_view'=>array(), 'num_tc' => 0, 'has_linked_items' => 0);
    $result['spec_view'] = $tplan_linked_tcversions;
    $result['num_tc'] = count($tplan_linked_tcversions);
    return $result;
