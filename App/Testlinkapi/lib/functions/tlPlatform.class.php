...
class tlPlatform extends tlObjectWithDB
{
  ...
  public function create($name, $notes=null)
  {
    ...
    {
      $sql = "INSERT INTO ".$this->db->get_table('platforms')." " .
  ...
  public function getByID($id)
  {
    $sql =  " SELECT id, name, notes " .
            " FROM ".$this->db->get_table('platforms')." " .
            " WHERE id = " . intval($id);
  ...
  {
    $safeName = $this->throwIfEmptyName($name);
    $sql = " UPDATE ".$this->db->get_table('platforms')." " .
  ...
  {
    $sql = "DELETE FROM ".$this->db->get_table('platforms')." WHERE id = {$id}";
    $result = $this->db->exec_query($sql);
  ...
  public function linkToTestplan($id, $testplan_id)
  {
    ...
    if( !is_null($id) )
    {
      ...
      {
        $sql = " INSERT INTO ".$this->db->get_table('testplan_platforms')." " .
  ...
  public function unlinkFromTestplan($id,$testplan_id)
  {
    ...
    if( !is_null($id) )
    {
      ...
      foreach ($idSet as $platform_id)
      {
        $sql = " DELETE FROM ".$this->db->get_table('testplan_platforms')." " .
  ...
  {
    $sql = " SELECT id FROM ".$this->db->get_table('platforms')." " .
  ...
  public function getAll($options = null)
  {
    ...
    $sql =  " SELECT id, name, notes  FROM ".$this->db->get_table('platforms')." PLAT {$tproject_filter} " .
            " ORDER BY name";
    ...
    if( !is_null($rs) && $options['include_linked_count'])
    {
      ...
      $sql =  " SELECT PLAT.id, COUNT(TPLAT.testplan_id) AS linked_count " .
          " FROM ".$this->db->get_table('platforms')." PLAT " .
          " LEFT JOIN ".$this->db->get_table('testplan_platforms')." TPLAT " .
          " ON TPLAT.platform_id = PLAT.id " . $tproject_filter .
          " GROUP BY PLAT.id ";
  ...
  public function platformsActiveForTestplan($testplan_id)
  {
    ...
    $sql = "/* $debugMsg */ SELECT COUNT(0) AS num " .
         " FROM ".$this->db->get_table('testplan_platforms')." " .
         " WHERE testplan_id = {$testplan_id}";
  ...
  public function getLinkedToTestplan($testplanID, $options = null)
  {
    ...
    $sql = "/* $debugMsg */ SELECT P.id, P.name, P.notes " .
           " FROM ".$this->db->get_table('platforms')." P " .
           " JOIN ".$this->db->get_table('testplan_platforms')." TP " .
  ...
  {
    $sql = "DELETE FROM ".$this->db->get_table('platforms')." WHERE testproject_id = {$tproject_id}";
    $result = $this->db->exec_query($sql);
  ...
  public function testProjectCount($opt=null)
  {
    ...
    $sql = $debugMsg . " SELECT COALESCE(COUNT(PLAT.id),0) AS platform_qty, TPROJ.id AS tproject_id " .
           " FROM ".$this->db->get_table('testprojects')." TPROJ " .
           " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLAT ON PLAT.testproject_id = TPROJ.id ";
  ...
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = " SELECT platform_id FROM ".$this->db->get_table('testplan_platforms')." " .
