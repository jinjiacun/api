...
class tlRequirementFilterControl extends tlFilterControl 
{
  ...
  private $all_filters = array('filter_doc_id' => array("POST", tlInputParameter::STRING_N),
                               ...
                               'filter_testplan' => array("POST", tlInputParameter::STRING_N),
  ...
  /**
   * @author zhouzhaoxin
   * @method init_filter_testplan add testplan filter
   * @since 20161229
   */
  private function init_filter_testplan()
  {
      $key = 'filter_testplan';
      $items = array();
      $selection = $this->args->{$key};
      ...
      $tplan_list = $this->testproject_mgr->get_all_testplans($this->args->testproject_id);
      $items[0] = '-All-';
      ...
      if (!$selection || $this->args->reset_filters || $selection == 0)
      {
          $selection = 0;
      }
      else
