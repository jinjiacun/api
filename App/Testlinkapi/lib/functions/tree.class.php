...
class tree extends tlObject
{
  ...
  function get_available_node_types() 
  {
    ...
    if( !$nodeTypes )
    {
      $sql = " SELECT * FROM ".$this->db->get_table('node_types')." "; 
   ...
   */
  function new_root_node($name = '', $schema = '', $parent_id = '') 
  {
      $table = $schema . "." . $this->object_table;
      $sql = "INSERT INTO " . $table .
          " (id, name, node_type_id, node_order, node_depth_abs) VALUES('" . 
          $parent_id . "','" . $this->db->prepare_string($name). "'," .
          "{$this->ROOT_NODE_TYPE_ID}, " . "1, '');";  
  ...
  */
  ...
  {
    $sql = "INSERT INTO " . $this->db->get_table($this->object_table) .
    ...
    {
      $lock_sql = "lock tables " . $this->db->get_table($this->object_table) . " WRITE";
      $this->db->exec_query($lock_sql);
      $sub_count = $this->get_subtree_count($parent_id);
      $parent_abs = $this->get_node_abs_by_id($parent_id);
      if ($sub_count > 9998) {
          return -1;
      }
      $abs_format = "%0" . TL_NODE_ABS_LENGTH . "d";
      $node_abs = $parent_abs . sprintf($abs_format, $sub_count + 1);
      $sql .= ",parent_id,node_depth_abs) {$values},{$parent_id},'{$node_abs}')";
      $lock_t = true;
    }
  ...
  */
  function get_node_hierarchy_info($node_id,$parent_id = null,$options=null)
  {
    ...
    if ($opt['nodeType'] == 'testproject')
    {
        //modify by zhouzhaoxin 20161109 for divide schema, if node is project,should query project table 
        $sql = "SELECT pj.id, pj.name, nh.parent_id, nh.node_type_id, nh.node_order" .
               " FROM " . $this->db->get_table('testprojects') . " pj " .
               "join " . $this->db->get_schema($node_id) . "." . "nodes_hierarchy  nh " .
               "on pj.node_id = nh.id";
    ...
    else 
    {
        $sql = "SELECT {$opt['fields']} " . 
               "FROM " . $this->db->get_table($this->object_table) . " WHERE id";
  ...
  */
  ...
  {
    $sql = "SELECT id from " . $this->db->get_table($this->object_table) . " WHERE parent_id = {$node_id}";
  ...
  */
  function delete_subtree($node_id)
  {
    ...
    $sql = "/* $debugMsg */ DELETE FROM " . $this->db->get_table($this->object_table) . " WHERE id IN ({$id2del})";
  ...
  */
  function _get_path($node_id,&$node_list,$to_node_id=null,$format='full') 
  {
    ...
    $sql = "/* $debugMsg */ " . 
           ...
           " FROM " . $this->db->get_table($this->object_table) . " WHERE id = " . intval($node_id);
  ...
  */
  function change_parent($node_id, $parent_id) 
  {
    ...
    $process_id_list = array();
    if (!is_array($node_id))
    {
        $process_id_list[] = $node_id;
    }
    else 
    {
    ...
    while (count($process_id_list, COUNT_NORMAL) > 0)
    {
        foreach ($process_id_list as $key => $rep_node_id)
        {
            $lock_sql = "lock tables " . $this->db->get_table($this->object_table) . " WRITE";
            $this->db->exec_query($lock_sql);
            ...
            if ($first_layer)
            {
                ...
                $sub_count = $this->get_subtree_count($node_parent);
                $parent_abs = $this->get_node_abs_by_id($node_parent);
                if ($sub_count > 9998) {
                    return -1;
                }
                $abs_format = "%0" . TL_NODE_ABS_LENGTH . "d";
                ...
                $sql = "/* $debugMsg */ UPDATE " . $this->db->get_table($this->object_table) .
                    " SET node_depth_abs = '" . $node_abs . "' where id = " . $rep_node_id;
                $this->db->exec_query($sql);
            }
            else 
            {
                $sql = "select parent_id from " . $this->db->get_table($this->object_table) .
                " where id = " . $rep_node_id;
                $node_parent = $this->db->fetchOneValue($sql);
                $parent_abs = $this->get_node_abs_by_id($node_parent);
                $child_abs = $this->get_node_abs_by_id($rep_node_id);  
                $node_abs = $parent_abs . substr($child_abs, -4);
                $sql = "/* $debugMsg */ UPDATE " . $this->db->get_table($this->object_table) .
                    " SET node_depth_abs = '" . $node_abs . "' where id = " . $rep_node_id;
                $this->db->exec_query($sql);
            }
            ...
            $sql = "select * from " . $this->db->get_table($this->object_table) . 
                " where parent_id = " . $rep_node_id;
            $res = $this->db->get_recordset($sql);
            if (count($res,COUNT_NORMAL) > 0)
            {
                $loop2do = count($res);
                for($idx=0; $idx < $loop2do; $idx++)
                {
                    $temp_id_list[] = $res[$idx]['id'];
                }
            }
        }
        
        unset($process_id_list);
        foreach ($temp_id_list as $key => $val)
        {
            $process_id_list[$key] = $val;
        }
        unset($temp_id_list);
  ...
  */
  function get_children($id,$exclude_node_types=null,$opt=null) 
  {
    ...
    $sql = "/* $debugMsg */ " .
           " SELECT id,name,parent_id,node_type_id,node_order FROM " . $this->db->get_table($this->object_table) .
  ...
  function change_order_bulk($nodes) 
  {
    foreach($nodes as $order => $node_id)
    {
      ...
      $node_id = intval($node_id);
        $sql = "UPDATE " . $this->db->get_table($this->object_table) . " SET node_order = {$order} WHERE id = {$node_id}";
        $result = $this->db->exec_query($sql);
    }
  }
  ...
  */
  function change_child_order($parent_id,$node_id,$top_bottom,$exclude_node_types=null)
  {
      ...
      $sql = " SELECT NH.id, NH.node_order, NH.name " .
             " FROM " . $this->db->get_table($this->object_table)." NH, ".$this->db->get_table('node_types')." NT " .
  ...
  */
  function getBottomOrder($parentID,$opt=null)
  {
    ...
    $sql = "SELECT MAX(node_order) AS max_order" .
           " FROM " . $this->db->get_table($this->object_table) . 
  ...
  */
    ...
    switch($my['options']['order_cfg']['type'])
    {
      case 'spec_order':
        $sql = " SELECT id,name,parent_id,node_type_id,node_order {$my['options']['addFields']}" .
               " FROM " . $this->db->get_table($this->object_table) . " {$my['options']['addJoin']} " .
        ...
        case 'rspec':
          $sql = " SELECT OBT.id,name,parent_id,node_type_id,node_order,RSPEC.doc_id " .
                 " FROM " . $this->db->get_table($this->object_table) . " AS OBT " .
                 " JOIN ".$this->db->get_table('req_specs')." AS RSPEC ON RSPEC.id = OBT.id " .
                 " WHERE parent_id = {$node_id} {$my['filters']['additionalWhereClause']}" .
      ...
      // 20101009 - franciscom - added tcversion_id , neeed for test plan export
          $sql="SELECT * FROM ( SELECT NH.node_order AS spec_order," . 
               ...
               "                FROM " . $this->db->get_table($this->object_table) . " NHA, " . $this->db->get_table($this->object_table) . " NHB," .
               "                     ".$this->db->get_table('testplan_tcversions')."  T,".$this->db->get_table('node_types')." NT" .
  ...
  function _get_subtree_rec($node_id,&$pnode,$filters = null, $options = null)
  {
    ...
    if (!$tcNodeTypeID)
    {
      ...
      {
          case 'spec_order':
            $sql = " SELECT id,name,parent_id,node_type_id,node_order FROM " . $this->db->get_table($this->object_table) .
                      " WHERE parent_id = {$node_id} {$my['filters']['additionalWhereClause']}" .
                 " ORDER BY node_order,id";
        break;
      ...
      $sql="SELECT * FROM ( SELECT NH.node_order AS spec_order," . 
           ...
           "                     ".$this->db->get_table('testplan_tcversions')." T" .
           "                WHERE NHA.id=NHB.parent_id " .
  ...
  function get_full_path_verbose(&$items,$options=null)
  {
      ...
      { 
        ...
        $sql="/* $debugMsg */ " . 
             " SELECT id,name FROM ".$this->db->get_table('nodes_hierarchy')."  WHERE id IN ({$unique_nodes})"; 
  ...
  function nodeNameExists($name,$node_type_id,$id=null,$parent_id=null)
  {
    ...
    if( !is_null($id) )
    {
      ...
      {
        $sql = "/* {$debugMsg} */ " . 
               " SELECT parent_id FROM " . $this->db->get_table($this->object_table) . " NHA " .
    ...
    $sql = "/* {$debugMsg} */ " . 
           " SELECT count(0) AS qty FROM " . $this->db->get_table($this->object_table) . " NHA " .
                                  ...
                                  $exclude_branches = null)
  {
    ...
    $sql = "/* $debugMsg */ SELECT NH.* FROM " . $this->db->get_table($this->object_table) . " NH " .
    ...
    if( !is_null($root_id) && ($node_id != $root_id) )
    {
      ...
      if( count($children) == 0 )
      {
        $sql2 = "/* $debugMsg */ SELECT NH.* FROM " . $this->db->get_table($this->object_table) . " NH " .
  ...
  */
  ...
  function getAllItemsID($parentList,&$itemSet,$coupleTypes)
  {
    ...
    $sql = "/* $debugMsg */  " .
           " SELECT id,node_type_id from ".$this->db->get_table('nodes_hierarchy')." " .
  ...
  function getNodeByAttributes($attr)
  {
    ...
    $sql .= " SELECT NH_MAIN.id,NH_MAIN.parent_id,NH_MAIN.name,NH_MAIN.node_type_id " .
            " FROM " . $this->db->get_table($this->object_table) . " AS NH_MAIN " .
            " JOIN ".$this->db->get_table('node_types')." AS NT ON NT.id = NH_MAIN.node_type_id ";
    ...
    foreach($attr as $key => $value)
    {
      switch($key)
      {
        ...
        case 'parent_id':
          ...
          $addJoin = " JOIN " . $this->db->get_table($this->object_table) . " AS NH_PARENT ON NH_PARENT.id = NH_MAIN.parent_id "; 
        break;
      }
    }
  ...
  /* get_subtree_count
   * get sub tree count on one node, include all types
   * @param   $node_id => node id
   * @return  node child count
   * @author  zhouzhaoxin
   * @history add on 20161111
   */
  function get_subtree_count($node_id)
  {
      $sql = " SELECT COUNT(id) AS count FROM " . $this->db->get_table($this->object_table) . " " .
           " WHERE parent_id = {$node_id}";
      ...
      {
          $sql = "select max(node_depth_abs) from " . $this->db->get_table('nodes_hierarchy') .
              " where parent_id =  {$node_id}";
          $node_abs = $this->db->fetchOneValue($sql); 
          $node_count = substr($node_abs, -4);
          return $node_count;
      }
  }
   ...
   * @history add on 20161111
   */
  function get_node_abs_by_id($node_id)
  {
      $sql = " SELECT node_depth_abs FROM " . $this->db->get_table($this->object_table) . " " .
          " WHERE id = {$node_id}";
  
      $abs = $this->db->fetchOneValue($sql);
      return $abs;
  } 
}// end class
