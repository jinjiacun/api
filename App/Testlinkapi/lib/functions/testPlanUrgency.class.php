...
class testPlanUrgency extends testplan
{
  ...
  {
    $sql = " UPDATE ".$this->db->get_table('testplan_tcversions')." SET urgency={$urgency} " .
  ...
  {
    $sql = " UPDATE ".$this->db->get_table('testplan_tcversions')." " . 
...
//            " {$this->tables['nodes_hierarchy']} NHB, {$this->tables['node_types']} NT " .
           ...
           " ".$this->db->get_table('nodes_hierarchy')." NHB, ".$this->db->get_table('node_types')." NT " .
...
//            " WHERE testprojects.id = ";
    $sql = " SELECT testprojects.prefix  FROM ".$this->db->get_table('testprojects')." testprojects " .
           " WHERE testprojects.id = ";
...
//               "  WHERE id=" . intval($testplan_id) . " ) ";
      $sql .= "( SELECT parent_id AS testproject_id FROM ".$this->db->get_table('nodes_hierarchy')." " .
              "  WHERE id=" . intval($testplan_id) . " ) ";
    }
