...
class testcase extends tlObjectWithAttachments
{
  ...
  const EXECUTION_TYPE_MANUAL_AUTO=3;
  ...
  var $import_file_types = array("XLS"=>"XLS","XML" => "XML");
  var $export_file_types = array("XLS"=>"XLS","XML" => "XML");
  var $execution_types = array();
  var $complexity=array();
  var $reviewed_status=array();
  var $cfg;
  var $debugMsg;
  var $layout;
  ...
  function __construct(&$db)
  {
    ...
    $this->reviewed_status = $this->getReviewed_status();
  ...
  static function getExecutionTypes()
  {
    $stdSet = array(self::EXECUTION_TYPE_MANUAL => lang_get('manual'),
                    ...
                    self::EXECUTION_TYPE_MANUAL_AUTO =>lang_get('manual_automated'));
   ...
   * history:2016.6.8
   * **/
  static function getComplexity()
  { 
      $stdSet=array('1'=>lang_get('very_simple'),'2'=>lang_get('little_simple'),'3'=>lang_get('general'),'4'=>lang_get('complex'),'5'=>lang_get('very_complex'));
   ...
   * history:2016.6.8
   * **/
  static function getReviewed_status()
  {
      $stdSet=array('1'=>lang_get('wait_review'),'2'=>lang_get('reviewed_passed'),'3'=>lang_get('reviewed_not_pass'));
  ...
  */
  ...
  function createFromObject($item)
  {
    ...
    $ret = $this->create($item->testSuiteID,$item->name,$item->summary,$item->preconditions,
                         ...
                         $item->executionType,$item->importance,$item->tc_id,$item->bpm_id,
                         $item->complexity,$item->reviewed_status,$item->creation_ts,
                         $item->reviewer_id,$item->authorID,$options);
    return $ret;
  }
  ...
  function create($parent_id,$name,$summary,$preconditions,$steps,$author_id,
                  ...
                  $importance=2,$tc_id,$bpm_id,$complexity,$reviewed_status,
                  $reviewer_id,$options=null)
  {
    $status_ok = 1;
    ...
    if($ret["status_ok"])
    {
      ...
      $ix->tc_id=$tc_id;
      $ix->bpm_id=$bpm_id;
      $ix->complexity=$complexity;
      $ix->reviewed_status=$reviewed_status;
      $ix->reviewer_id=$reviewer_id;
      ...
      if($ret['update_name'])
      {
        $sql = " UPDATE ".$this->db->get_table('nodes_hierarchy')." SET name='" .
 ...
 private function createVersion($item)
 {
    ...
    $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('tcversions')." " .
           " (id,tc_external_id,version,summary,preconditions," . 
           "  author_id,creation_ts,execution_type,importance,tc_id,bpm_id,complexity,reviewed_status,reviewer_id ";
    ...
    ", {$item->executionType},{$item->importance},'$tc_id','$bpm_id',
    '$complexity','$reviewed_status','$reviewer_id'";
    ...
    if ($result && ( !is_null($item->steps) && is_array($item->steps) ) )
    {
      ...
      for($jdx=0 ; ($jdx < $steps2create && $op['status_ok']); $jdx++)
      {
        ...
        if (isset($item->steps[$jdx]['step_number']))
        {
            if (!isset($item->steps[$jdx]['actions']))
            {
                $item->steps[$jdx]['actions'] = "";
            }
            if (!isset($item->steps[$jdx]['expected_results']))
            {
                $item->steps[$jdx]['expected_results'] = "";
            }
            if (!isset($item->steps[$jdx]['actions']))
            {
                $item->steps[$jdx]['execution_type'] = 1;
            }
  ...
  */
  function getDuplicatesByName($name, $parent_id, $options=null)
  {
    ...
    $sql = " SELECT DISTINCT NHA.id,NHA.name,TCV.tc_external_id" .
           ...
           " ".$this->db->get_table('nodes_hierarchy')." NHB, ".$this->db->get_table('tcversions')." TCV  " .
  ...
  */
  function get_by_name($name, $tsuite_name = '', $tproject_name = '')
  {
    ...
    $sql = "/* $debugMsg */ " .       
           ...
           " FROM ".$this->db->get_table('nodes_hierarchy')." NH_TCASE, " .
           " ".$this->db->get_table('nodes_hierarchy')." NH_TCASE_PARENT, " .
           " ".$this->db->get_table('nodes_hierarchy')." NH_TCVERSIONS," .
           " ".$this->db->get_table('tcversions')."  TCV  " .
  ...
  */
  function get_all()
  {
    $sql = " SELECT nodes_hierarchy.name, nodes_hierarchy.id
             FROM  ".$this->db->get_table('nodes_hierarchy')." nodes_hierarchy
  ...
  function show(&$smarty,$guiObj,$identity,$grants)
  {
    ...
    if($status_ok && sizeof($idSet))
    {
      ...
      foreach($idSet as $key => $tc_id)
      {
        ...
        //modify by zhouzhaoxin 20160708 for show assgin to build info, not version
        if($cfg->show_tplan_usage)
        {
          $gui->linked_versions[] = $this->get_linked_builds($tc_id);
        }  
        ...
        //add by zhouzhaoxin 20161115 for add reviewer name
        $reviewer_id = $tc_array[0]['reviewer_id'];
        $gui->reviewer_name = get_user_name_by_id($this->db, $reviewer_id);
  ...
  function update($id,$tcversion_id,$name,$summary,$preconditions,$steps,
                  ...
                  $execution_type=TESTCASE_EXECUTION_TYPE_MANUAL,$importance=2,$tc_id,$bpm_id,
                  $complexity,$reviewed_status,$reviewer_id,$attr=null,$opt=null)
  {
    $ret['status_ok'] = 1;
    $ret['msg'] = '';
    $ret['reason'] = '';
    ...
    {    
      ...
      {
        $sql = " SELECT id FROM ".$this->db->get_table('executions')." " .
      ...
      $dummy = " UPDATE ".$this->db->get_table('tcversions')." " .
               ...
               "tc_id='$tc_id', bpm_id='$bpm_id',complexity='$complexity',reviewed_status='$reviewed_status',".
      ...
      if( $ret['status_ok'] )
      {      
          if (isset($my['opt']['importbyxls']) && $my['opt']['importbyxls'])
          {
              $this->addNewKeywords($id,$keywords_id);
          }
          else
          {
  ...
  //20170430 add by zhouzhaoxin for add import keywords but not delete old keywords
  private function addNewKeywords($id,$keywords_id)
  {
      $items = array();
      ...
      if(trim($keywords_id) != "")
      {
          ...
          $sql = " SELECT id,keyword " .
              " FROM ".$this->db->get_table('keywords')." " .
  ...
  function delete($id,$version_id = self::ALL_VERSIONS)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      ...
      $sql="/* $debugMsg */ " . 
           ...
           " FROM ".$this->db->get_table('nodes_hierarchy')." NH_TCV " .
           " LEFT OUTER JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCSTEPS " . 
  ...
  */
    ...
    switch ($exec_status)
    {
      case "ALL":
            $sql = "/* $debugMsg */ " .       
             ...
             " ".$this->db->get_table('tcversions')." tcversions," .
             " ".$this->db->get_table('testplan_tcversions')." TTC, " .
  ...
  /*
   function: get_linked_builds
   For a test case get information about builds and testplans linked to builds.
  
   args : 
   [id]: testcase id
   [filters]: null, for retainde
   [options]: null, for retained
   ...
   value: map with following structure:
     ...
     build_name
   */
  function get_linked_builds($id, $filters=null, $options=null)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      
//       $sql = "SELECT NH.parent_id AS testcase_id, TTC.build_id, TTC.tcversion_id, TTC.testplan_id,  TTC.platform_id, " .
//              "tcversions.id, tcversions.tc_external_id,tcversions.version,tcversions.status, " .
//              "tcversions.importance,tcversions.active, tcversions.is_open,tcversions.execution_type, " .
//              "tcversions.layout,tcversions.summary,tcversions.preconditions,tcversions.author_id, " .
//              "tcversions.creation_ts,tcversions.updater_id,tcversions.modification_ts, " .
//              "NHB.name AS tplan_name, builds.name as build_name " .
//              "FROM nodes_hierarchy NH, " .
//              "tcversions tcversions, " .
//              "testplan_tcversions TTC, " .
//              "nodes_hierarchy NHB, " .
//              "builds builds " .
//              "WHERE  TTC.tcversion_id = tcversions.id " .
//              "AND    tcversions.id = NH.id " .
//              "AND    NHB.id = TTC.testplan_id " .
//              "AND    TTC.build_id = builds.id " .
//              "AND    NH.parent_id = {$id}";
      ...
      $sql = "SELECT NH.parent_id AS testcase_id, TTC.build_id, TTC.tcversion_id, TTC.testplan_id,  TTC.platform_id, " .
          ...
          "".$this->db->get_table('testplan_tcversions')." TTC, " .
          "".$this->db->get_table('nodes_hierarchy')." NHB, " .
          "".$this->db->get_table('builds')." builds " .
          "WHERE  TTC.tcversion_id = tcversions.id " .
          "AND    tcversions.id = NH.id " .
          "AND    NHB.id = TTC.testplan_id " .
          "AND    TTC.build_id = builds.id " .
      ...
      $recordset = $this->db->fetchArrayRowsIntoMap($sql, 'build_id');
  ...
  */
  function _blind_delete($id,$version_id=self::ALL_VERSIONS,$children=null)
  {
    ...
    $sql[] = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('user_assignments')." " .
             " WHERE feature_id in (" .
             " SELECT id FROM ".$this->db->get_table('testplan_tcversions')."  " .
             " WHERE tcversion_id IN ({$tcversion_list}))";
    
    $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('testplan_tcversions')."  " .
           " WHERE tcversion_id IN ({$tcversion_list})";
    ...
    if( !is_null($children['step']) )
    {
      ...
      foreach($children['step'] as $key => $value)
      {
        ...
        { 
          ...
          $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('tcsteps')."  " .
                 " WHERE id IN ({$step_list})";
        }
      }
      $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('tcversions')."  " .
      ...
      if($destroyTC)
      {
        ...
        $sql = null;
        $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('testcase_keywords')." WHERE testcase_id = {$id}";
        $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('req_coverage')."  WHERE testcase_id = {$id}";
  ...
  */
  function _execution_delete($id,$version_id=self::ALL_VERSIONS,$children=null)
  {
    ...
    if( $version_id == self::ALL_VERSIONS )
    {
      ...
      $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('execution_bugs')." " .
             " WHERE execution_id IN (SELECT id FROM ".$this->db->get_table('executions')." " .
               " WHERE tcversion_id IN ({$tcversion_list}))";

          $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('cfield_execution_values')."  " .
               " WHERE tcversion_id IN ({$tcversion_list})";

          $sql[]="/* $debugMsg */ DELETE FROM ".$this->db->get_table('executions')."  " .
               " WHERE tcversion_id IN ({$tcversion_list})";
    ...
    if ($tcase_info)
    {
      ...
      if($newTCObj['status_ok'])
      {
        ...
        foreach($tcase_info as $tcversion)
        {
          ...
          $ix->tc_id = $tcversion['tc_id'];
          $ix->bpm_id = $tcversion['bpm_id'];
          $ix->complexity = $tcversion['complexity'];
          $ix->reviewed_status = $tcversion['reviewed_status'];
          $ix->authorID = $tcversion['author_id'];
          $ix->reviewer_id = $tcversion['reviewer_id'];
  ...
  */
  function get_last_version_info($id,$options=null)
  {
    ...
    $sql = "/* $debugMsg */ SELECT MAX(version) AS version " .
           ...
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.id = TCV.id ".
    ...
    if ($max_version)
    {
      $sql = " SELECT {$fields2get} FROM ".$this->db->get_table('tcversions')." TCV " .
             " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.id = TCV.id ".
             " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TC ON NH_TC.id = NH_TCV.parent_id ".
  ...
  */
  function copy_tcversion($id,$from_tcversion_id,$to_tcversion_id,$as_version_number,$user_id)
  {
    ...
    $now = $this->db->db_now();
    $sql = "/* $debugMsg */ " . 
           " INSERT INTO ".$this->db->get_table('tcversions')." " . 
           " (id,version,tc_external_id,author_id,creation_ts,summary, " . 
           "  importance,execution_type,preconditions,estimated_exec_duration,tc_id,complexity,reviewed_status,reviewer_id,bpm_id) " .
           " SELECT {$to_tcversion_id} AS id, {$as_version_number} AS version, " .
           "        tc_external_id, " .
           "        {$user_id} AS author_id, {$now} AS creation_ts," .
           "        summary,importance,execution_type, preconditions,estimated_exec_duration,tc_id,complexity,reviewed_status,reviewer_id,bpm_id " .
           " FROM ".$this->db->get_table('tcversions')." " .
  ...
  */
  function get_by_id_bulk($id,$version_id=self::ALL_VERSIONS, $get_active=0, $get_open=0)
  {
    ...
    $sql = " /* $debugMsg */ SELECT nodes_hierarchy.parent_id AS testcase_id, ".
           ...
           " FROM ".$this->db->get_table('nodes_hierarchy')." nodes_hierarchy " .
           " JOIN ".$this->db->get_table('tcversions')." tcversions ON nodes_hierarchy.id = tcversions.id " .
             " LEFT OUTER JOIN ".$this->db->get_table('users')." users ON tcversions.author_id = users.id " .
    ...
    if($recordset)
    {
     ...
     $sql = " /* $debugMsg */ " . 
            ...
            " FROM ".$this->db->get_table('nodes_hierarchy')." nodes_hierarchy {$where_clause_names} ";
     ...
     $sql = " /* $debugMsg */ " . 
            ...
            " FROM ".$this->db->get_table('nodes_hierarchy')." nodes_hierarchy " .
            " JOIN ".$this->db->get_table('tcversions')." tcversions ON nodes_hierarchy.id = tcversions.id " .
              " LEFT OUTER JOIN ".$this->db->get_table('users')." users ON tcversions.updater_id = users.id " .
    ...
    switch($my['options']['output'])
    {
      ...
      case 'full_without_steps':
        $sql = "SELECT UA.login AS updater_login,UB.login AS author_login,
                ...
                JOIN ".$this->db->get_table('nodes_hierarchy')." NHTC ON NHTCV.parent_id = NHTC.id
                JOIN ".$this->db->get_table('tcversions')." TCV ON NHTCV.id = TCV.id
                LEFT OUTER JOIN ".$this->db->get_table('users')." UB ON TCV.author_id = UB.id
                LEFT OUTER JOIN ".$this->db->get_table('users')." UA ON TCV.updater_id = UA.id
  ...
  */
  function get_versions_status_quo($id, $tcversion_id=null, $testplan_id=null)
  {
    ...
    if(!is_null($testplan_id))
      ...
      $execution_join=" LEFT OUTER JOIN ".$this->db->get_table('executions')." E " .
                      " ON (E.tcversion_id = NH.id {$testplan_filter})";
  
    $sqlx=  " SELECT TCV.id,TCV.version " .
              " FROM ".$this->db->get_table('nodes_hierarchy')." NHA " .
              " JOIN ".$this->db->get_table('nodes_hierarchy')." NHB ON NHA.parent_id = NHB.id " .
              " JOIN ".$this->db->get_table('tcversions')."  TCV ON NHA.id = TCV.id " .
    ...
    $sql="SELECT DISTINCT NH.id AS tcversion_id,T.tcversion_id AS linked, " .
         ...
         " LEFT OUTER JOIN ".$this->db->get_table('testplan_tcversions')." T ON T.tcversion_id = NH.id " .
  ...
  */
  function get_exec_status($id,$filters=null, $options=null)
  {
    ...
    if(!is_null($tplan_id))
    {
          $sqlx .= " JOIN ".$this->db->get_table('testplan_tcversions')."  TTCV ON TTCV.tcversion_id = TCV.id ";
    ...
    $sql = "/* $debugMsg */ " .
           ...
           " FROM   ".$this->db->get_table('nodes_hierarchy')." NH " .
           " JOIN ".$this->db->get_table('testplan_tcversions')."  T ON T.tcversion_id = NH.id " .
           " JOIN ".$this->db->get_table('tcversions')."  TCV ON T.tcversion_id = TCV.id " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHB ON T.testplan_id = NHB.id " .
           " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLAT " .
  ...
  {          
    ...
    {  
      $sql = " SELECT id,is_public  FROM ".$this->db->get_table('testprojects')." " .
  ...
  */
  ...
  function getKeywords($tcID,$kwID = null,$opt = null)
  {
    ...
    $sql = " SELECT {$f2g}  
             ...
             JOIN ".$this->db->get_table('keywords')." keywords 
  ...
  */
  function get_keywords_map($id,$opt=null)
  {
    ...
    $sql .= " FROM ".$this->db->get_table('testcase_keywords')." testcase_keywords, " .
            " ".$this->db->get_table('keywords')." keywords WHERE keyword_id = keywords.id ";
  ...
  // add by steven.zhou
  function get_keywords_projectid($testproject_id)
  {
      $sql = "SELECT id,keyword,testproject_id,notes FROM ".$this->db->get_table('keywords'). " k WHERE k.testproject_id = {$testproject_id}";
  ...
  */
  function addKeyword($id,$kw_id,$audit=self::AUDIT_ON)
  {
    ...
    // Ok, ho ahead
    $sql = " INSERT INTO ".$this->db->get_table('testcase_keywords')." (testcase_id,keyword_id) " .
         " VALUES ($id,$kw_id)";
  ...
  */
  ...
  {
    $sql = " DELETE FROM ".$this->db->get_table('testcase_keywords')."  WHERE testcase_id = {$tcID} ";
    if (!is_null($kwID))
    {
      if(is_array($kwID))
        {
  ...
  */
  function get_executions($id,$version_id,$tplan_id,$build_id,$platform_id,$options=null)
  {
    ...
    $sql="/* $debugMsg */ SELECT NHB.name,NHA.parent_id AS testcase_id, tcversions.*,
        ...
        FROM ".$this->db->get_table('nodes_hierarchy')." NHA
          JOIN ".$this->db->get_table('nodes_hierarchy')." NHB ON NHA.parent_id = NHB.id
          JOIN ".$this->db->get_table('tcversions')." tcversions ON NHA.id = tcversions.id
          JOIN ".$this->db->get_table('executions')." e ON NHA.id = e.tcversion_id
                                               AND e.testplan_id = {$tplan_id}
                                               {$filterBy['build_id']} {$filterBy['platform_id']}
          JOIN ".$this->db->get_table('builds')."  b ON e.build_id=b.id
          LEFT OUTER JOIN ".$this->db->get_table('users')." users ON users.id = e.tester_id
          LEFT OUTER JOIN ".$this->db->get_table('platforms')." p ON p.id = e.platform_id
          $where_clause
  ...
  */
  function get_last_execution($id,$version_id,$tplan_id,$build_id,$platform_id,$options=null)
  {
    ...
    $sql="/* $debugMsg */ " . 
         ...
         " FROM ".$this->db->get_table('nodes_hierarchy')." NHA " .
         " JOIN ".$this->db->get_table('executions')." e ON NHA.id = e.tcversion_id AND e.testplan_id = {$tplan_id} " .
         " {$filterBy['build_id']} {$filterBy['platform_id']}" .
    ...
    $executions_join=" JOIN ".$this->db->get_table('executions')." e ON NHA.id = e.tcversion_id " .
    ...
    switch ($localOptions['output']) 
    {
      ...
      default:
        $sql= "/* $debugMsg */ SELECT e.id AS execution_id, " .
              ...
              " JOIN ".$this->db->get_table('tcversions')." tcversions ON NHA.id = tcversions.id" .
              " {$executions_join}" .
              " LEFT OUTER JOIN ".$this->db->get_table('builds')." builds ON builds.id = e.build_id" .
  ...
  */
  function get_version_exec_assignment($tcversion_id, $tplan_id, $build_id)
  {
    ...
    $sql =  "SELECT T.tcversion_id AS tcversion_id,T.id AS feature_id,T.platform_id, " .
            ...
            " LEFT OUTER JOIN ".$this->db->get_table('user_assignments')."  UA ON UA.feature_id = T.id " .
  ...
  function get_assigned_to_user($user_id,$tproject_id,$tplan_id=null,$options=null, $filters=null)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      ...
      $sql="/* $debugMsg */ SELECT TPROJ.id as testproject_id,TPTCV.testplan_id,TPTCV.tcversion_id, " .
           ...
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCV ON NHTCV.id = TCV.id " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTC ON NHTC.id = NHTCV.parent_id " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTPLAN ON  NHTPLAN.id=TPTCV.testplan_id " .
           " JOIN ".$this->db->get_table('testprojects')." TPROJ ON  TPROJ.id = NHTPLAN.parent_id " .
           " JOIN ".$this->db->get_table('testplans')." TPLAN ON  TPLAN.id = TPTCV.testplan_id " .
           " JOIN ".$this->db->get_table('builds')." BUILDS ON  BUILDS.id = UA.build_id " .
  ...
  */
  ...
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql =  " /* $debugMsg */ UPDATE ".$this->db->get_table('tcversions')." " .
  ...
  */
  function update_external_id($id,$external_id)
  {
    ...
    $sql =  "/* $debugMsg */ UPDATE ".$this->db->get_table('tcversions')." " .
        ...
        " WHERE id IN (" .
        " SELECT id FROM ".$this->db->get_table('nodes_hierarchy')." WHERE parent_id={$id} ) ";
  ...
  /**
    ...
    $sql = array("DELETE FROM ".$this->db->get_table('execution_bugs')." {$whereClause} ", 
                 "DELETE FROM ".$this->db->get_table('cfield_execution_values')." {$whereClause} ",
                 "DELETE FROM ".$this->db->get_table('executions')." WHERE id = {$executionID}" );
  ...
  */
  ...
  function get_basic_info($id,$accessVersionBy)
  {
    ...
    $sql = "/* $debugMsg */ " .
           ...
           " FROM ".$this->db->get_table('nodes_hierarchy')." NH_TCASE " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.parent_id = NH_TCASE.id" .
                         ...
                         $execution_type=TESTCASE_EXECUTION_TYPE_MANUAL)
  {
    ...
    $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('tcsteps')." " .
  ...
  function get_steps($tcversion_id,$step_number=0,$options=null)
  {
    ...
    $sql = "/* $debugMsg */ " . 
           ...
           " FROM ".$this->db->get_table('tcsteps')." TCSTEPS " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_STEPS " .
    ...
    if ($my['options']['fields2get'] == '*' ||
        (strpos($my['options']['fields2get'], 'actions') && strpos($my['options']['fields2get'], 'expected_results')))
    {
  ...
  function get_step_by_id($step_id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* $debugMsg */ " . 
           " SELECT TCSTEPS.* FROM ".$this->db->get_table('tcsteps')." TCSTEPS " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_STEPS " .
           " ON NH_STEPS.id = TCSTEPS.id " . 
  ...
  function get_step_numbers($tcversion_id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* $debugMsg */ " . 
           " SELECT TCSTEPS.id, TCSTEPS.step_number FROM ".$this->db->get_table('tcsteps')." TCSTEPS " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_STEPS " .
           " ON NH_STEPS.id = TCSTEPS.id " . 
  ...
  function get_latest_step_number($tcversion_id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* $debugMsg */ " . 
           " SELECT MAX(TCSTEPS.step_number) AS max_step FROM ".$this->db->get_table('tcsteps')." TCSTEPS " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_STEPS " .
           " ON NH_STEPS.id = TCSTEPS.id " . 
  ...
  function delete_step_by_id($step_id)
  {
    ...
    $sqlSet[] = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('tcsteps')." {$whereClause} ";
    $sqlSet[] = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " .
  ...
  function set_step_number($step_number)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
        ...
        {
          $sql = "/* $debugMsg */ UPDATE ".$this->db->get_table('tcsteps')." " . 
  ...
  function get_last_active_version($id,$filters=null,$options=null)
  {
    ...
    $sql = "/* $debugMsg */ " .       
           ...
           " FROM ".$this->db->get_table('tcversions')." TCV " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCVERSION " .
    ...
    if( !is_null($recordset) )
    {
      ...
      if( !is_null($my['filters']['cfields']) )
      {
        ...
        $cfJoin = " JOIN ".$this->db->get_table('cfield_design_values')." CFDV ON CFDV.node_id = TCV.id ";
        $cfQuery = " AND ({$cfQuery}) ";
      }
  ...
  function update_tcversion_steps($tcversion_id,$steps)
  {
    ...
    #checked step is update
    $debug = true;
    if($debug){
       
      $template_sql = "select count(1) "
                        ." from ".$this->db->get_table('tcsteps')
                        ." where id=%d and step_number=%d "
                        ." and (actions <> '%s' or expected_results <> '%s' "
                        ." or execution_type <> %d)";
      for($idx=0; $idx < $loop2do; $idx++){
        $sql = sprintf($template_sql, 
                       $tcversion_id,
                       $steps[$idx]['step_number'],
                       $this->db->prepare_string($steps[$idx]['actions']),
                       $this->db->prepare_string($steps[$idx]['expected_results']),
                       $steps[$idx]['execution_type']);
        
        $rs = $this->db->exec_query($sql);
        //echo $sql.':'."<br/>";
        //echo time().':';var_dump($rs);echo "<br/>";
        if($rs){          
         //file_put_contents("D:\\jim\\test.log", "test.".time().":".$sql."\r\n", FILE_APPEND);
          $this->testCaseStepAndTitleChangeToLog($tcversion_id);
          break;
        }
      }
  ...
  function update_last_modified($tcversion_id,$user_id,$time_stamp=null)
  {
    ...
    $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
  ...
  function filter_tcversions_by_cfields($tcversion_id,$cf_hash,$options=null)
  {
    ...
    $sql = "/* $debugMsg */ " .       
         ...
         " FROM ".$this->db->get_table('tcversions')." TCV " .
         " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCVERSION ON NH_TCVERSION.id = TCV.id " .
         " JOIN ".$this->db->get_table('cfield_design_values')." CFDV ON CFDV.node_id = TCV.id " .
  ...
  function getExecutionSet($id,$version_id=null,$filters=null,$options=null)
  {
    ...
    //modify by zhouzhaoxin on 20161229 for change assign to build
    $sql = "/* $debugMsg */ SELECT NHTC.name,NHTCV.parent_id AS testcase_id, NHTCV.id AS tcversion_id, " .
       ...
       " /* To get build name */ " .
       " JOIN ".$this->db->get_table('builds')." B ON B.id=E.build_id " .

       " /* To get test plan name */ " .
       // " JOIN {$this->tables['testplans']} TPLAN ON TPLAN.id = E.testplan_id " .
       " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTPLAN ON NHTPLAN.id = E.testplan_id " .

       " JOIN ".$this->db->get_table('testplan_tcversions')." TPTCV " .
       " ON  TPTCV.build_id = E.build_id " .
       " AND TPTCV.tcversion_id = E.tcversion_id " .
       " AND TPTCV.platform_id = E.platform_id " .
       " LEFT OUTER JOIN ".$this->db->get_table('users')." U ON U.id = E.tester_id " .
       " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLATF ON PLATF.id = E.platform_id  " .
       $where_clause .
  ...
  function getExecutedPlatforms($id,$filters=null,$options=null)
  {
    ...
    $sql = "/* $debugMsg */ SELECT DISTINCT e.platform_id,p.name " .
         " FROM ".$this->db->get_table('nodes_hierarchy')." NHTCV " . 
           " JOIN ".$this->db->get_table('tcversions')." tcversions ON NHTCV.id = tcversions.id " .
           " JOIN ".$this->db->get_table('executions')." e ON NHTCV.id = e.tcversion_id " .
...
//         "WHERE TPROJ.id = {$tproject_id}";
      $sql = "/* $debugMsg */ SELECT ".
          ...
          "TCV.creation_ts, TCV.modification_ts, TPROJ.prefix, U.first  AS first_name,".
          "U.last AS last_name, U.login, (TPTCV.urgency * TCV.importance) AS priority " .
          "FROM ".$this->db->get_table('testprojects')." TPROJ, ".$this->db->get_table('users')." U JOIN ".$this->db->get_table('tcversions')." TCV ON U.id = TCV.author_id " .
          "JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCV ON TCV.id = NHTCV.id " .
          "JOIN ".$this->db->get_table('nodes_hierarchy')." NHTC ON NHTCV.parent_id = NHTC.id " .
          "LEFT OUTER JOIN ".$this->db->get_table('testplan_tcversions')." TPTCV ON TCV.id = TPTCV.tcversion_id " .
          "LEFT OUTER JOIN ".$this->db->get_table('testplans')." TPLAN ON TPTCV.testplan_id = TPLAN.id " .
          "LEFT OUTER JOIN ".$this->db->get_table('testprojects')." TPROJ_TPLAN ON TPLAN.testproject_id = TPROJ_TPLAN.id " .
          "WHERE TPROJ.id = {$tproject_id}";
  ...
  function setExecutionType($tcversionID,$value)
  {
    ...
    $sql = "/* $debugMsg */ " . 
           " UPDATE ".$this->db->get_table('tcversions')." " .
   ...
   * history:2016.6.7
   * **/
  function settc_id($tcversionID,$value)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $safe = trim($value);
      $safe=(is_null($safe) || $safe == '') ? 'NULL' : $safe;
      $tcversionID=$this->db->prepare_int($tcversionID);
      
      $sql="/* $debugMsg */ " ." UPDATE ".$this->db->get_table('tcversions')." " .
      " SET tc_id='$safe' WHERE id ='$tcversionID'" ;
      $this->db->exec_query($sql);
      return array($value,$safe,$sql);
  }
   ...
   * history:2016.6.8
   * **/
  function setComplexity($tcversionID,$value)
  {
      $complexity=$this->db->prepare_int($value);
      $tcversionID=$this->db->prepare_int($tcversionID);
      
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
      " SET complexity='$complexity' WHERE id ='$tcversionID'";
      $this->db->exec_query($sql);
  }
   ...
   * history:2016.6.8
   * **/
  function setReviewed_status($tcversionID,$value)
  {
      $reviewed_status=$this->db->prepare_int($value);
      $tcversionID=$this->db->prepare_int($tcversionID);
  
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
      " SET reviewed_status='$reviewed_status' WHERE id ='$tcversionID'";
      $this->db->exec_query($sql);
  }
  /**
   * author:liuchunping
   * history:2016.6.8
   * **/
  function setReviewer_id($tcversionID,$value)
  {
      $reviewer_id=$this->db->prepare_int($value);
      $tcversionID=$this->db->prepare_int($tcversionID);
  
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
      " SET reviewer_id='$reviewer_id' WHERE id ='$tcversionID'";
      $this->db->exec_query($sql);
  }
   ...
   * history:2016.6.8
   * **/
  /* function setAuthor_id($tcversionID,$value)
  {
      $author_id=$this->db->prepare_int($value);
      $tcversionID=$this->db->prepare_int($tcversionID);
  
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
      " SET author_id='$author_id' WHERE id ='$tcversionID'";
      $this->db->exec_query($sql);
  } */
   ...
   * history:2016.6.8
   * **/
  function setBpm_id($tcversionID,$value)
  {
      $tcversionID=$this->db->prepare_int($tcversionID);
      $safe = trim($value);
      $safe=(is_null($safe) || $safe == '') ? 'NULL' : $safe;
      ...
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
      " SET bpm_id='$safe' WHERE id ='$tcversionID'";
      $this->db->exec_query($sql);
  }
   ...
   * history:2016.6.8
   * **/
  /* function setCreation_ts($tcversionID,$value)
  {
      $creation_ts=$value;
      $tcversionID=$this->db->prepare_int($tcversionID);
      ...
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
      " SET creation_ts='$creation_ts' WHERE id ='$tcversionID'";
      $this->db->exec_query($sql);
  } */
  ...
  function getLatestExecSingleContext($identity,$execContext,$options=null)
  {
    ...
    {                  
      $addJoinLEX = " JOIN ".$this->db->get_table('nodes_hierarchy')." H2O " .
    ...
    switch($my['opt']['output'])
    {
      ...
      default:
          $sql= "/* $debugMsg */ SELECT E.id AS execution_id, " .
                ...
                " JOIN ".$this->db->get_table('builds')." B ON B.id = {$safeContext['build_id']} " .
                " LEFT OUTER JOIN ".$this->db->get_table('users')." U ON U.id = E.tester_id " .
                " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLATF ON PLATF.id = {$safeContext['platform_id']} " .
                " WHERE TPTCV.testplan_id = {$safeContext['tplan_id']} " .
                " AND TPTCV.platform_id = {$safeContext['platform_id']} " .
                $addWhere .
                " AND (E.build_id = {$safeContext['build_id']} OR E.build_id IS NULL)";
    ...
    switch($my['opt']['output'])
    {
      ...
      default:
        $sql= "/* $debugMsg */ SELECT E.id AS execution_id, " .
              ...
              " JOIN ".$this->db->get_table('builds')." B ON B.id = E.build_id " .
              " JOIN ".$this->db->get_table('users')." U ON U.id = E.tester_id " .
                 
              " /* Left outer on Platforms because Test plan can have NO PLATFORMS */ " .
              " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLATF " .
  ...
  function getIdCardByStepID($step_id)
  {
    ...
    $sql = "/* $debugMsg */ " . 
           ...
           " FROM ".$this->db->get_table('nodes_hierarchy')." NH_STEPS " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.id = NH_STEPS.parent_id " .
  ...
  {   
    ...
    $goo->complexity = $this->complexity;
    $goo->reviewed_status = $this->reviewed_status;
    $goo->reviewer_name = $guiObj->reviewer_name;
  ...
  function getStepsExecInfo($execution_id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* {$debugMsg} */ " . 
           " SELECT id, execution_id,tcstep_id,notes,status FROM ".$this->db->get_table('execution_tcsteps')." " .
  ...
  public function getRelations($id) 
  {
    ...
    $sql = " $debugMsg SELECT id, source_id, destination_id, relation_type, author_id, creation_ts " . 
           " FROM ".$this->db->get_table('testcase_relations')." " .
  ...
  public function deleteAllRelations($id) 
  {
    ...
    $sql = " $debugMsg DELETE FROM ".$this->db->get_table('testcase_relations')." " . 
  ...
  public function addRelation($source_id, $destination_id, $type_id, $author_id, $ts=null) 
  {
    ...
    if( !$this->relationExits($source_id, $destination_id, $type_id) )
    {
      ...
      $sql = " $debugMsg INSERT INTO ".$this->db->get_table('testcase_relations')." "  . 
  ...
  {
    $debugMsg = "/* {$this->debugMsg}" . __FUNCTION__ . ' */';
    $sql = " $debugMsg DELETE FROM ".$this->db->get_table('testcase_relations')." WHERE id=" . intval($relID);
    $this->db->exec_query($sql);
  }
...
//    $children =
    ...
    $sql = " UPDATE ".$this->db->get_table('tcversions')." " .
           ...
           " WHERE id IN (" . 
           "  SELECT NHTCV.id FROM ".$this->db->get_table('nodes_hierarchy')." NHTCV " .
  ...
  /**
   *new add test case step and title change to log
   * 
   */
  function testCaseStepAndTitleChangeToLog($tcversion_id = 0){
    /*
      var_dump($tcase_id);
      echo 'test case step and title change log ...';
      die;
      */
      $re_status = false;
      ...
      if(isset($_SESSION['jim_case_title_step_change'])){
        unset($_SESSION['jim_case_title_step_change']);
      }
  ...
  /**
   * author:jinjiacun
   * time:2017-12-31 17:46
	  * */
  function getCaseExecStatByVersionIdList($tcversion_id_list){
    global $tlCfg;
	  if(!isset($tcversion_id_list)
		  || count($tcversion_id_list) <= 0){
		  return null;
	  }

	  $re_value 	= array();
	  $sql 		= '';
	  $sql_template = "select count(distinct(tcversion_id)) as count ,status,tester_id"
		  ." from ".$this->db->get_table("executions")
		  ." where tcversion_id in (%s) "
      ." group by tester_id,status";
	  $sql = sprintf($sql_template, implode(",", $tcversion_id_list));
	  file_put_contents($tlCfg->cache_file['cache_dir']."CaseExecStat.sql", 
		  time().":".$sql."\r\n", FILE_APPEND);
	$re_value = $this->db->fetchRowsIntoMap($sql,"tester_id",1);
	return $re_value;
  }

  function getNotCaseExecCountByVersionIdList($tcversion_id_list){
	  if(!isset($tcversion_id_list)
		  || count($tcversion_id_list) <= 0){
		return null;
	  }
	  ...
	  $re_value 	= 0;
	  $sql		= "";
	  $sql_template = "select count(distinct(tcversion_id)) as count "
		  ." from ".$this->db->get_table('executions')
		  ." where tcversion_id in (%s) ";
	  $sql = sprintf($sql_template, implode(",", $tcversion_id_list));
	  $re_value = $this->db->fetchOneValue($sql);

    if($re_value){
      $re_value = count($tcversion_id_list) - $re_value;
    }
  ...
  /**
   * [getCaseCountNoExec description]
   * @param  [type] $tcversion_id_list [description]
   * @return [type]                    [description]
   *
   * author:jinjiacun
   * time:2018-01-03 16:40
   */
  function getCaseCountNoExec($tcversion_id_list, $testplan_id, $build_id, $assigner_id = 0){
	  $re_list = array();
	  if(count($tcversion_id_list) <= 0){
		return $re_list;
	  }

    #get assign tcversion group by assigner_id
    $sql_template = "select assigner_id,group_concat(distinct(tcversion_id)) as version_id_list "
	    ." from ".$this->db->get_table("user_assignments")
	    ." as ua inner join ".$this->db->get_table("testplan_tcversions")." as tt "
                 ." on ua.feature_id = tt.id"
                 ." %s "
                 ." group by assigner_id";
    $sql_where = " where testplan_id=%d and tt.build_id=%d and tcversion_id in (%s)";
    if($assigner_id != 0){
	$sql_where .= " and assigner_id = ".$assigner_id;
    }
    $sql = sprintf($sql_template, sprintf($sql_where, $testplan_id, $build_id, implode(",", $tcversion_id_list)));
    unset($sql_template, $sql_where);
#    var_dump($sql);die;
    $assigner_id_version_map = $this->db->fetchColumnsIntoMap($sql, "assigner_id", "version_id_list");

    #get exec tcversion group by tester_id
    $sql_template = "select tester_id, GROUP_CONCAT(distinct(tcversion_id)) as version_id_list"
		   ." from ".$this->db->get_table("executions")
		   ." %s "
		   ." group by tester_id;";
    $sql_where = " where testplan_id = %d and build_id = %d and id in (%s)";
    if($assigner_id != 0){
    	$sql_where .= " and tester_id = ".$assigner_id;
    }
    $sql = sprintf($sql_template, sprintf($sql_where, $testplan_id, $build_id, implode(",", $tcversion_id_list)));
    $execer_id_version_map = $this->db->fetchColumnsIntoMap($sql, "tester_id", "version_id_list");

     #calcutlate from assign_list some exec in it
    if(count($assigner_id_version_map) > 0
    && count($execer_id_version_map) > 0){
    foreach($assigner_id_version_map as $user_id => $version_id_list_str){
	    if(isset($execer_id_version_map[$user_id])){
		    $assign_array 	= explode(",", $version_id_list_str);
		    $exec_array   	= explode(",", $execer_id_version_map[$user_id]);
		    $intersect_array   	= array_intersect($assign_array, $exec_array);
		    $diff_array         = array_diff($assign_array, $intersect_array);
		    unset($assign_array, $exec_array, $intersect_array);
		    echo 'test';
		    		    var_dump($diff_array);echo "<br/><br/>";
		    $re_list[$user_id] = count($diff_array);
		    unset($diff_array);
	    }
    }
    }
  ...
  function get_modules($proejct_id){
    $re_list = array();

    $sql_template = "select id,name "
                   ." from ".$this->db->get_table('nodes_hierarchy')
                   ." where parent_id = %d and node_type_id = 2 ";
    $sql = sprintf($sql_template, $proejct_id);
    $re_list = $this->db->fetchColumnsIntoMap($sql, "id", "name");
