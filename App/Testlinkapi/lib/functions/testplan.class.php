...
class testplan extends tlObjectWithAttachments
{
  ...
  function create($name,$notes,$testproject_id,$is_active=1,$is_public=1)
  {
    ...
    $sql = "/* $debugMsg */ " . 
           " INSERT INTO ".$this->db->get_table('testplans')." (id,notes,api_key,testproject_id,active,is_public) " .
  ...
  function updateFromObject($item,$opt=null)
  {
    ...
    try 
    {
      ...
      if( property_exists($item, 'name') )
      {
        ...
        $sql = "/* $debugMsg */ " .
               " UPDATE ".$this->db->get_table('nodes_hierarchy')." " .
      ...
      if($upd != '')
      {
        $sql = " UPDATE ".$this->db->get_table('testplans')." " .
  ...
  function update($id,$name,$notes,$is_active=null,$is_public=null)
  {
    ...
    if($do_update)
    {
      // Update name
      $sql = "/* $debugMsg */ ";
      $sql .= "UPDATE ".$this->db->get_table('nodes_hierarchy')." " .
              "SET name='" . $this->db->prepare_string($name) . "'" .
              "WHERE id={$id}";
      $result = $this->db->exec_query($sql);
...
//             " {$this->tables['nodes_hierarchy']} NH" .
            $sql .= " FROM ".$this->db->get_table('testplans')." testplans, " .
...
//          " {$this->tables['nodes_hierarchy']} NHB " .
         " FROM ".$this->db->get_table('tcversions')." TCV,".$this->db->get_table('nodes_hierarchy')." NHA, " .
         " ".$this->db->get_table('nodes_hierarchy')." NHB " .
         " WHERE NHA.id=TCV.id " .
   ...
   * this is the way to populate a test plan
    ...
    rev: 20080629 - franciscom - audit message improvements
  */
  function link_tcversions($id,&$items_to_link,$userId,$build_id = 0)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    ...
    $sql = "/* $debugMsg */ " .
           "INSERT INTO ".$this->db->get_table('testplan_tcversions')." " .
         "(testplan_id,author_id,creation_ts,tcversion_id,platform_id,build_id) " . 
    ...
    foreach($items_to_link['items'] as $tcase_id => $items)
    {
      ...
      {
        $addInfo='';
        $result = $this->db->exec_query($sql . "{$tcversion}, {$platform_id}, {$build_id})");
        if ($result)
        {
...
//          " JOIN {$this->tables['testplan_tcversions']} TPTCV ON TPTCV.tcversion_id = NHTC.id " .
         " SELECT DISTINCT parent_id FROM ".$this->db->get_table('nodes_hierarchy')." NHTC " .
         " JOIN ".$this->db->get_table('testplan_tcversions')." TPTCV ON TPTCV.tcversion_id = NHTC.id " .
...
//            " FROM {$this->tables['testplan_tcversions']} ";
              " FROM ".$this->db->get_table('testplan_tcversions')." ";

    if($options['addEstimatedExecDuration'])
    {
//       $sql .= " JOIN {$this->tables['tcversions']} TCV ON TCV.id = tcversion_id ";
      $sql .= " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.id = tcversion_id ";
    }  
...
//          " SELECT id FROM {$this->tables['testplan_tcversions']} " .
         " SELECT id FROM ".$this->db->get_table('testplan_tcversions')." " .
...
//          " WHERE TPTCV.testplan_id = {$id} ";
    $sql = " /* $debugMsg */ ".
        ...
        " ON TPTCV.tcversion_id = NHTCV.id " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCASE " .
        " ON NHTCASE.id = NHTCV.parent_id " .
        " WHERE TPTCV.testplan_id = {$id} ";
...
//           " FROM {$this->tables['nodes_hierarchy']} " . 
  ...
  function helper_keywords_sql($filter,$options=null)
  {
    ...
    if( $sql['filter'] != '' )
    {
      $sql['join'] = " JOIN ".$this->db->get_table('testcase_keywords')." TK ON TK.testcase_id = NH_TCV.parent_id ";
    }
  ...
  {  
    ...
    $join = " JOIN ".$this->db->get_table('user_assignments')." UA " .
  ...
  function helper_bugs_sql($filter)
  {
    ...
    if(!is_null($items))
    {
      ...
      $sql['join'] = " JOIN ".$this->db->get_table('execution_bugs')." EB ON EB.execution_id = E.id ";
    }  
  ...
  */
  function get_linked_and_newest_tcversions($id,$tcase_id=null)
  {
    ...
    $sql = " /* $debugMsg */ SELECT MAX(NHB.id) AS newest_tcversion_id, " .
           ...
           " FROM ".$this->db->get_table('nodes_hierarchy')." NHA " .
        
           // NHA - will contain ONLY nodes of type testcase_version that are LINKED to test plan
           " JOIN ".$this->db->get_table('testplan_tcversions')." T ON NHA.id = T.tcversion_id " . 
        
           // Get testcase_version data for LINKED VERSIONS
           " JOIN ".$this->db->get_table('tcversions')." TCVA ON TCVA.id = T.tcversion_id" .
        
           // Work on Sibblings - Start
           // NHB - Needed to get ALL testcase_version sibblings nodes
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHB ON NHB.parent_id = NHA.parent_id " .
        
           // Want only ACTIVE Sibblings
           " JOIN ".$this->db->get_table('tcversions')." TCVB ON TCVB.id = NHB.id AND TCVB.active=1 " . 
           ...
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHC ON NHC.id = NHA.parent_id " .
...
//             " FROM {$this->tables['tcversions']} TCV, ( $sql ) AS SUBQ " .
            " FROM ".$this->db->get_table('tcversions')." TCV, ( $sql ) AS SUBQ " .
   ...
   *    20160613 - zhouzhaoxin - testcase assign to build, so unlink only to build
   */
  function unlink_tcversions($id,&$items,$build_id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    if(is_null($items))
    {
...
//     $sql = " SELECT id AS execution_id FROM {$this->tables['executions']} " .
    $sql = " SELECT id AS execution_id FROM ".$this->db->get_table('executions')." " .
           " WHERE testplan_id = {$id} and build_id = {$build_id} AND ${where_clause}";
    ...
    if( !is_null($exec_ids) and count($exec_ids) > 0 )
    {
      ...
      $sql=" DELETE FROM ".$this->db->get_table('execution_bugs')." {$exec_id_where} ";
      $result = $this->db->exec_query($sql);
      
      // now remove executions
      $sql=" DELETE FROM ".$this->db->get_table('executions')." " .
         " WHERE testplan_id = {$id} and build_id = {$build_id} AND ${where_clause}";
      $result = $this->db->exec_query($sql);
    }
...
//     $sql=" SELECT id AS link_id FROM {$this->tables['testplan_tcversions']} " .
    $sql=" SELECT id AS link_id FROM ".$this->db->get_table('testplan_tcversions')." " .
       " WHERE testplan_id={$id} and build_id = {$build_id} AND {$where_clause} ";
    ...
    $sql=" DELETE FROM ".$this->db->get_table('testplan_tcversions')." " .
       " WHERE testplan_id={$id} and build_id = {$build_id} AND {$where_clause} ";
    $result = $this->db->exec_query($sql);
  ...
  function get_keywords_map_for_project($id,$order_by_clause='')
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $map_keywords=null;
      ...
      //
      $sql = " /* $debugMsg */ " .
          " SELECT KW.id as keyword_id,KW.keyword FROM " .
	      $this->db->get_table('keywords') .  " KW " .
          " where testproject_id = " . $id . " " .
          $order_by_clause;
...
//          " FROM {$this->tables['keywords']} KW " .
         " FROM ".$this->db->get_table('keywords')." KW " .
         " JOIN ( " .
...
//          "     JOIN {$this->tables['testcase_keywords']} TCKW " .
         "     JOIN ".$this->db->get_table('testcase_keywords')." TCKW " .
...
//         {$this->tables['keywords']} keywords
        FROM ".$this->db->get_table('testcase_keywords')." testcase_keywords,
        ".$this->db->get_table('keywords')." keywords
  ...
  private function copy_builds($id,$new_tplan_id)
  {
    ...
    if(!is_null($rs))
    {
      foreach($rs as $build)
      {
        ...
        $sql = " /* $debugMsg */ INSERT INTO ".$this->db->get_table('builds')." " .
               " ({$fields}) " .
...
//               " LEFT OUTER JOIN {$this->tables['user_assignments']} UA ON " .
              " FROM ".$this->db->get_table('testplan_tcversions')." TPTCV " .
              " LEFT OUTER JOIN ".$this->db->get_table('user_assignments')." UA ON " .
              " UA.feature_id = TPTCV.id " .
              " WHERE testplan_id={$id} ";
    }
    else
    {
      $sql .= " SELECT TPTCV.* FROM ".$this->db->get_table('testplan_tcversions')." TPTCV" .
              " WHERE testplan_id={$id} ";
      }
...
//           $sql="/* $debugMsg */ SELECT * FROM {$this->tables['nodes_hierarchy']} WHERE id={$tcversion_id} ";
          $sql="/* $debugMsg */ SELECT * FROM ".$this->db->get_table('nodes_hierarchy')." WHERE id={$tcversion_id} ";
  ...
  private function copy_milestones($tplan_id,$new_tplan_id)
  {
    ...
    if(!is_null($rs))
    {
      foreach($rs as $mstone)
      {
        ...
        $sql = "INSERT INTO ".$this->db->get_table('milestones')." (name,a,b,c,target_date,{$add2fields} testplan_id)";        
...
//     $sql = "/* $debugMsg */ SELECT user_id,role_id FROM {$this->tables['user_testplan_roles']} " .
    $sql = "/* $debugMsg */ SELECT user_id,role_id FROM ".$this->db->get_table('user_testplan_roles')." " .
    ...
    if(!is_null($rs))
    {
        foreach($rs as $elem)
        {
            $sql="INSERT INTO ".$this->db->get_table('user_testplan_roles')."  " .
...
//     $sql = " /* $debugMsg */ SELECT user_id,role_id FROM {$this->tables['user_testplan_roles']} " .
  ...
  function addUserRole($userID,$id,$roleID)
  {
    ...
    $status = tl::ERROR;
    $sql = " /* $debugMsg */ INSERT INTO ".$this->db->get_table('user_testplan_roles')." (user_id,testplan_id,role_id) VALUES " .
         " ({$userID},{$id},{$roleID})";
  ...
  function deleteUserRoles($id,$users=null,$opt=null)
  {
    ...
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $status = tl::ERROR;
    $sql = " /* $debugMsg */ DELETE FROM ".$this->db->get_table('user_testplan_roles')." " .
           " WHERE testplan_id = " . intval($id);
    if(!is_null($users))
    {
  ...
  function delete($id)
  {
    ...
    $getFeaturesSQL = " /* $debugMsg */ SELECT id FROM ".$this->db->get_table('testplan_tcversions')." WHERE testplan_id={$id} "; 
    $the_sql[]="DELETE FROM ".$this->db->get_table('milestones')." WHERE testplan_id={$id}";
    
    // CF used on testplan_design are linked by testplan_tcversions.id
    $the_sql[]="DELETE FROM ".$this->db->get_table('cfield_testplan_design_values')." WHERE link_id ".
             "IN ({$getFeaturesSQL})";

    $the_sql[]="DELETE FROM ".$this->db->get_table('user_assignments')." WHERE feature_id ".
             "IN ({$getFeaturesSQL})";
    
    $the_sql[]="DELETE FROM ".$this->db->get_table('testplan_platforms')." WHERE testplan_id={$id}";

    $the_sql[]="DELETE FROM ".$this->db->get_table('testplan_tcversions')." WHERE testplan_id={$id}";

    $the_sql[]="DELETE FROM ".$this->db->get_table('cfield_execution_values')." WHERE testplan_id={$id}";
    $the_sql[]="DELETE FROM ".$this->db->get_table('user_testplan_roles')." WHERE testplan_id={$id}";
    
    
    // When deleting from executions, we need to clean related tables
    $the_sql[]="DELETE FROM ".$this->db->get_table('execution_bugs')." WHERE execution_id ".
           "IN (SELECT id FROM ".$this->db->get_table('executions')." WHERE testplan_id={$id})";
    $the_sql[]="DELETE FROM ".$this->db->get_table('executions')." WHERE testplan_id={$id}";
    $the_sql[]="DELETE FROM ".$this->db->get_table($this->tables['builds'])." WHERE testplan_id={$id}"; //BUGID 3845    
    ...
    $main_sql[]="DELETE FROM ".$this->db->get_table('testplans')." WHERE id={$id}";
    $main_sql[]="DELETE FROM ".$this->db->get_table('nodes_hierarchy')." " . 
...
//            " FROM {$this->tables['builds']} WHERE testplan_id = {$id} ";
              " FROM ".$this->db->get_table('builds')." WHERE testplan_id = {$id} ";
...
//            "FROM {$this->tables['nodes_hierarchy']}  NH " .
  ...
  */
  function get_builds($id,$active=null,$open=null,$opt=null)
  {
    ...
    $my['opt'] = array('fields'   => 'id,            testplan_id, name,       notes, 
                                      active,        is_open,     release_date,
...
//       " FROM {$this->tables['builds']} BUILDS " .
      " FROM ".$this->db->get_table('builds')." BUILDS " .
...
//       " FROM {$this->tables['builds']} builds " .
  ...
  */
  function create_build($tplan_id,$name,$notes = '',$active=1,$open=1)
  {
    ...
    $sql = " /* $debugMsg */ INSERT INTO ".$this->db->get_table('builds')." (testplan_id,name,notes,active,is_open) " .
           " VALUES ('". $tplan_id . "','" .
...
//       {$this->tables['custom_fields']} custom_fields
      FROM ".$this->db->get_table('cfield_testprojects')." cfield_testprojects, 
      ".$this->db->get_table('custom_fields')." custom_fields
      WHERE
...
//         $sql = " /* $debugMsg */ SELECT CFD.value FROM {$this->tables['cfield_design_values']} CFD," .
        $sql = " /* $debugMsg */ SELECT CFD.value FROM ".$this->db->get_table('cfield_design_values')." CFD," .
               " ".$this->db->get_table('nodes_hierarchy')." NH" .
...
//                       " JOIN {$this->tables['tcversions']} TCV ON TCV.id = tcversion_id " .
                      " FROM ".$this->db->get_table('testplan_tcversions')." " .
                      " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.id = tcversion_id " .
                      " WHERE testplan_id=" . intval($id)  .
...
//       $sql .= " AS SUM_VALUE FROM {$this->tables['cfield_design_values']} CFDV " .
      $sql .= " AS SUM_VALUE FROM ".$this->db->get_table('cfield_design_values')." CFDV " .
...
//       $sql .= " AS SUM_VALUE FROM {$this->tables['cfield_execution_values']} CFEV " .
      $sql .= " AS SUM_VALUE FROM ".$this->db->get_table('cfield_execution_values')." CFEV " .
...
//            " JOIN {$this->tables['nodes_hierarchy']}  NH ON T.tcversion_id=NH.id " .
           " FROM ".$this->db->get_table('testplan_tcversions')."  T " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')."  NH ON T.tcversion_id=NH.id " .
           " AND NH.node_type_id={$node_types['testcase_version']} " .
//            " LEFT OUTER JOIN {$this->tables['executions']} E ON T.tcversion_id = E.tcversion_id " .
           " LEFT OUTER JOIN ".$this->db->get_table('executions')." E ON T.tcversion_id = E.tcversion_id " .
    ...
    $sql = " SELECT EE.status,SQ1.tcversion_id, NH.parent_id AS tcase_id, COUNT(EE.status) AS exec_qty " .
         " FROM ".$this->db->get_table('executions')." EE, ".$this->db->get_table('nodes_hierarchy')." NH," .
         " (SELECT E.tcversion_id,E.build_id,MAX(E.id) AS last_exec_id " .
//          " FROM {$this->tables['executions']} E " .
         " FROM ".$this->db->get_table('executions')." E " .
...
//            " LEFT OUTER JOIN {$this->tables['executions']} E ON T.tcversion_id = E.tcversion_id " .
           " FROM ".$this->db->get_table('testplan_tcversions')." T " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NH ON T.tcversion_id=NH.id  AND NH.node_type_id=4 " .
           " LEFT OUTER JOIN ".$this->db->get_table('executions')." E ON T.tcversion_id = E.tcversion_id " .
   ...
   *
    ...
    */
    function changeLinkedTCVersionsPlatform($id,$from,$to,$tcversionSet=null)
    {
      ...
      $sqlStm = array();
      $sqlStm[] = "/* {$debugMsg} */ " . 
                  " UPDATE ".$this->db->get_table('testplan_tcversions')." " .
                  " SET platform_id = {$to} " . $whereClause;

      $sqlStm[] = "/* {$debugMsg} */" .
                  " UPDATE ".$this->db->get_table('executions')." " .
                  " SET platform_id = {$to} " . $whereClause;
...
//            " JOIN {$this->tables['testplan_tcversions']} TPTCVX " . 
           " from ".$this->db->get_table('testplan_tcversions')." TPTCVMAIN " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCV ON NHTCV.id = TPTCVMAIN.tcversion_id " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTC ON NHTC.id = NHTCV.parent_id " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTSET ON NHTSET.parent_id = NHTC.parent_id " .
           " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCVSET ON NHTCVSET.parent_id = NHTSET.id " .
           " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.id = NHTCVSET.id " .
           " JOIN ".$this->db->get_table('testplan_tcversions')." TPTCVX " .
           " ON TPTCVX.tcversion_id = NHTCVSET.id " .
           " AND TPTCVX.build_id = TPTCVMAIN.build_id " .
...
/* deleted by zhouzhaoxin 20171206 for performance                 
                 " JOIN ".$this->db->get_table('user_assignments')." UAMAIN " .
                 " ON UAMAIN.feature_id = TPTCVMAIN.id " . 
                 " AND UAMAIN.build_id = " . $build_id . 
                 " AND UAMAIN.user_id = " . $user_id . 
                 " AND UAMAIN.type = {$this->execTaskCode} " .
*/
                 " JOIN ".$this->db->get_table('user_assignments')." UAX " .
    ...
    $sql .= " WHERE TPTCVMAIN.build_id = {$id} AND TPTCVMAIN.tcversion_id = {$tcversion_id} " .
...
//         " LEFT OUTER JOIN {$this->tables['user_assignments']} UA " .
        " FROM ".$this->db->get_table('testplan_tcversions')." TPTCV " .
        " JOIN ".$this->db->get_table('builds')." B ON B.testplan_id = TPTCV.testplan_id " .
  ...
  /**
   * getAllTestsuitesByDepth
   *
   * get all testsuite info 
   *
   * Development Note:
   * used by execTree funciton in execTreeMenu.inc.php 
   * due to the need to improve performace of load exec tree
   *
   * @return testsuite info array order by id
   *
   * @since 20170604 add by zhouzhaoxin
   *
   */
  function getAllTestsuitesByDepth($project_id, $filters)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      
      $sql = " /* $debugMsg - Get TestSuites One By One Depth */ " .
      ...
      " NHTS.node_depth_abs as node_abs " .
      " FROM " . $this->db->get_table('nodes_hierarchy') . " NHTS " .
      " WHERE NHTS.node_type_id = {$this->tree_manager->node_descr_id['testsuite']} " .
      " ORDER BY NHTS.node_order, NHTS.id";
      $rs = $this->db->fetchRowsIntoMap($sql,'id');
      
      // first foreach to division level
      $tree_by_level = array();
      foreach ($rs as $row)
      {
          $node_abs = $row['node_abs'];
          $depth = strlen($node_abs) / TL_NODE_ABS_LENGTH;
          if (!array_key_exists($depth, $tree_by_level))
          {
              $tree_by_level[$depth] = array();
          }
          ...
          $tree_by_level[$depth][$row['id']] = $node;
      }
      
      // filter by top level testsuite
      if (isset($filters['exclude_branches']) && is_array($filters['exclude_branches'])
          && isset($tree_by_level[1]))
      {
          foreach ($tree_by_level[1] as $tsuite_id => $tsuite_info)
          {
              if (array_key_exists($tsuite_id, $filters['exclude_branches']))
              {
                  unset($tree_by_level[1][$tsuite_id]);
              }
          }
      }
   ...
   *
   */
  function getSkeleton($id,$build_id,$tprojectID,$filters=null,$options=null)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $items = array();
    ...
    //modify by zhouzhaoxin 20161212 to disable recuision to lift performance
    //$method2call = $my['options']['recursive'] ? '_get_subtree_abs' : '_get_subtree';
    $method2call = '_get_subtree_abs';
    $qnum = $this->$method2call($id, $build_id, $tprojectID,$items,$my['filters'],$my['options']);
    return $items;
  }
  
  /**
     * get subtree by abs not rec , faster
     * @param :  $tplan_id => testplan id
     * @param :  $build_id => build id
     * @param :  $node_id => parent node id
     * @param :  &$pnode => return node list
     ...
     * **/
  function _get_subtree_abs($tplan_id, $build_id, $node_id, &$pnode, $filters = null, $options = null)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $qnum=0;
      ...
      $staticSql[1] =  " /* $debugMsg - Get ONLY Test Cases with version linked to (build,platform) */ " .
          ...
          " NHTC.node_depth_abs as node_abs " . 
          " FROM " . $this->db->get_table('nodes_hierarchy') . " NHTC " .
          " JOIN " . $this->db->get_table('nodes_hierarchy') . " NHTCV ON NHTCV.parent_id = NHTC.id " . 
          " JOIN " . $this->db->get_table('testplan_tcversions') . " TPTCV ON TPTCV.tcversion_id = NHTCV.id " .
          " WHERE NHTC.node_type_id = {$this->tree_manager->node_descr_id['testcase']} ";
      
      if (isset($build_id) && $build_id != "" && $build_id > 0)
      {
          $staticSql[1] .= " AND TPTCV.build_id = " . $build_id . " {$platformFilter} ";
      }
      else 
      {
          if (isset($tplan_id) && $tplan_id != "" && $tplan_id > 0)
          {
              $staticSql[1] .= " AND TPTCV.testplan_id = " . $tplan_id . " ";
          }
          $staticSql[1] .= " {$platformFilter} ";
      }
      ...
      foreach ($rs as $row)
      {
          ...
          {
              $tree_by_level[$depth] = array();
              if ($min_level == 0 || $min_level > $depth)
              {
                  $min_level = $depth;
              }
              if ($max_level == 0 || $max_level < $depth)
              {
                  $max_level = $depth;
              }
          }
      ...
      // get child node for parent node
      for ($level = $max_level; $level > $min_level; $level--)
      {
          // min level not need to traverse to find parent
          foreach ($tree_by_level[$level] as $node_id => $node_info)
          {
              // not empty dic node can be insert to parent
              $doRemove = is_null($node_info['childNodes']) &&
                  ($node_info['node_type_id'] == $my['options']['remove_empty_nodes_of_type']);
              
              if (!$doRemove)
              {
                  if ($tree_by_level[$level - 1][$node_info['parent_id']]['childNodes'] == null)
                  {
                      $tree_by_level[$level - 1][$node_info['parent_id']]['childNodes'] = array();
                  }
                  $tree_by_level[$level - 1][$node_info['parent_id']]['childNodes'][] = $node_info;
              }
          }  
      }
      
      // add min level node to root node
      $pnode['childNodes'] = array();
      foreach ($tree_by_level[$min_level] as $node_id => $node_info)
      {
          // not empty dic node can be insert to parent
          $doRemove = is_null($node_info['childNodes']) &&
              ($node_info['node_type_id'] == $my['options']['remove_empty_nodes_of_type']);
   ...
   *
   */
  function _get_subtree_rec($tplan_id, $build_id,$node_id,&$pnode,$filters = null, $options = null)
  {
    static $qnum;
    static $my;
    ...
    if (!$my)
    {
      ...
      $buildFilter = "";
      if (array_key_exists('filter_build_id', $my['filters']) 
          && !is_null($my['filters']['filter_build_id']) && $my['filters']['filter_build_id'] > 0)
      {
          $buildFilter = " AND TPTCV.build_id = " . intval($my['filters']['filter_build_id']) ;
      }
...
//         " JOIN {$this->tables['nodes_hierarchy']} NHTCV ON " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCV ON " .
        " NHTCV.id = TPTCV.tcversion_id " .
//         " LEFT OUTER JOIN {$this->tables['executions']} E ON " .
        " LEFT OUTER JOIN ".$this->db->get_table('executions')." E ON " .
   ...
   * @internal revisions
   * @since 1.9.4
   * 20170804 modified by zhouzhaoxin for assigned by build
   */
  function getHitsNotRunForBuild($id, $buildID) 
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    // list($safe_id,$buildsCfg,$sqlLEBBP) = $this->helperGetHits($id,$platformID,$buildSet);
    ...
    $sql =   "/* $debugMsg */ " .
        "select tt.id as feature_id, tt.tcversion_id, tt.node_order, tt.urgency, tt.platform_id from " . 
        $this->db->get_table('testplan_tcversions') . " tt join " .
        $this->db->get_table('nodes_hierarchy') . " nh on tt.tcversion_id = nh.id " .
        " where tt.build_id = " . $safe_id['build'] . " and tt.testplan_id = " . $safe_id['tplan'] .
        " and tt.tcversion_id not in " . 
        " (select distinct tcversion_id from " . 
        $this->db->get_table('executions') . " where build_id = " . $safe_id['build'] . ")";
         
    $recordset = $this->db->fetchRowsIntoMap($sql,'feature_id');
    return $recordset;
  }
...
//         " LEFT OUTER JOIN {$this->tables['executions']} E " .
   ...
   * 20170804 modified by zhouzhaoxin for assigned by build
   */
  function getHitsSingleStatusFull($id, $status, $buildSet) 
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = " /* $debugMsg */ " .
        " select ees.feature_id, ee.tcversion_id, ees.node_order, ees.urgency, ees.platform_id  from " . 
        $this->db->get_table('executions') . " ee " .
        " inner join ( " .
        " select max(e.id) as id, tt1.id as feature_id, tt1.node_order, tt1.urgency, tt1.platform_id from " . 
        $this->db->get_table('executions') . " e join " . 
        $this->db->get_table('testplan_tcversions') . " tt1 " .
        " on e.tcversion_id = tt1.tcversion_id and e.build_id = tt1.build_id " .
        " where e.build_id = " . $buildSet .
        " group by tt1.tcversion_id " . 
        " ) ees on ee.id = ees.id join " . 
        $this->db->get_table('nodes_hierarchy') . " nh on ee.tcversion_id = nh.id " .
        " where ee.status = '" . $this->db->prepare_string($status) . "'";
...
//         " JOIN {$this->tables['nodes_hierarchy']} NHTCV ON NHTCV.id = TPTCV.tcversion_id " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCV ON NHTCV.id = TPTCV.tcversion_id " .
  ...
  function helperColumns($tplanID,&$filters,&$opt)
  {
    ...
    switch($opt['details'])
    {
      case 'full':
        ...
        $join['tsuite'] = " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TSUITE " . 
...
//       $join['ua'] = " LEFT OUTER JOIN {$this->tables['user_assignments']} UA ON " .
      $join['ua'] = " LEFT OUTER JOIN ".$this->db->get_table('user_assignments')." UA ON " .
              " UA.feature_id = TPTCV.id " .
...
//             " JOIN {$this->tables['builds']} BB " .
            " JOIN ".$this->db->get_table('builds')." BB " .
...
//                           " LEFT OUTER JOIN {$this->tables['executions']} E " .
                          ...
                          " WHERE TPTCV.build_id =" . $my['filters']['build_id'] .
                          $my['where']['not_run'] .
...
//                      " JOIN {$this->tables['executions']} E " .
   ...
   *                    
   * function getLinkedForExecTree use join between testplan_tcversion table and user_assignment table
   * if they both have many data , sql will be too slow
   * so user the new function without join them, but only get testplan_tcversion data, then filter them by app
   * @internal revisions  add by zhouzhaoxin 20170531 to improve the performance
   */
  
  function getLinkedForExecTreeWithoutUA($id,$filters=null,$options=null)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      ...
      $ic['filters'] = array_merge($ic['filters'], (array)$filters);
      $ic['options'] = array_merge($ic['options'], (array)$options);
      
      if (isset($ic['filters']['build_id']) && $ic['filters']['build_id'] > 0)
      {
      ...
      if($ic['options']['addTSuiteOrder'])
      {
          ...
          $ic['join']['tsuites'] = " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TSUITE " .
      ...
      if( isset($ic['filters']['cf_hash']) && !is_null($ic['filters']['cf_hash']) )
      {
          ...
          if(strlen(trim($cf_sql)) > 0)
          {
              ...
              $ic['join']['cf'] = " JOIN ".$this->db->get_table('cfield_testplan_design_values')." CFTPD " .
      ...
      if (isset($ic['filters']['filtersingletc']) && $ic['filters']['filtersingletc'] == 1)
      {
          // add where clause to select testcases with muti tcversions
          //         $ic['where']['where'] .= " and exists (select 1 from nodes_hierarchy nh2 " .
          $ic['where']['where'] .= " and exists (select 1 from ".$this->db->get_table('nodes_hierarchy nh2')." " .
              " where nh2.node_type_id = 4  and NH_TCASE.id=nh2.parent_id GROUP BY parent_id HAVING count(*) > 1)";
      }
      ...
      //stitch sql with array
      //may be 5 sqls 
      //first sql is to get tcversion assigned to build and by own filters, with index 'assign'
      //second sql is to get tcversion executed on build and bug filters ,with index 'exec'
      //third sql is to get lasted tcversion executed on build ,with index 'exec_build'
      //fourth sql is to get feature assign to one or some users on build, with index 'ua'
      //fith sql is to get feature assigne to user on build, signle tcversion, with index 'ua_assigned'
      $link_sql['assign'] = "/* {$debugMsg} get assign */" .
      " SELECT NH_TCASE.id AS id, NH_TCASE.parent_id as parent_id, NH_TCASE.name as name, " . 
      " NH_TCASE.id AS tcase_id,NH_TCASE.parent_id as tsuite_id, " . 
      " TPTCV.tcversion_id,TCV.version," .
      " TPTCV.id as feature_id, TCV.tc_external_id AS external_id, " .
      " TPTCV.node_order AS exec_order, 0 as drop_info " .
      $ic['fields']['tsuites'] .
      " FROM ".$this->db->get_table('testplan_tcversions')." TPTCV " .
      " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.id = TPTCV.tcversion_id " .
      " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.id = TPTCV.tcversion_id " .
      ...
      if ($has_build)
      {
          $link_sql['assign'] .= " WHERE TPTCV.build_id =" . $ic['filters']['build_id'] . " ";
      }
      else 
      {
          $link_sql['assign'] .= " WHERE TPTCV.testplan_id =" . $safe['tplan_id'] . " ";
      }
      
      $link_sql['assign'] .= $ic['where']['where'] . " order by TPTCV.tcversion_id";
      
      // get execute info can not include not_run , so cannot filter by status , should do it at call level
      ...
      {
          $platform4EE = " and ee.platform_id = " . intval($ic['filters']['platform_id']);
      }
      
      if ($has_build)
      {
          $link_sql['exec'] = "select e.tcversion_id, e.status from " . 
              $this->db->get_table('executions') . " e join (" . 
              "select ee.tcversion_id,ee.testplan_id,ee.platform_id,ee.build_id, max(ee.id) as id " . 
              "from " . $this->db->get_table('executions') . " ee " . 
              " where ee.build_id = " . $ic['filters']['build_id'] . 
              $platform4EE .
              " group by ee.tcversion_id,ee.testplan_id,ee.platform_id,ee.build_id )" . 
              " ecSingle on e.id = ecSingle.id " . 
              $ic['join']['bugs'] . 
              " where e.build_id = " . $ic['filters']['build_id'];
          if (isset($ic['where']['bugs']))
          {
              $link_sql['exec'] .= $ic['where']['bugs'];
          }
          $link_sql['exec'] .= " order by e.tcversion_id ";
      }
      else 
      {
          $link_sql['exec'] = "select e.tcversion_id, e.status from " .
      ...
      if ($has_build)
      {
          $link_sql['exec_build'] = "select e.tcversion_id, e.status from " .
              $this->db->get_table('executions') . " e join (" .
              "select ee.tcversion_id,ee.testplan_id,ee.platform_id,ee.build_id, max(ee.id) as id " .
              "from " . $this->db->get_table('executions') . " ee" .
              " where ee.build_id = " . $ic['filters']['build_id'] .
      ...
      {
          $ff = (array)$ic['filters']['assigned_to'];
          ...
          {
              $link_sql['ua'] = "select distinct(ua.feature_id) from " . 
                  $this->db->get_table('user_assignments') . " ua " .
                  "where ua.build_id = ". $ic['filters']['build_id'] . 
                  " and ua.type = 1 and ua.user_id in (" . implode(",",$ff) . ") ";
          }
          
          $link_sql['ua_assigned'] = "select distinct(ua.feature_id) from " . 
              $this->db->get_table('user_assignments') . " ua " . 
              " where ua.build_id = " . $ic['filters']['build_id'] . 
              " and ua.type = 1 ";          
      }
  ...
  function initGetLinkedForTree($tplanID,$filtersCfg,$optionsCfg)
  {
    ...
    {
      $ic['join']['ua'] = " LEFT OUTER JOIN ".$this->db->get_table('user_assignments')." UA " .
                          " ON UA.feature_id = TPTCV.id and UA.build_id = TPTCV.build_id" . 
...
//         $ic['where']['where'] .= " and exists (select 1 from nodes_hierarchy nh2 " .
   ...
   * @internal revisions
   */
  function newinitGetLinkedForTree($tplanID,$filtersCfg,$optionsCfg)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $dummy = array('exec_type','tc_id','builds','keywords','executions','platforms');
      ...
      if( isset($ic['options']['assigned_on_build']) &&
          !is_null($ic['options']['assigned_on_build']) )
      {
          $ic['join']['ua'] = " LEFT OUTER JOIN ".$this->db->get_table('testcase_keywords')." TK " .
              " ON NH_TCASE.id = TK.testcase_id JOIN testlink.keywords k on k.id = TK.keyword_id ";
      }
  ...
  // [exec_type] default null -> all types. 
  //       
  function getLinkedForTesterAssignmentTree($id,$build_id,$filters=null,$options=null)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $has_build_id = false;
    $safe['tplan_id'] = intval($id);
    if (isset($build_id) && $build_id != 0 && $build_id != "")
    {
    ...
    if(  ($my['options']['allow_empty_build'] == 0) && $safe['build_id'] <= 0 )
    {
      // CRASH IMMEDIATELY
      throw new Exception( $debugMsg . " Can NOT WORK with \$my['filters']['build_id'] <= 0");
    }
    ...
    if ($has_build_id)
    {
        $where_clause = " TPTCV.build_id =" . $safe['build_id'] .
              $my['where']['where'] .
              " and not exists (select e.id from " .$this->db->get_table('executions') . 
              " e where e.build_id = " . $safe['build_id'] .
              " and e.tcversion_id = TPTCV.tcversion_id)";
        
    }
    else
    {
         $where_clause = " 1 = 1 " .$my['where']['where'];
    }
    $union['not_run'] .= $where_clause;
...
//                  " JOIN {$this->tables['nodes_hierarchy']} NH_TCASE ON NH_TCASE.id = NH_TCV.parent_id " .
    ...
    if ($has_build_id)
    {
        $union['exec'] .= " WHERE TPTCV.build_id =" . $safe['build_id'] .
            $my['where']['where'];
        return $union;
    }
    else 
    {
        $union['exec'] .= " WHERE TPTCV.testplan_id =" . $safe['tplan_id'] .
            $my['where']['where'];
        return $union['not_run'];
    }
  }
  
  function getLinkInfoForTc($tcase_id)
  {
      $item = array();
      
      $tcase_sql = "select nhtc.name as tcase_name,nhtc.id as tcase_id,nhtc.id as tc_id,nhtc.parent_id AS testsuite_id " .
          " from " . $this->db->get_table('nodes_hierarchy') . " nhtc " .
          " where nhtc.id = " . $tcase_id;
      
      $tcv_sql = "select nhc.id as id,tcv.id as tcv_id,tcv.version,tcv.execution_type,tcv.status,tcv.tc_external_id as external_id " .
          " from " . $this->db->get_table('tcversions') . " tcv " .
          " join " . $this->db->get_table('nodes_hierarchy') . " nh on tcv.id = nh.id " .
          " join " . $this->db->get_table('nodes_hierarchy') . " nhc on nh.parent_id = nhc.id " .
          " where nhc.id = " . $tcase_id . " order by tcv.version desc"; 
      
      $keyword_sql = "select tk.testcase_id, tk.keyword_id from " .
          $this->db->get_table('testcase_keywords') . " tk " . 
          " where tk.testcase_id = "  . $tcase_id;
      
      $tcase_info = $this->db->fetchFirstRow($tcase_sql);
      $tcv_info = $this->db->fetchFirstRow($tcv_sql);
      $keyword_info = $this->db->fetchRowsIntoMap($keyword_sql, 'keyword_id');
      
      $item = $tcase_info;
      $item = array_merge($item, $tcv_info);
      $item['keyword'] = array();
      foreach ($keyword_info as $keyword_id => $tcase_kw)
      {
          $item['keyword'][$keyword_id] = $keyword_id;
      }
...
//            " JOIN {$this->tables['nodes_hierarchy']} NHTCV " .
    ...
    switch($my['opt']['output'])
    {
      case 'tcase_info':
        ...
        $sql .= " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTC ON NHTC.id = NHTCV.parent_id ";
      break;
      ...
      case 'assignment_info':
        ...
        $sql .= " LEFT OUTER JOIN ".$this->db->get_table('user_assignments')." UA " .
                " ON UA.build_id = " . $safe_id['build_id'] .
                " AND UA.feature_id = TPTCV.id AND UA.build_id = TPTCV.build_id";
                 
        $sql .= " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLAT " .
                " ON PLAT.id = TPTCV.platform_id ";

        $sql .= " LEFT OUTER JOIN ".$this->db->get_table('users')." USERS " .
    ...
    if ($my['opt']['output'] == 'assignment_info')
    {
        $safe_id['build_id'] = intval($my['opt']['build4assignment']);
        $sql .= " and TPTCV.build_id = " . $safe_id['build_id']; 
    }
  ...
  public function getLinkedStaticView($id,$filters=null,$options=null)
  {
    ...
    if( !is_null($my['filters']['build_id']) )
    {
        ...
        $addWhere['tcv_build'] = ' AND TPTCV.build_id =' . $dummy;
        if (isset($my['filters']['setAssignedTo']) && $my['filters']['setAssignedTo'])
        {
            $addWhere['build'] = 'AND UA.build_id =' . $dummy;
            
            $join['build'] = " JOIN ".$this->db->get_table('user_assignments')." UA " .
                " ON UA.feature_id = TPTCV.id ";
...
//            $join['build'] .
//            " LEFT OUTER JOIN {$this->tables['platforms']} PLAT ON PLAT.id = TPTCV.platform_id ";
       $sql .=" FROM ".$this->db->get_table('nodes_hierarchy')." NH_TCV " .
            " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TCASE ON NH_TCV.parent_id = NH_TCASE.id " .
            " JOIN ".$this->db->get_table('testplan_tcversions')." TPTCV ON TPTCV.tcversion_id = NH_TCV.id " .
            " JOIN  ".$this->db->get_table('tcversions')." TCV ON  TCV.id = NH_TCV.id " .
            $join['build'] .
            " LEFT OUTER JOIN ".$this->db->get_table('platforms')." PLAT ON PLAT.id = TPTCV.platform_id ";

    $sql .= " WHERE TPTCV.testplan_id={$safe['tplan']} " . $addWhere['tcv_build']  .
...
//         $sql2run = " SELECT * FROM ($sql2run) XX ORDER BY " . $my['options']['orderBy'];
        $sql2run = " SELECT * FROM ".$this->db->get_table($sql2run)." XX ORDER BY " . $my['options']['orderBy'];
      }
  ...
  function getLinkedTCVersionsSQL($id,$filters=null,$options=null)
  {
    ...
    //add by zhouzhaoxin 20160606 for show testcases filter by plan and build not only by plan
    $addBuildClause = "";
    if (isset($filters['build_id']))
    {
        $safe['build_id'] = intval($filters['build_id']);
        $addBuildClause = " and TPTCV.build_id = " . $safe['build_id'] . " ";
    }
...
//                 " FROM {$this->tables['executions']} EE " . 
    ...
    if($my['options']['testSuiteInfo'])
    {
      ...
      $my['join']['tsuites'] = " JOIN ".$this->db->get_table('nodes_hierarchy')." NH_TSUITE " . 
                               " ON NH_TSUITE.id = NH_TCASE.parent_id ";
    }
...
//            " SELECT is_public FROM {$this->tables['testplans']} " .
           " SELECT is_public FROM ".$this->db->get_table('testplans')." " .
           " WHERE id =" . intval($id);   
...
//                " FROM {$this->tables['builds']} WHERE testplan_id = {$id} " ;
  ...
  function writeExecution($ex)
  {
    ...
    $sql = "/* {$debugMsg} */ " .
           "INSERT INTO ".$this->db->get_table('executions')." " .
...
//            "FROM {$this->tables['executions']} E " .
  ...
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* $debugMsg */ " . "UPDATE ".$this->db->get_table('testplans')." SET active=1 WHERE id=" . intval($id);
    $this->db->exec_query($sql); 
  }
  ...
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $sql = "/* $debugMsg */ " . "UPDATE ".$this->db->get_table('testplans')." SET active=0 WHERE id=" . intval($id); 
    $this->db->exec_query($sql); 
  }
...
//            " SELECT {$fields2get} FROM {$this->tables['testplans']} " .
           " SELECT {$fields2get} FROM ".$this->db->get_table('testplans')." " .
...
//            " WHERE TPTCV.testplan_id = " . $safe['tplan_id'];
    $sql = " /* $debugMsg */ ".
        ...
        " ON TPTCV.tcversion_id = E.tcversion_id " .
        " AND TPTCV.testplan_id = E.testplan_id " .
        " JOIN ".$this->db->get_table('execution_bugs')." EB " .
        " ON EB.execution_id = E.id " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTCV " .
        " ON NHTCV.id = E.tcversion_id " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTC " .
        ...
        " JOIN ".$this->db->get_table('tcversions')." TCV " .
        " ON TCV.id = E.tcversion_id " .
        " JOIN ".$this->db->get_table('nodes_hierarchy')." NHTSUITE " .
        " ON NHTSUITE.id = NHTC.parent_id " .
        " WHERE TPTCV.testplan_id = " . $safe['tplan_id'];
...
//             " JOIN {$this->tables['nodes_hierarchy']} NH_TCASE ON NH_TCASE.id = NH_TCV.parent_id " .
    ...
    if ($safe['build_id'] > 0)
    {
        $sql .=  " and TPTCV.build_id = " . $safe['build_id'] . " ";   
    }
    $sql .= $my['where']['where'];
   ...
   *
   */
  function newgetLinkedTCVXmen($id, $filters=null, $options=null)
  {
      $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
      $tsuite_id = intval($id);
      $tsuite_depth_abs = null;
      $sql = "select node_depth_abs from " . $this->db->get_table('nodes_hierarchy') . 
          " where id = " . $tsuite_id;
      $tsuite_depth_abs = $this->db->fetchOneValue($sql);      
      $tcase_sql = "select nhtc.name as tcase_name,nhtc.id as tcase_id,nhtc.id as tc_id,nhtc.parent_id AS testsuite_id " .
          " from " . $this->db->get_table('nodes_hierarchy') . " nhtc " .
          " where nhtc.node_depth_abs like '" . $tsuite_depth_abs . "%'" .
          " and nhtc.node_type_id = 3 ";
      if (isset($filters['filter_tc_id']) && $filters['filter_tc_id'] > 0) 
      {
          $tcase_sql .= " and nhtc.id = " . $filters['filter_tc_id'];
      }
      if (isset($filters['filter_testcase_name']) && $filters['filter_testcase_name'] != null)
      {
          $tcase_sql .= " and nhtc.name like '%" . $filters['filter_testcase_name'] . "%'";
      }
      $tcase_sql .= " order by nhtc.id";
      ...
      $tcv_sql = "select nhc.id as id,tcv.id as tcv_id,tcv.version,tcv.execution_type,tcv.status,tcv.tc_external_id as external_id " . 
          ...
          " join " . $this->db->get_table('nodes_hierarchy') . " nh on tcv.id = nh.id " . 
          " join " . $this->db->get_table('nodes_hierarchy') . " nhc on nh.parent_id = nhc.id " .
          " where nhc.node_depth_abs like '" . $tsuite_depth_abs . "%'" .
          " and nhc.node_type_id = 3 ";
      if (isset($filters['filter_execution_type']))
      {
          $tcv_sql .= " and tcv.execution_type = " . $filters['filter_execution_type'];
      }
      if (isset($filters['filter_importance']))
      {
          $tcv_sql .= " and tcv.importance = " . $filters['filter_importance'];
      }
      if (isset($filters['filter_workflow_status']))
      {
          $tcv_sql .= " and tcv.status = " . $filters['filter_workflow_status'];
      }
      $tcv_sql .= " order by nhc.id, tcv.version desc";
      $tcv_set = $this->db->get_recordset($tcv_sql);
      
      $tcase_count = count($tcase_set);
      $tcv_count = count($tcv_set);
      $tc_final_set = array();
      $idx = 0;
      $jdx = 0;
      $to_end = false;
      while($idx < $tcase_count)
      {
          ...
          if ($tcase_set[$idx]['tcase_id'] == $tcv_set[$jdx]['id'])
          {
              $node = array();
              $node = $tcase_set[$idx];
              $node['keyword'] = array();
              $node['version'] = $tcv_set[$jdx]['version'];
              $node['execution_type'] = $tcv_set[$jdx]['execution_type'];
              $node['status'] = $tcv_set[$jdx]['status'];
              $node['external_id'] = $tcv_set[$jdx]['external_id'];
              $tc_final_set[] = $node;
              $idx++;    
              $jdx++;
          }
          else if ($tcase_set[$idx]['tcase_id'] > $tcv_set[$jdx]['id'])
          {
              $jdx++;
          }
          else 
      ...
      $keyword_sql = "select tk.testcase_id, tk.keyword_id from " .
          $this->db->get_table('testcase_keywords') . " tk order by tk.testcase_id ";
      $keyword_set = $this->db->get_recordset($keyword_sql);
      
      $tcase_count = count($tc_final_set);
      $keyword_count = count($keyword_set);
      $idx = 0;
      $jdx = 0;
      $to_end = false;
      while ($idx < $tcase_count)
      {
          ...
          if ($tc_final_set[$idx]['tcase_id'] == $keyword_set[$jdx]['testcase_id'])
          {
              $tc_final_set[$idx]['keyword'][$keyword_set[$jdx]['keyword_id']] = $keyword_set[$jdx]['keyword_id'];
              $jdx++;
          }
          else if ($tc_final_set[$idx]['tcase_id'] > $keyword_set[$jdx]['testcase_id'])
          {
              $jdx++;
          }
          else 
      ...
      $items = array();
      for ($idx = 0; $idx < $tcase_count; $idx++)
      {
          $items[$tc_final_set[$idx]['tcase_id']] = $tc_final_set[$idx];
      }
...
class build_mgr extends tlObject
{
  ...
  function setZeroOneAttr($id,$attr,$zeroOne)
  {
    ...
    $sql = "/* $debugMsg */ " . 
           "UPDATE ".$this->db->get_table('builds')." SET {$attr}=" . ($zeroOne ? 1 : 0) . " WHERE id=" . intval($id);
    $this->db->exec_query($sql); 
  }
  ...
  */
  function create($tplan_id,$name,$notes = '',$active=1,$open=1,$release_date='',
                  $stage_id, $begin_date, $end_date)
  {
    $targetDate=trim($release_date);
    $sql = " INSERT INTO ".$this->db->get_table('builds')." " .
           " (testplan_id,name,notes,release_date,active,is_open,creation_ts, stage_id,begin_date,end_date) " .
    ...
    // Important: MySQL do not support default values on datetime columns that are functions
    // that's why we are using db_now().
    $sql .= "{$active},{$open},{$this->db->db_now()},$stage_id, '$begin_date','$end_date')";                        
    
    $new_build_id = 0;
    $result = $this->db->exec_query($sql);
    if ($result)
    {
      $new_build_id = $this->db->insert_id($this->db->get_table('builds'));
    }
  ...
  /*chy20160614
   ...
   rev :
   */
  function getTcidByStatus($sourceBulidID,$status){
      $sql="";
      $tcidstr="";
      if($status=="n"){
//           $sql="SELECT tcv.tcversion_id as tcid
//                     from testplan_tcversions tcv
//                     where 
//                     (tcv.build_id, tcv.tcversion_id) not in
//                     (select build_id, tcversion_id from executions)
//                     and tcv.build_id = '$sourceBulidID'";
          $sql="SELECT tcv.tcversion_id as tcid
          from ".$this->db->get_table('testplan_tcversions')." tcv
          where
          (tcv.build_id, tcv.tcversion_id) not in
          (select build_id, tcversion_id from ".$this->db->get_table('executions').")
          and tcv.build_id = '$sourceBulidID'";
...
//           $sql="SELECT DISTINCT executions.tcversion_id as tcid FROM executions
          $sql="SELECT DISTINCT executions.tcversion_id as tcid FROM ".$this->db->get_table('executions')." "."
        WHERE executions.build_id='$sourceBulidID' AND executions.status='$status'";
  ...
  /*chy20160614
   function: isExistTcid
   returns: array();
   rev :
   */
  function isExistTcid($tplan_id,$buildID,$tcid){
     $isexcist=0;
//       $sql="SELECT testplan_tcversions.tcversion_id as tcid 
//           FROM testplan_tcversions
//           WHERE testplan_tcversions.build_id='$buildID' AND 
//           testplan_tcversions.testplan_id='$tplan_id' AND 
//           testplan_tcversions.tcversion_id='$tcid'";
      $sql="SELECT testplan_tcversions.tcversion_id as tcid
      FROM ".$this->db->get_table('testplan_tcversions')." "."
      WHERE testplan_tcversions.build_id='$buildID' AND
      testplan_tcversions.testplan_id='$tplan_id' AND
  ...
  /*chy20160614
   function: insertInheritTcid
   returns: $tcidmsg array();
   rev :
   */
   function insertInheritTcid($tcidmsg){
       
     $tplan_id=$tcidmsg['tplan_id'];
     $user_id=$tcidmsg['user_id'];
     $tc_id=$tcidmsg['tc_id'];
     $platform=$tcidmsg['platform'];
     $build_id=$tcidmsg['build_id'];

      $sql = "INSERT INTO ".$this->db->get_table('testplan_tcversions')." 
      (testplan_id,author_id,creation_ts,tcversion_id,platform_id,build_id)
      VALUES ('$tplan_id','$user_id',{$this->db->db_now()},'$tc_id','$platform','$build_id')";
  ...
  */
  function update($id,$name,$notes,
                  $active=null,$open=null,$release_date='',
                  $closed_on_date='',
                  $stage_id, $begin_date, $end_date)
  {
    $closure_date = '';
    $targetDate=trim($release_date);
    $sql = " UPDATE ".$this->db->get_table('builds')." " .
           " SET name='" . $this->db->prepare_string($name) . "'," .
    ...
    $sql .= ",stage_id=$stage_id,begin_date='$begin_date',end_date='$end_date' ";
...
//     " WHERE execution_id IN (SELECT id FROM {$this->tables['executions']} {$where}) ";
    $sql = " DELETE FROM ".$this->db->get_table('execution_bugs')." " .
           " WHERE execution_id IN (SELECT id FROM ".$this->db->get_table('executions')." {$where}) ";
    $result = $this->db->exec_query($sql);

    $sql = " DELETE FROM ".$this->db->get_table('executions')." {$where}";
    $result = $this->db->exec_query($sql);

    $sql = " DELETE FROM ".$this->db->get_table('user_assignments')."  {$where}";
    $result=$this->db->exec_query($sql);

    // Custom fields
    $this->cfield_mgr->remove_all_design_values_from_node($safe_id,'build');

    $sql = " DELETE FROM ".$this->db->get_table('builds')." WHERE id={$safe_id}";
    $result=$this->db->exec_query($sql);
    
    //add by chenye 0614 delete testplan_tcversions record
    $sql = " DELETE FROM ".$this->db->get_table('testplan_tcversions')." WHERE build_id={$safe_id}";
    $result=$this->db->exec_query($sql);
    return $result ? 1 : 0;
  }
...
//         $sql .= " SELECT id,is_open,active FROM {$this->tables['builds']} "; 
        $sql .= " SELECT id,is_open,active FROM ".$this->db->get_table('builds')." ";
      break;
      
      case 'full':
      default:
//         $sql .= " SELECT * FROM {$this->tables['builds']} "; 
        $sql .= " SELECT * FROM ".$this->db->get_table('builds')." ";
      break;
    }
...
//     $sql .= " FROM {$this->tables['builds']} B " .
    $sql .= " FROM ".$this->db->get_table('builds')." B " .
  ...
  */
  ...
  /*
   function: count_tcversions
   count tcversions assign to builds(because we assign tcversion to build, not only to plan)
   args :
   $id   filter build_id

   returns:    build_id to tcversion count maps
  
   author :  zhouzhaoxin
   history:  create by zhouzhaoxin on 20160711
   */
  function count_tcversions($id)
  {
      //check input params
      if( is_null($id) || (is_int($id) && intval($id) <= 0 ) || (is_array($id) && count($id) == 0) )
      {
...
//       $sql = "select build_id, count(*) as qty from testplan_tcversions " .
      $sql = "select build_id, count(*) as qty from ".$this->db->get_table('testplan_tcversions')." " .
      " where build_id in (" . implode(',',(array)$id) . ")  group by build_id";
      
      $out = $this->db->fetchRowsIntoMap($sql,'build_id');
...
class milestone_mgr extends tlObject
{
  ...
  */
  function create($mi)
  {
    ...
    /* for future
    $sql = "INSERT INTO ".$this->db->get_table('milestones')." " .
  ...
  */
  ...
  {
    $sql = "UPDATE ".$this->db->get_table('milestones')." " . 
           " SET name='{$this->db->prepare_string($name)}', " .
  ...
  */
  function delete($id)
  {
    $sql = "DELETE FROM ".$this->db->get_table('milestones')." WHERE id={$id}";
    $result=$this->db->exec_query($sql);
    return $result ? 1 : 0;
  }
...
//     $sql = " SELECT id, name FROM {$this->tables['milestones']} " .
    $sql = " SELECT id, name FROM ".$this->db->get_table('milestones')." " .
...
//        " FROM {$this->tables['milestones']} M, {$this->tables['nodes_hierarchy']} NH " .
       " FROM ".$this->db->get_table('milestones')." M, ".$this->db->get_table('nodes_hierarchy')." NH " .
