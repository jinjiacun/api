...
function generateTestSpecTree(&$db,$tproject_id, $tproject_name,$linkto,$filters=null,$options=null)
{
  ...
  */
  ...
  $test_cases = getTestSpecTreeFast($tproject_id,$tproject_mgr,$my['filters']);
  $tcase_list = array();
  if ($test_cases)
  {
      ...
      {
          $tck_map = $tproject_mgr->get_only_tcase_by_keyword($tproject_id,
              $my['filters']['filter_keywords'],
              $my['filters']['filter_keywords_filter_type']);
          if (is_null($tck_map) || count($tck_map) <= 0)
          {
              $tck_map = null;  // means that tree will be empty
          }
          else 
          {
              $tcase_count = count($test_cases, COUNT_NORMAL);
              $keyword_count = count($tck_map, COUNT_NORMAL);
              $idx = 0;
              $jdx = 0;
              $tc_to_end = false;
              while ($idx < $tcase_count)
              {
                  ...
                  if ($test_cases[$idx]['id'] == $tck_map[$jdx]['testcase_id'])
                  {
                      $tcase_list[] = $test_cases[$idx];
                      $idx++;
                  ...
                  else if ($test_cases[$idx]['id'] < $tck_map[$jdx]['testcase_id'])
                  {
                      $idx++;
                  }
                  else 
  ...
  if (is_null($my['filters']['filter_keywords']))
  {
      $tcase_list = $test_cases;
  }
  
  if( $my['options']['viewType'] == 'testSpecTree' )
  {
      $tc2show = renderTreeNodeFastWithTc($test_suites, $test_spec, $tcase_list, $hash_id_descr, $linkto, 
          $tcase_prefix, $my['options']);
  }
  else
  {
      $tc2show = renderTreeNodeFast($test_suites, $test_spec, $tcase_list, $hash_id_descr, $linkto,
          $tcase_prefix, $my['options']);
  }
/*  
...
//                      " FROM {$tables['tcversions']}  TCV " .
                     " FROM ".$db->get_table('tcversions')."  TCV " .
...
//            " FROM {$tables['tcversions']} TCV, {$tables['nodes_hierarchy']} NH " .
           " FROM ".$db->get_table('tcversions')." TCV, ".$db->get_table('nodes_hierarchy')." NH " .
...
/**
 * Create the string representation suitable to create a graphic visualization
 * of a testsuite with not rev, fast mode
 *
 * @internal revisions
 * 20170611 add by zhouzhaoxin
 */
function renderTreeNodeFast($tsuite, &$pnode, $tcase, $hash_id_descr, $linkto, $testCasePrefix, $opt)
{
    $testCasesIDList = '';
    ...
    foreach ($tcase as $idx => $tcase_info)
    {
        if (!array_key_exists($tcase_info['parent_id'], $tccount_list))
        {
            $tccount_list[$tcase_info['parent_id']] = array();
        }
        
        $tccount_list[$tcase_info['parent_id']][] = REMOVEME; 
        $testCasesIDList .= $tcase_info['id'] . ',';
    }
    
    $depth_cnt = count($tsuite, COUNT_NORMAL);
    
    for ($depth = $depth_cnt; $depth > 0; $depth--)
    {
        if (!isset($tsuite[$depth]))
        {
            continue;
        }
        
        foreach ($tsuite[$depth] as $tsuite_id => $tsuite_info)
        {
            $testcase_count = 0;
            
            if (isset($tsuite[$depth][$tsuite_id]['childNodes'])
                && count($tsuite[$depth][$tsuite_id]['childNodes'], COUNT_NORMAL) > 0)
            {
                // if testsuite has child testsuite ,first count child suite info
                foreach ($tsuite[$depth][$tsuite_id]['childNodes'] as $id => $child_suite)
                {
                    $testcase_count += $child_suite['testcase_count'];
                }
            }
            
            
            if (isset($tccount_list[$tsuite_id]))
            {
                if (!isset($tsuite[$depth][$tsuite_id]['childNodes']))
                {
                    $tsuite[$depth][$tsuite_id]['childNodes'] = $tccount_list[$tsuite_id];
                }
                else 
                {
                    foreach ($tccount_list[$tsuite_id] as $key => $row)
                    {
                        $tsuite[$depth][$tsuite_id]['childNodes'][] = $row;
                    }
                }
                $testcase_count += count($tccount_list[$tsuite_id]);
            }
            
            $tsuite[$depth][$tsuite_id]['testcase_count'] = $testcase_count;
            if ($testcase_count <= 0)
            {
                unset($tsuite[$depth][$tsuite_id]);
                continue;
            }
            
            $tsuite[$depth][$tsuite_id]['testlink_node_name'] = str_replace(array("\n","\r"), array("",""), $tsuite_info['name']);
            $tsuite[$depth][$tsuite_id]['testlink_node_name'] = htmlspecialchars($tsuite[$depth][$tsuite_id]['testlink_node_name'], ENT_QUOTES);
            $tsuite[$depth][$tsuite_id]['testlink_node_type'] = $hash_id_descr[$tsuite_info['node_type_id']];
            $tsuite[$depth][$tsuite_id]['forbidden_parent'] = $forbidden_parents[$tsuite[$depth][$tsuite_id]['testlink_node_type']];
            
            $pfn = $f2call[$tsuite[$depth][$tsuite_id]['testlink_node_type']];
            $tsuite[$depth][$tsuite_id]['text'] =  $tsuite[$depth][$tsuite_id]['testlink_node_name'] . " (" . $testcase_count . ")";
            if (isset($opt['nodeHelpText'][$tsuite[$depth][$tsuite_id]['testlink_node_type']]))
            {
                $tsuite[$depth][$tsuite_id]['text'] = '<span title="' . $opt['nodeHelpText'][$tsuite[$depth][$tsuite_id]['testlink_node_type']] . '">' .
                    $tsuite[$depth][$tsuite_id]['text'] . '</span>';
            }
            $tsuite[$depth][$tsuite_id]['position'] = isset($tsuite[$depth][$tsuite_id]['node_order']) ? $tsuite[$depth][$tsuite_id]['node_order'] : 0;
            $tsuite[$depth][$tsuite_id]['href'] = "javascript:{$pfn}({$tsuite[$depth][$tsuite_id]['id']})";
            
            if ($depth > 1 && array_key_exists($tsuite[$depth][$tsuite_id]['parent_id'], $tsuite[$depth - 1]))
            {
                $tsuite[$depth - 1][$tsuite[$depth][$tsuite_id]['parent_id']]['childNodes'][] = $tsuite[$depth][$tsuite_id];
            }
        }
    }

    $depth = 1;
    if (!isset($pnode['testcase_count']))
    {
        $pnode['testcase_count'] = 0;
    }
    
    if (isset($tsuite[$depth]) && count($tsuite[$depth]) > 0)
    {
        foreach ($tsuite[$depth] as $tsuite_id => $tsuite_info)
        {
            $pnode['testcase_count'] += $tsuite_info['testcase_count'];
            $pnode['childNodes'][] = $tsuite[$depth][$tsuite_id];
        }
    }
    
    $pnode['testlink_node_name'] = str_replace(array("\n","\r"), array("",""), $pnode['name']);
    $pnode['testlink_node_name'] = htmlspecialchars($pnode['testlink_node_name'], ENT_QUOTES);
    
    $pnode['testlink_node_type'] = 'testproject';
    $pnode['forbidden_parent'] = $forbidden_parents[$pnode['testlink_node_type']];
    
    $pfn = $f2call[$pnode['testlink_node_type']];
    $pnode['text'] =  $pnode['testlink_node_name'] . " (" . $pnode['testcase_count'] . ")";
    if(isset($opt['nodeHelpText'][$pnode['testlink_node_type']]))
    {
        $pnode['text'] = '<span title="' . $opt['nodeHelpText'][$pnode['testlink_node_type']] . '">' .
            $pnode['text'] . '</span>';
    }
    $pnode['position'] = isset($pnode['node_order']) ? $pnode['node_order'] : 0;
    $pnode['href'] = "javascript:{$pfn}({$pnode['id']})";
...
/**
 * Create the string representation suitable to create a graphic visualization
 * of a testsuite with not rev, fast mode, with testcase node
 *
 * @internal revisions
 * 20170611 add by zhouzhaoxin
 */
function renderTreeNodeFastWithTc($tsuite, &$pnode, $tcase, $hash_id_descr, $linkto, $testCasePrefix, $opt)
{
    $testCasesIDList = '';
    ...
    foreach ($tcase as $idx => $tcase_info)
    {
        ...
        $tccount_list[$tcase_info['parent_id']][] = $tcase_info;
    }
    ...
    for ($depth = $depth_cnt; $depth > 0; $depth--)
    {
        ...
        foreach ($tsuite[$depth] as $tsuite_id => $tsuite_info)
        {
            ...
            if (isset($tccount_list[$tsuite_id]) && count($tccount_list[$tsuite_id]) > 0)
            {
                if (!isset($tsuite[$depth][$tsuite_id]['childNodes']))
                {
                    $tsuite[$depth][$tsuite_id]['childNodes'] = array();
                }
                ...
                foreach ($tccount_list[$tsuite_id] as $key => $row)
                {
                    ...
                    $node['testlink_node_name'] = str_replace(array("\n","\r"), array("",""), $node['name']);
                    $node['testlink_node_name'] = htmlspecialchars($node['testlink_node_name'], ENT_QUOTES);
                    $node['testlink_node_type'] = 'testcase';
                    $node['forbidden_parent'] = $forbidden_parents[$node['testlink_node_type']];
                    ...
                    $node['href'] = "javascript:{$pfn}({$node['id']})";
                    $node['leaf'] = true;
                    $tsuite[$depth][$tsuite_id]['childNodes'][] = $node;
                }
                $testcase_count += count($tccount_list[$tsuite_id]);
            }
    ...
    if (isset($tsuite[$depth]) && count($tsuite[$depth]) > 0)
    {
        foreach ($tsuite[$depth] as $tsuite_id => $tsuite_info)
        {
            if (isset($tsuite_info['testcase_count']))
            {
                $pnode['testcase_count'] += $tsuite_info['testcase_count'];
            }
            $pnode['childNodes'][] = $tsuite[$depth][$tsuite_id];
...
//                                    " FROM {$tables['tcversions']} TCVX " .
                                   " FROM ".$db->get_table('tcversions')." TCVX " .
//                                    " JOIN {$tables['nodes_hierarchy']} NHTCX " .
                                   " JOIN ".$db->get_table('nodes_hierarchy')." NHTCX " .
...
//              " FROM {$tables['cfield_design_values']} CFD, {$tables['nodes_hierarchy']} NH " .
             " FROM ".$db->get_table('cfield_design_values')." CFD, ".$db->get_table('nodes_hierarchy')." NH " .
                             ...
                             $filters = null, $options = null) 
{
  ...
  //change by zhouzhaoxin 20161229 to add source testplan filter
...
//       " FROM {$tables['requirements']} R " .
         " FROM ".$db->get_table('requirements')." R " .
         " JOIN ".$db->get_table('nodes_hierarchy')." NH_R ON NH_R.id = R.id " .
         " JOIN ".$db->get_table('nodes_hierarchy')." NH_RV ON NH_RV.parent_id = NH_R.id " .
         " JOIN ".$db->get_table('req_versions')." RV ON RV.id = NH_RV.id " .
         " JOIN ".$db->get_table('req_specs')." RS ON RS.id = R.srs_id " .
         " JOIN ".$db->get_table('req_specs_revisions')." RSPECREV ON RSPECREV.parent_id = RS.id " .
         " JOIN ".$db->get_table('nodes_hierarchy')." NH_RS ON NH_RS.id = RS.id ";

  if (isset($filters['filter_relation'])) 
  {
    $sql .= " JOIN ".$db->get_table('req_relations')." RR " .
  ...
  if (isset($filters['filter_tc_id'])) 
  {
    ...
    $sql .= " JOIN ".$db->get_table('req_coverage')." RC ON RC.req_id = R.id " .
            " JOIN ".$db->get_table('nodes_hierarchy')." NH_T ON NH_T.id = RC.testcase_id " .
            " JOIN ".$db->get_table('nodes_hierarchy')." NH_TV on NH_TV.parent_id = NH_T.id " .
            " JOIN ".$db->get_table('tcversions')." TV ON TV.id = NH_TV.id " .
  ...
  if (isset($filters['filter_custom_fields'])) 
  {
    ...
    {
      $sql .= " JOIN ".$db->get_table('cfield_design_values')." CF{$suffix} " .
  ...
  if (isset($filters['filter_testplan']) && $filters['filter_testplan'] != 0)
  {
      $testplan = $db->prepare_string($filters['filter_testplan']);
      $sql .= " AND RV.testplans_id = '{$testplan}' ";
  }
...
//       "   FROM {$tables['executions']} E2 " .
      "   FROM ".$dbHandler->get_table('executions')." E2 " .
...
function getTestSpecTree($tprojectID,&$tprojectMgr,&$fObj)
{
  ...
  if( isset($fObj['filter_assign_status']) && !is_null($fObj['filter_assign_status']) )
  {
      $flt['assign_status'] = intval($fObj['filter_assign_status']);
  }
...
/**
 * get test spec tree sql info for fast mode 
 * 
 * @internal revisions
 * 20170611 add by zhouzhaoxin for improve performance
 */
function getTestSpecTreeFast($tprojectID,&$tprojectMgr,&$fObj)
{
    $flt = array();
  $flt['exclude_branches'] = isset($fObj['filter_toplevel_testsuite']) && is_array($fObj['filter_toplevel_testsuite']) ?
                             $fObj['filter_toplevel_testsuite'] : null;
  ...
  if( isset($fObj['filter_priority']) && !is_null($fObj['filter_priority']) )
  {
      $xx = (array)$fObj['filter_priority'];
      if($xx[0] >0)
      {
          $flt['priority'] = $xx;
      }
  ...
  $opt = array('recursive' => true,'exclude_testcases' => false);
  $items = $tprojectMgr->getTestSpecFast($tprojectID,$flt,$opt); 
