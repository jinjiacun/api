...
//         "compression_type,file_path,fk_id,fk_table FROM {$this->tables['attachments']} ";
            "compression_type,file_path,fk_id,fk_table FROM ".$db->get_table('attachments')." ";
  ...
  public function writeToDB(&$db,&$itemID=null)
  {
    ...
    $query = "INSERT INTO ".$db->get_table('attachments')." 
  ...
  {
    $query = "DELETE FROM ".$db->get_table('attachments')." WHERE id = {$this->dbID}";
    $result = $db->exec_query($query);
