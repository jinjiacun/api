...
class testsuite extends tlObjectWithAttachments
{
  ...
  */
                  ...
                  $action_on_duplicate_name='allow_repeat')
  {
    ...
    if ($ret['status_ok'])
    {
      ...
      $tsuite_id = $this->tree_manager->new_node($parent_id,$this->my_node_type,
                                                 $name,$node_order);
      $sql = " INSERT INTO ".$this->db->get_table('testsuites')." (id,details) " .
  ...
  function update($id, $name, $details, $parent_id=null, $node_order=null)
  {
    ...
    if($check['status']==0)
    {
      $sql = "/* $debugMsg */ UPDATE ".$this->db->get_table('testsuites')." " .
  ...
  function delete($unsafe_id)
  {
    ...
    $sql = "DELETE FROM " . $this->db->get_table($this->object_table). " WHERE id={$id}";
    $result = $this->db->exec_query($sql);
  ...
  */
  function get_by_name($name, $parent_id=null, $opt=null)
  {
    ...
    $sql .= " NH.parent_id " .
            " FROM ".$this->db->get_table('testsuites')." TS " .
            " JOIN ".$this->db->get_table('nodes_hierarchy')." NH " .
            " ON NH.id = TS.id " .
  ...
  */
  function get_by_id($id,$opt=null)
  {
    ...
    $sql = "/* $debugMsg */ SELECT {$f2g} " .
           "  FROM ".$this->db->get_table('testsuites')." TS " .
           "  JOIN ".$this->db->get_table('nodes_hierarchy')." NH ON TS.id = NH.id " .
           "  WHERE TS.id "; 
  ...
  */
  function get_all()
  {
    $sql = " SELECT testsuites.*, nodes_hierarchy.name " .
           " FROM ".$this->db->get_table('testsuites')." testsuites, " .
           " ".$this->db->get_table('nodes_hierarchy')." nodes_hierarchy " . 
  ...
  */
  function copy_to($id, $parent_id, $user_id,$options=null,$mappings=null)
  {
    ...
    // 20170703 add by zhouzhaoxin to process new node_depth_abs info
    $process_id_list = array();

    $process_id_list[] = $new_tsuite_id;
    ...
    while (count($process_id_list, COUNT_NORMAL) > 0)
    {
        foreach ($process_id_list as $key => $rep_node_id)
        {
            ...
            $sub_count = $this->tree_manager->get_subtree_count($node_parent);
            $parent_abs = $this->tree_manager->get_node_abs_by_id($node_parent);
            if ($sub_count > 9998) {
                return -1;
            }
  ...
  */
  function getKeywords($id,$kw_id = null)
  {
    ...
    $sql = "/* $debugMsg */ SELECT keyword_id,keywords.keyword, notes " .
           " FROM ".$this->db->get_table('object_keywords').", ".$this->db->get_table('keywords')." keywords " .
  ...
  */
  ...
  function addKeyword($id,$kw_id)
  {
    ...
    if( ($doLink = !sizeof($kw)) )
    {
      $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('object_keywords')." " .
  ...
  */
  ...
  {
    $sql = " DELETE FROM ".$this->db->get_table('object_keywords')." WHERE fk_id = {$id} ";
    if (!is_null($kw_id))
    {
      $sql .= " AND keyword_id = {$kw_id}";
    } 
  ...
  */
  ...
  function copy_cfields_values($source_id,$target_id)
  {
    ...
    {
      $sql = "/* $debugMsg */ " . 
             " INSERT INTO ".$this->db->get_table('cfield_design_values')." " . 
             " (field_id,value,node_id) " .
             " SELECT field_id,value,{$target_id} AS target_id" .
             " FROM ".$this->db->get_table('cfield_design_values')." " .
