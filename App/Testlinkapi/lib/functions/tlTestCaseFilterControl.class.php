...
class tlTestCaseFilterControl extends tlFilterControl {
  ...
  private $all_filters = array('filter_tc_id' => array("POST", tlInputParameter::STRING_N,0,30),
                               ...
                               //20170611 modify by zhouzhaoxin to hide cf filter
                               'filter_custom_fields' => array("POST", tlInputParameter::ARRAY_STRING_N),
                               'filter_result' => null,
                               'filter_assign_status' => array("POST", tlInputParameter::INT_N),
  ...
  private $mode_filter_mapping = array('edit_mode' => array('filter_tc_id',
                                       ...
                                       'plan_add_mode' => array('filter_tc_id',
                                                                ...
                                                                //20170611 modify by zhouzhaoxin to hide cf filter
                                                                //'filter_custom_fields',
                                                                'filter_assign_status'),
                                        'plan_assignedkword_mode' => array('filter_tc_id',
                                                            'filter_testcase_name',
                                                            'filter_toplevel_testsuite',
                                                            'filter_workflow_status',
                                                            'filter_importance',
   ...
   * @var array
   * history: 20160603 zhaozhaoxin add setting_build to plan_add_mode for assign tc to build not only to plan
   */
  ...
  protected function init_args() 
  {
    ...
    //modify by zhouzhaoxin 20160707 for change testcase assign to build, so no need to filter build
    //$extra_keys = array('filter_result_result','filter_result_method','filter_result_build');
    $extra_keys = array('filter_result_result','filter_result_method');
  ...
  protected function init_settings() 
  {
    ...
    // modify by zhouzhaoxin 20160603 for assing testcases to build not only on plan, so setting_build should be true
  ...
  protected function init_filters() 
  {
    ...
    // modify by zhouzhaoxin 20160607 to delele filter result build becasue filter add to setting now
    ...
    switch ($this->mode) 
    {
      case 'plan_mode':
        ...
        switch($this->args->feature)
        {
          ...
          case 'planUpdateTC':
            ...
            $filters->filtersingletc = 1;
          break;
          
        }
        
        // add by zhouzhaoxin 20161212 to add build id to filter the testcaset by build not only by plan   
      ...
      */
      ...
      default:
        ...
        // add by zhouzhaoxin 20160607 to add build id to filter the testcaset by build not only by plan
        $filters->filter_build_id = $this->args->setting_build;

        list($tree_menu, $testcases_to_show) = execTree($this->db, $this->args->setting_build, $gui->menuUrl,
  ...
  private function init_setting_refresh_tree_on_action() 
  {
    ...
    if (isset($this->args->{$key})) {
      ...
      // add by zhouzhaoxin 20170601 to disable auto refresh on execute page
      if ($this->mode == 'execution_mode')
      {
          $selection = 0;
      }
  ...
  private function init_setting_build() 
  {
    ...
    // modify by zhouzhaoxin 20160603 for assign testcases to build
  ...
  private function init_setting_testplan() 
  {
    ...
    foreach ($testplans as $plan) 
    {
      ...
      // modify by zhouzhaoxin 20160606 for testcases belong to build not to plan, so should show build info
      //if(!$add_plan) 
      //{
  ...
  private function init_filter_keywords() 
  {
    ...
    switch ($this->mode) 
    {
      ...
      //20170607 add by zhouzhaoxin to improve performance
      case 'execution_mode':
          ...
          $tplan_id = $this->settings['setting_testplan']['selected'];
          $keywords = $this->testplan_mgr->get_keywords_map_for_project($this->args->testproject_id, ' ORDER BY keyword ');
          break;
   ...
   * @internal revisions
   * @since 20170501
   * filter assign status for plan_add_mode
   */
  private function init_filter_assign_status()
  {
      $key = 'filter_assign_status';
      ...
      $this->filters[$key]['items']['1'] = lang_get('tc_assigned_to_build'); 
      $this->filters[$key]['items']['2'] = lang_get('tc_not_assigned_to_build');
      $this->filters[$key]['items'] = array(0 => $this->option_strings['any'])
                                          + $this->filters[$key]['items'];
