...
class cfield_mgr extends tlObject
{
  ...
  */
                                        ...
                                        $node_type=null,$node_id=null,$access_key='id')
  {
    ...
    {
      $additional_join .= " JOIN ".$this->db->get_table('cfield_node_types')." CFNT ON CFNT.field_id=CF.id " .
    ...
    if( !is_null($node_id) )
    {
      ...
      $additional_join .= " LEFT OUTER JOIN ".$this->db->get_table($table_key)." CFDV ON CFDV.field_id=CF.id " .
...
//          " JOIN {$this->tables['cfield_testprojects']} CFTP ON CFTP.field_id=CF.id " .
         " FROM ".$this->db->get_table($this->object_table)." CF " .
         " JOIN ".$this->db->get_table('cfield_testprojects')." CFTP ON CFTP.field_id=CF.id " .
         $additional_join .
...
//         $sql = "/* $debugMsg */ SELECT value FROM {$this->tables[$table_key]} " .
        $sql = "/* $debugMsg */ SELECT value FROM ".$this->db->get_table($table_key)." " .
        ...
        if( $rowCount > 0 ) 
        {
          if( $value != "" )
          {
            $sql = "/* $debugMsg */ UPDATE ".$this->db->get_table($table_key)." " .
                   " SET value='{$safe_value}' ";
          }  
          else
          {
            // bye, bye record
            $sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table($table_key)." ";
          }  
        ...
        else if ($rowCount == 0 && $value != "")
        {
          # Remark got from Mantis code:
  		    ...
  		    $sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table($table_key)." " .
...
//          "     {$this->tables['node_types']} NT " .
         " FROM ".$this->db->get_table($this->object_table)." CF, " .
         "     ".$this->db->get_table('cfield_node_types')." CFNT, " .
         "     ".$this->db->get_table('node_types')." NT " .
...
//          "      {$this->tables['node_types']} NT " .
         " FROM ".$this->db->get_table($this->object_table)." CF, " .
         "      ".$this->db->get_table('cfield_testprojects')." CFTP, " .
         "      ".$this->db->get_table('cfield_node_types')." CFNT, " .
         "      ".$this->db->get_table('node_types')." NT " .
         " WHERE CF.id=CFNT.field_id " .
  ...
  */
    ...
    $tproject_info = $this->tree_manager->get_node_hierarchy_info($safeID);
		...
		{
			$sql = "/* $debugMsg */ INSERT INTO ".$this->db->get_table('cfield_testprojects')." " .
  ...
  */
    ...
    $tproject_info = $this->tree_manager->get_node_hierarchy_info($tproject_id);
		...
		foreach($cfield_ids as $field_id)
		{
			$sql = "/* $debugMsg */ UPDATE ".$this->db->get_table('cfield_testprojects')." " .
	...
	function unlink_from_testproject($tproject_id,$cfield_ids)
  {
  	...
  	$tproject_info = $this->tree_manager->get_node_hierarchy_info($tproject_id);
		foreach($cfield_ids as $field_id)
		{
			$sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('cfield_testprojects')." " .
...
// 	  	     " {$this->tables['node_types']} NT" .
	  	     " FROM ".$this->db->get_table('custom_fields')." CF, ".$this->db->get_table('cfield_node_types')." CFNT," .
	  	     " ".$this->db->get_table('node_types')." NT" .
	  	     " WHERE CF.id=CFNT.field_id " .
...
// 	  	     "      {$this->tables['nodes_types']} NT " .
	  	     " FROM ".$this->db->get_table('cfield_node_types')." CFNT, " .
	  	     "      ".$this->db->get_table('nodes_types')." NT " .
	...
	function create($cf)
  {
    ...
    if($ncv[$safecf['node_type_id']] == 'build')
    {
      ...
      // at least for Postgres DBMS table name is needed.
	    ...
	    $sql="/* $debugMsg */ INSERT INTO ".$this->db->get_table('cfield_node_types')." " .
  ...
  */
    ...
    if($ncv[$safecf['node_type_id']] == 'build')
    {
      ...
      $safecf['enable_on_execution'] = 0;
		...
		$sql =	"UPDATE ".$this->db->get_table('custom_fields')."  " .
		...
		if ($result)
		{
			$sql = 	"UPDATE ".$this->db->get_table('cfield_node_types')." " .
   ...
   */
    ...
    if( $this->is_used($id) )
    {
      $this->remove_all_scopes_values($id);
		...
		$sql="DELETE FROM ".$this->db->get_table('cfield_node_types')." WHERE field_id={$id}";
		$result=$this->db->exec_query($sql);
		if($result)
		{
			$sql="DELETE FROM ".$this->db->get_table('custom_fields')." WHERE id={$id}";
			$result=$this->db->exec_query($sql);
		}
		return $result ? 1 : 0;
	}
...
// 	  $sql="SELECT field_id FROM {$this->tables['cfield_design_values']} " .
	  $sql="SELECT field_id FROM ".$this->db->get_table('cfield_design_values')." " .
	       "WHERE  field_id={$id} " .
         "UNION " .
//          "SELECT field_id FROM {$this->tables['cfield_build_design_values']} " .
         "SELECT field_id FROM ".$this->db->get_table('cfield_build_design_values')." " .
         "WHERE  field_id={$id} " .
	       "UNION " .
// 	       "SELECT field_id FROM {$this->tables['cfield_testplan_design_values']} " .
	       "SELECT field_id FROM ".$this->db->get_table('cfield_testplan_design_values')." " .
	       "WHERE  field_id={$id} " .
	       "UNION " .
// 	       "SELECT field_id FROM {$this->tables['cfield_execution_values']} " .
	       "SELECT field_id FROM ".$this->db->get_table('cfield_execution_values')." " .
	       "WHERE  field_id={$id} ";
	  $result=$this->db->exec_query($sql);
  ...
  */
                                           ...
                                           $access_key='id',$location=null)
  {
    ...
    if( !is_null($node_id) && !is_null($execution_id) && !is_null($testplan_id) )
    {
        ...
        $additional_join .= " LEFT OUTER JOIN ".$this->db->get_table('cfield_execution_values')." CFEV ON CFEV.field_id=CF.id " .
    ...
    else
    {
        if( !is_null($testplan_id) )
        {
          ...
          $additional_join .= " JOIN ".$this->db->get_table('cfield_execution_values')." CFEV ON CFEV.field_id=CF.id " .
                              " AND CFEV.testplan_id={$testplan_id} " .
                              " JOIN ".$this->db->get_table('executions')." EXECU ON CFEV.tcversion_id = EXECU.tcversion_id " .
                              " AND CFEV.execution_id = EXECU.id " ;
            
          $additional_join .= " JOIN ".$this->db->get_table('builds')." B ON B.id = EXECU.build_id " .
                              " AND B.testplan_id = EXECU.testplan_id " ;

          $additional_join .= " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.version = EXECU.tcversion_number " .
	                            " AND TCV.id = EXECU.tcversion_id " ;
            
          $additional_join .= " JOIN ".$this->db->get_table('users')." U ON  U.id = EXECU.tester_id " .
                              " JOIN ".$this->db->get_table('nodes_hierarchy')." NHA ON NHA.id = EXECU.tcversion_id " .
                              " JOIN ".$this->db->get_table('nodes_hierarchy')." NHB ON NHB.id = NHA.parent_id  " ;

          // Use left join, if platforms is not used platform_name will become null
          $additional_join .= " LEFT JOIN ".$this->db->get_table('platforms')." PLAT ON EXECU.platform_id = PLAT.id";
...
//            " JOIN {$this->tables['cfield_testprojects']} CFTP ON CFTP.field_id=CF.id " .
           " FROM ".$this->db->get_table('custom_fields')." CF " .
           " JOIN ".$this->db->get_table('cfield_testprojects')." CFTP ON CFTP.field_id=CF.id " .
           $additional_join .
...
//                " FROM {$this->tables['cfield_execution_values']} " . $where_clause;
        $sql = " SELECT value,field_id,execution_id " .
            " FROM ".$this->db->get_table('cfield_execution_values')." " . $where_clause;
        ...
        {
          $sql = " UPDATE ".$this->db->get_table('cfield_execution_values')." " .
        ...
        else if (count($rs) == 0 && $value != "")
        {
          ...
          # Remark got from Mantis code:
  		    ...
  		    # This is important, as the definitions might change but the
  		    #  values stored with a bug must not change
  		    $sql = "INSERT INTO ".$this->db->get_table('cfield_execution_values')." " .
        ...
        {
  			  $sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('cfield_execution_values')." " . $where_clause;
  			  $this->db->exec_query($sql);
  		  }
 ...
 */
 function set_display_order($tproject_id, $map_field_id_display_order)
 {
    ...
    {
		$sql = "UPDATE ".$this->db->get_table('cfield_testprojects')."  " .
...
// 					" JOIN {$this->tables['custom_fields']}  cf ON " .
					" FROM ".$this->db->get_table('cfield_testplan_design_values')." cfv " .
					" JOIN ".$this->db->get_table('custom_fields')."  cf ON " .
					" cfv.field_id = cf.id " .
...
// 					" JOIN {$this->tables['custom_fields']}  cf ON " .
					" FROM ".$this->db->get_table('cfield_design_values')." cfv " .
					" JOIN ".$this->db->get_table('custom_fields')."  cf ON " .
					" cfv.field_id = cf.id " .
					" WHERE cf.name LIKE '{$likeTarget}' " .
...
// 	    $sql = "SELECT value FROM {$this->tables['cfield_testplan_design_values']} " .
	    $sql = "SELECT value FROM ".$this->db->get_table('cfield_testplan_design_values')." " .
	    ...
	    {
	      $sql = "UPDATE ".$this->db->get_table('cfield_testplan_design_values')." " .
	             " SET value='{$safe_value}' " .
			     " WHERE field_id={$field_id} AND	link_id={$link_id}";
	      $this->db->exec_query($sql);
	    }
	    ...
	    else if ($this->db->num_rows( $result ) == 0 && $value != "")
	    {
	      # Remark got from Mantis code:
		    ...
		    # This is important, as the definitions might change but the
		    #  values stored with a bug must not change
		    $sql = "INSERT INTO ".$this->db->get_table('cfield_testplan_design_values')." " .
				   " ( field_id, link_id, value ) " .
				   " VALUES	( {$field_id}, {$link_id}, '{$safe_value}' )";
		    $this->db->exec_query($sql);
	    // BUGID 3989
        } else if ($this->db->num_rows( $result ) > 0 && $value == "") {
  			$sql = "/* $debugMsg */ DELETE FROM ".$this->db->get_table('cfield_testplan_design_values')." " .
  				   " WHERE field_id={$field_id} AND	link_id={$link_id}";
  			$this->db->exec_query($sql);
  		}
  ...
  */
                                                 ...
                                                 $link_id=null,$testplan_id=null,$access_key = 'id')
  {
    ...
    if( is_null($link_id) && !is_null($testplan_id))
    {
        ...
        $additional_join .= "JOIN ".$this->db->get_table('testplan_tcversions')." TPTC" .
                            " ON TPTC.testplan_id = {$testplan_id}" .
        				            " JOIN ".$this->db->get_table('cfield_testplan_design_values')." CFTDV " .
                            " ON CFTDV.field_id=CF.id " .
                            " AND CFTDV.link_id = TPTC.id ";
        
        $additional_join .= " JOIN ".$this->db->get_table('tcversions')." TCV ON TCV.id = TPTC.tcversion_id " .
		                        " AND TCV.id = TPTC.tcversion_id " .
         					          " JOIN ".$this->db->get_table('nodes_hierarchy')." NHA ON NHA.id = TPTC.tcversion_id " .
                            " JOIN ".$this->db->get_table('nodes_hierarchy')." NHB ON NHB.id = NHA.parent_id  " ;
    ...
    {
        $additional_values .= ",CFTDV.value AS value, CFTDV.link_id AS node_id";
        $additional_join .= " LEFT OUTER JOIN ".$this->db->get_table('cfield_testplan_design_values')." CFTDV " .
                            " ON CFTDV.field_id=CF.id " .
...
function remove_all_scopes_values($id)
{
  ...
  {
    $sql = "DELETE FROM ".$this->db->get_table($tt)." WHERE field_id={$safe_id} ";
    $this->db->exec_query($sql);        
  }
}
...
//          " FROM {$this->tables['cfield_testprojects']} CFTP, {$this->tables['nodes_hierarchy']} NH " .
         " FROM ".$this->db->get_table('cfield_testprojects')." CFTP, ".$this->db->get_table('nodes_hierarchy')." NH " .
...
function getByLinkID($linkID, $options=null)
{
	...
	{
		case 'design':
			 $sql .= " ".$this->db->get_table('cfield_design_values')." " .
			         " WHERE node_id = {$linkID} ";
		break;
		
		case 'testplan_design':
			 $sql .= " ".$this->db->get_table('cfield_testplan_design_values')." " .
			         " WHERE feature_id = {$linkID} ";
		break;
		
		case 'execution':
			 $sql .= " ".$this->db->get_table('cfield_execution_values')." " .
