...
function get_tproject_effective_role(&$db,$tproject,$user_id = null,$users = null)
{
  ...
  {
    // add by zhouzhaoxin 20160608 to reduce the load time for user init
//     $whereClause = "where id in (select distinct user_id from user_testproject_roles where testproject_id = " . 
    $whereClause = "where id in (select distinct user_id from ".$db->get_table('user_testproject_roles')." where testproject_id = " .
        $tproject_id . ")";
    $users = tlUser::getAll($db, $whereClause);
  }
