...
if(!is_null($linked_tcversions))
{
      ...
      $args->doMoveNext || $args->doMovePrevious)
  {
    ...
    {  
      ...
      if (isset($args->filter_assigned_to)
          && isset($args->filter_assigned_to[$args->user_id])
          && $args->filter_assigned_to[$args->user_id] > 0
          && isset($args->include_unassigned)
      ...
      $nextItem = $tplan_mgr->getTestCaseNextSibling($args->build_id,$tcversion_id,$args->platform_id,$opt4sibling);
      if(!$doSingleStep)
      { 
        while (!is_null($nextItem) && !in_array($nextItem['tcase_id'], $args->testcases_to_show)) 
        {
          $nextItem = $tplan_mgr->getTestCaseNextSibling($args->build_id,$nextItem['tcversion_id'],
  ...
  {  
	...
	$gui->flush_edit_testcase_id = $tcase_id;
...
function get_ts_name_details(&$db,$tcase_id)
{
  ...
  $sql = "SELECT TS.id AS tsuite_id, TS.details, 
             ...
             FROM ".$db->get_table('testsuites')." TS, ".$db->get_table('nodes_hierarchy')." NHA, 
             ".$db->get_table('nodes_hierarchy')." NHB
...
function do_remote_execution(&$dbHandler,$context)
{
  ...
  $myResult = array();
  $sql = " /* $debugMsg */ INSERT INTO " . $dbHandler->get_table('executions')." " . 
...
function initializeGui(&$dbHandler,&$argsObj,&$cfgObj,&$tplanMgr,&$tcaseMgr,&$issueTracker)
{
  ...
  //add testcase_id by steven.zhou start
  $gui->translate_edit_tcase_id=$argsObj->tc_id;
  $gui->translate_edit_version_id=$argsObj->version_id;
  //add testcase_id by steven.zhou end  
