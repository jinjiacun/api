...
require_once('../results/displayMgr.php');
...
function initializeGui(&$dbHandler,&$args,$images)
{
  ...
  switch($args->do_action)
  {
    ...
    case 'result':
      //initializeGuiForInput($dbHandler,$args,$gui);
      initializeGuiForResult($dbHandler,$args,$gui, 1);
 ...
 *
 */
function initializeGuiForResult(&$dbHandler,$argsObj,&$guiObj, $submit = 0)
{
  ...
  $guiObj->searchDone = 1;
  if($submit == 1){
    if($mgr->updateCheckerIdOnExecution($_POST['execution_ids'])){
      echo '复核成功';
    } else{
      echo '复核失败';
    }
  ...
  $guiObj->resultSet = $mgr->getTestCaseExection($argsObj->tproject_id,$argsObj->user_id,$options);
