...
//       $query = "SELECT id FROM {$this->tables['testplans']} WHERE id={$tplanid}";
      $query = "SELECT id FROM " . $this->dbObj->get_table('testplans')." WHERE id={$tplanid}";
...
//         $query = "SELECT id FROM {$this->tables['testprojects']} WHERE id={$testprojectid}";
        $query = "SELECT id FROM " . $this->dbObj->get_table('testprojects')." WHERE id={$testprojectid}";
        $result = $this->dbObj->fetchFirstRowSingleColumn($query, "id");           
        if(null == $result)
        {
...
//                  " {$this->tables['node_types']} NT " .
                 " FROM " . $this->dbObj->get_table('nodes_hierarchy')." NH, " .
                 " " . $this->dbObj->get_table('node_types')." NT " .
...
//           $query = "SELECT id FROM {$this->tables['users']} WHERE script_key='{$this->devKey}'";
          $query = "SELECT id FROM " . $this->dbObj->get_table('users')." WHERE script_key='{$this->devKey}'";
...
//         $sql = " SELECT MAX(id) AS exec_id FROM {$this->tables['executions']} " .
        $sql = " SELECT MAX(id) AS exec_id FROM ".$this->dbObj->get_table('executions')." " .
               " WHERE testplan_id = {$this->args[self::$testPlanIDParamName]} " .
               " AND tcversion_id IN (" .
//                " SELECT id FROM {$this->tables['nodes_hierarchy']} " .
               " SELECT id FROM ".$this->dbObj->get_table('nodes_hierarchy')." " .
...
//           $sql = "SELECT * FROM {$this->tables['executions']} WHERE id=" . $targetID;
          $sql = "SELECT * FROM " . $this->dbObj->get_table('executions')." WHERE id=" . $targetID;
...
//             $sql = " SELECT DISTINCT bug_id FROM {$this->tables['execution_bugs']} " . 
            $sql = " SELECT DISTINCT bug_id FROM " . $this->dbObj->get_table('execution_bugs')." " .
  ...
  protected function _insertResultToDB($user_id=null,$exec_ts=null)
  {
    ...
    $query = "INSERT INTO " . $this->dbObj->get_table('executions')." " .
...
//     $sql = " SELECT keyword,id FROM {$this->tables['keywords']} " .
    $sql = " SELECT keyword,id FROM " . $this->dbObj->get_table('keywords')." " .
...
//        $sql=" SELECT id FROM {$this->tables['testplans']}" .
       $sql=" SELECT id FROM " . $this->dbObj->get_table('testplans') .
...
//              " FROM {$this->tables['nodes_hierarchy']} NH, {$this->tables['tcversions']} TCV " .
             " FROM " . $this->dbObj->get_table('nodes_hierarchy')." NH, ".$this->dbObj->get_table('tcversions')." TCV " .
...
//              " LEFT OUTER JOIN {$this->tables['platforms']} PLAT ON PLAT.id = platform_id " . 
      $sql = " SELECT tcversion_id, platform_id, PLAT.name FROM " . $this->dbObj->get_table('testplan_tcversions')." TPTCV " .
             " LEFT OUTER JOIN ". $this->dbObj->get_table('platforms')." PLAT ON PLAT.id = platform_id " .
      ...
      {
        $in_clause=implode(",",$id_set);
        $sql=" DELETE FROM " . $this->dbObj->get_table('testplan_tcversions')." " .
      ...
      {  
        ...
        $sql=" INSERT INTO " . $this->dbObj->get_table('testplan_tcversions')." ({$fields}) VALUES({$sql_values})"; 
        $this->dbObj->exec_query($sql);
...
//     $sql="SELECT id FROM {$this->tables['executions']} WHERE id={$executionID}";
    $sql="SELECT id FROM " . $this->dbObj->get_table('executions')." WHERE id={$executionID}";
...
//       $sql="SELECT execution_id FROM {$this->tables['execution_bugs']} " .  
      $sql="SELECT execution_id FROM " . $this->dbObj->get_table('execution_bugs')." " .
           "WHERE execution_id={$executionID} AND bug_id='{$safeBugID}'";
        
      if( is_null($this->dbObj->fetchRowsIntoMap($sql, 'execution_id')) )
      {
        $sql = "INSERT INTO " . $this->dbObj->get_table('execution_bugs')." " .
...
//                " {$this->tables['executions']} E, {$this->tables['builds']} B ".
               "FROM " . $this->dbObj->get_table('execution_bugs')." ," .
               " " . $this->dbObj->get_table('executions')." E, " . $this->dbObj->get_table('builds')." B ".
...
//     $sql="SELECT id FROM {$this->tables['executions']} WHERE id={$executionID}";
  ...
  protected function _updateResult($user_id=null,$exec_ts=null)
  {
    ...
    if( !is_null($exec_id) )
    {
      ...
      $sql = " UPDATE " . $this->dbObj->get_table('executions')." " .
...
//     $query = "SELECT id FROM {$this->tables[$fkTable]} WHERE id={$fkId}";
    $query = "SELECT id FROM " . $this->dbObj->get_table($fkTable)." WHERE id={$fkId}";
    $result = $this->dbObj->fetchFirstRowSingleColumn($query, "id");
  }
...
//             $sql = " SELECT id FROM {$this->tables['nodes_hierarchy']} " .
            $sql = " SELECT id FROM " . $this->dbObj->get_table('nodes_hierarchy')." " .
                   " WHERE id = " . $tplanid .
...
//       $sql = " SELECT id,tcversion_id FROM {$this->tables['executions']} " .
      $sql = " SELECT id,tcversion_id FROM " . $this->dbObj->get_table('executions')." " .
...
//        $sql = " SELECT id AS exec_id FROM {$this->tables['executions']} " . 
       $sql = " SELECT id AS exec_id FROM " . $this->dbObj->get_table('executions')." " .
              " WHERE testplan_id = {$this->args[self::$testPlanIDParamName]} " .
              " AND tcversion_id IN (" .
//               " SELECT id FROM {$this->tables['nodes_hierarchy']} " .
              " SELECT id FROM " . $this->dbObj->get_table('nodes_hierarchy')." " .
