...
if (isset($args->testprojectID))
{
    $gui->users = tlUser::getAll($db,"WHERE active=1 and id in " .
        "(select distinct user_id from " . $db->get_table('user_testproject_roles')." where testproject_id = " . $args->testprojectID . ")",
        null,null,tlUser::TLOBJ_O_GET_DETAIL_MINIMUM);
}
else 
{
