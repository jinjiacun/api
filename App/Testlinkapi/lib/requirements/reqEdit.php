...
function init_args(&$dbHandler)
{
  ...
  $iParams = array("requirement_id" => array(tlInputParameter::INT_N),
                   ...
                   "testplans_id" => array(tlInputParameter::INT_N), //new chy 160706
                   "business_rules" => array(tlInputParameter::STRING_N),//new chy 160706
                   "add_remaks" => array(tlInputParameter::STRING_N),//new chy 160706
                   "add_definitions" => array(tlInputParameter::STRING_N),//new chy 160706
  ...
  //add by chy 0706 get proj_tsplan
  $args->user = $_SESSION['currentUser'];
  $args->arrPlans = $args->user->getAccessibleTestPlans($dbHandler,$args->tproject_id);
...
function renderGui(&$argsObj,$guiObj,$opObj,$templateCfg,$editorCfg,&$dbHandler)
{
  ...
  //add element message for req by chy 0706
  $owebEditor_br = web_editor('business_rules',$argsObj->basehref,$editorCfg) ;
  $owebEditor_ar = web_editor('add_remaks',$argsObj->basehref,$editorCfg) ;
  $owebEditor_ad = web_editor('add_definitions',$argsObj->basehref,$editorCfg) ;
  ...
  switch($argsObj->doAction)
  {
    ...
    case "doCreate":
      ...
      $owebEditor_br->Value=$argsObj->business_rules;
      $owebEditor_ar->Value=$argsObj->add_remaks;
      $owebEditor_ad->Value=$argsObj->add_definitions;
    ...
    default:
      ...
      else
      {
        $owebEditor->Value = getItemTemplateContents('requirement_template',$owebEditor->InstanceName, 
                                                     $argsObj->scope);
        //new element chy0706
        $owebEditor_br->Value= getItemTemplateContents('requirement_template',$owebEditor_br->InstanceName,
            $argsObj->business_rules);
        $owebEditor_ar->Value=getItemTemplateContents('requirement_template',$owebEditor_ar->InstanceName,
            $argsObj->add_remaks);
        $owebEditor_ad->Value=getItemTemplateContents('requirement_template',$owebEditor_ad->InstanceName,
            $argsObj->add_definitions);
      }
    break;
  }
  ...
  //new element chy0706
  $guiObj->business_rules = $owebEditor_br->CreateHTML();
  $guiObj->add_remaks = $owebEditor_ar->CreateHTML();
  $guiObj->add_definitions = $owebEditor_ad->CreateHTML();
  $guiObj->editorType = $editorCfg['type'];
  
  //set tplans for curproject add by chy 0706
  $guiObj->tplanset = $argsObj->arrPlans;
  $guiObj->sel_testplan_id = 0;
  if (isset($opObj->sel_testplan_id) && $opObj->sel_testplan_id != 0)
  {
      $guiObj->sel_testplan_id = $opObj->sel_testplan_id;
  }
  else
  {
      $guiObj->sel_testplan_id = $_SESSION['testplanID'];
  }
  
  if( $guiObj->sel_testplan_id > 0)
  {
      $loop2do=count($guiObj->tplanset);
      for($idx=0; $idx < $loop2do; $idx++)
      {
          if ($guiObj->tplanset[$idx]['id'] == $guiObj->sel_testplan_id )
          {
              $guiObj->tplanset[$idx]['selected'] = 1;
              break;
          }
      }
  }
