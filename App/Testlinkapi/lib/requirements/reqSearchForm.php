...
$tplan_list = $tproject_mgr->get_all_testplans($args->tprojectID);
$items[0] = '-All-';
$selection = $args->{'test_plan'};
if (count($tplan_list, COUNT_NORMAL) > 0)
{
    foreach ($tplan_list as $tplan_id => $tplan_info)
    {
        $items[$tplan_id] = $tplan_info['name'];
    }
}
if (!$selection || $args->reset_filters || $selection == 0)
{
    $selection = 0;
}
else
{
    $gui->do_filtering = true;
}
$gui->filter_by['test_plan'] = array('selected' => $selection, 'items' => $items);
...
{              
    ...
    $args->test_plan = isset($_SESSION['test_plan']) ? $_SESSION['test_plan'] : 0;
