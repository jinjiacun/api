...
class reqCommands
{
  ...
  */
  function edit(&$argsObj,$request,$overwriteArgs=true)
  {
    ...
    $argsObj->business_rules = $obj->req['business_rules'];
    $argsObj->add_remaks = $obj->req['add_remaks'];
    $argsObj->add_definitions = $obj->req['add_definitions'];
    }
    ...
    $obj->sel_testplan_id = $obj->req['testplans_id'];
  ...
  */
  function doCreate(&$argsObj,$request)
  {
    ...
    $ret = $this->reqMgr->create($argsObj->req_spec_id,$argsObj->reqDocId,$argsObj->title,
                                 ...
                                 $argsObj->business_rules,$argsObj->add_remaks,$argsObj->add_definitions,
                                 $sel_plan);
  ...
  function doUpdate(&$argsObj,$request)
  {
    ...
    else if( $diff['nochange'] || ( ($createRev = $diff['force'] && !$obj->prompt_for_log) || $argsObj->do_save ) )
    {
      ...
      $ret = $this->reqMgr->update($argsObj->req_id,$argsObj->req_version_id,
          ...
          $argsObj->business_rules,$argsObj->add_remaks,$argsObj->add_definitions,$sel_plan);
  ...
  */
  ...
  function addTestCase(&$argsObj,$request)
  {
    ...
    if (isset($argsObj->req_id) || $argsObj->req_id == "" || $argsObj->req_id == 0)
    {
        $argsObj->req_id = $request['cur_reqid'];
    }
    
    if (isset($argsObj->req_version_id) || $argsObj->req_version_id == "" || $argsObj->req_version_id == 0)
    {
        $argsObj->req_version_id = $request['cur_req_ver'];
    }
    ...
    $externalid=$request['saveitem'];
    $exid_list=explode("/",$externalid);
    for($i=0;$i<count($exid_list);$i++)
    {
        $argsObj->tcaseIdentity=$exid_list[$i];
        ...
        //die('getmsg:'.$gluePos.';'.$argsObj->tcaseIdentity.';'.$cfg->glue_character);
        ...
        if($status_ok)
        {
            ...
            if($tcase_id > 0)
            {
                //die('dothis:'.$argsObj->req_id.'/'.$tcase_id.'/'.intval($argsObj->user_id));
