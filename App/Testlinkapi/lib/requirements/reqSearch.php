...
function init_args($dateFormat)
{
	...
	$strnull = array('requirement_document_id', 'name','test_plan', 'scope', 'reqStatus',
...
function build_search_sql(&$dbHandler,&$argsObj,&$guiObj)
{
	...
	if(!is_null($argsObj->relation_type)) 
	{
		...
		$from['ver']['relation_type'] = " JOIN ".$dbHandler->get_table('req_relations')." RR " .
	...
	if($argsObj->custom_field_id > 0) 
	{
        ...
        $from['ver']['custom_field'] = 	" JOIN " . $dbHandler->get_table('cfield_design_values')." CFD " .
        								" ON CFD.node_id = REQV.id "; 

        $from['rev']['custom_field'] = 	" JOIN " . $dbHandler->get_table('cfield_design_values')." CFD " .
    ...
    if ($argsObj->tcid != "" && strcmp($argsObj->tcid, $guiObj->tcasePrefix) != 0) 
    {
    	...
    	$from['ver']['tcid'] = 	" /* Look for Req Coverage info */ ".
    							" JOIN " . $dbHandler->get_table('req_coverage')." RC ON RC.req_id = NH_REQ.id " .  
    							" /* Need Test case children => test case versions */ ".
    					   		" JOIN " . $dbHandler->get_table('nodes_hierarchy')." NH_TCV ON NH_TCV.parent_id = RC.testcase_id " .
    							" /* Needed to search using External ID  */ ".
                           		" JOIN " . $dbHandler->get_table('tcversions')." TCV ON TCV.id = NH_TCV.id ";
	...
	$sql = 	$common .
			...
			" FROM " . $dbHandler->get_table('requirements')." REQ " .
			" JOIN " . $dbHandler->get_table('nodes_hierarchy')." NH_REQ ON NH_REQ.id=REQ.id " .
			" /* */ " .
			" /* Need to get all REQ children => REQ Versions */ " .
			" JOIN " . $dbHandler->get_table('nodes_hierarchy')." NH_REQV ON NH_REQV.parent_id = NH_REQ.id " .  
			" /* */ " .
			" /* Go for REQ REV data */ " .
			" JOIN " . $dbHandler->get_table('req_versions')." REQV ON REQV.id=NH_REQV.id " .
			" /* */ ";
	//when options of TestPlan is selected and value is not "All"
	if($argsObj->test_plan != 0){
	    $sql = $sql . " AND REQV.testplans_id=" . $argsObj->test_plan . "";
	}
	...
	$sql = 	$common .
			...
			" JOIN " . $dbHandler->get_table('req_revisions')." REQR ON REQR.parent_id=REQV.id ";
	//when options of TestPlan is selected and value is not "All"
	if($argsObj->test_plan != 0){
	    $sql = $sql . " AND REQR.testplans_id=" . $argsObj->test_plan . "";
	}
	foreach($map2use as $vv)
	{
		$ref = &$$vv;
