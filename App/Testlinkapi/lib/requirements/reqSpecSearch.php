...
if ($args->tprojectID)
{
  ...
  if($args->custom_field_id > 0) {
        ...
        $join['by_custom_field'] = " JOIN " . $db->get_table('cfield_design_values')." CFD " .
  ...
  $sql =  " SELECT NHRSPEC.name, NHRSPEC.id, RSPEC.doc_id, RSPECREV.id AS revision_id, RSPECREV.revision " .
      " FROM " . $db->get_table('req_specs')." RSPEC JOIN " . $db->get_table('req_specs_revisions')." RSPECREV " .   
      " ON RSPEC.id=RSPECREV.parent_id " .
      " JOIN " . $db->get_table('nodes_hierarchy')." NHRSPEC " .
